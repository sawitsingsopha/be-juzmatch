var tempusDominus = (function ($, core, luxon) {
        ...
        do(e, action) {
            ...
            switch (action) {
                ...
                case ActionTypes.today:
                    const today = new DateTime().setLocale(this._context._options.localization.locale, this._context._options.localization.numberingSystem); //***
        ...
        show() {
            ...
            if (this.widget == undefined) {
                if (this._context._options.useCurrent &&
                    ...
                    const date = new DateTime().setLocale(this._context._options.localization.locale, this._context._options.localization.numberingSystem); //***
                    if (!this._context._options.keepInvalid) {
                        let tries = 0;
                        let direction = 1;
    ...
    class TempusDominus {
        constructor(element, options = {}) {
            ...
            this._viewDate.setLocale(this._options.localization.locale, this._options.localization.numberingSystem); //***
            this._unset = true;
        ...
        _initializeOptions(config, mergeTo, includeDataset = false) {
            ...
            config.viewDate = config.viewDate.setLocale(config.localization.locale, config.localization.numberingSystem); //***
                ...
                config.hooks.inputFormat = (_, date) => {
                    if (config.display.format && typeof config.display.format === 'string') { //***
                        let options = {
                            ...
                            numberingSystem: (config.localization.numberingSystem || (new Intl.NumberFormat(config.localization.locale)).resolvedOptions().numberingSystem)
                        };
                        return date ? luxon__namespace.DateTime.fromJSDate(date, options).toFormat(config.display.format) : "";
                    }
                    else {
                        return date.format({
            ...
            if (config.hooks.inputParse === undefined) { //***
                if (config.display.format && typeof config.display.format == 'string') {
                    config.hooks.inputParse = (_, value) => {
                        ...
                        let parsedDate = new DateTime(luxon__namespace.DateTime.fromFormat(value, config.display.format, options).toJSDate());
                        ...
                        if (parsedDate && parsedDate.getTime() === parsedDate.getTime())
                            return parsedDate.setLocale(config.localization.locale, config.localization.numberingSystem); //***
                        else
                            return value;
                    };
                }
