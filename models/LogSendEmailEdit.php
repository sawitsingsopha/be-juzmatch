 ...
 * Page class
 */
class LogSendEmailEdit extends LogSendEmail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogSendEmailEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("log_email_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->log_email_id->setQueryStringValue($keyValue);
                $this->log_email_id->setOldValue($this->log_email_id->QueryStringValue);
            } elseif (Post("log_email_id") !== null) {
                $this->log_email_id->setFormValue(Post("log_email_id"));
                $this->log_email_id->setOldValue($this->log_email_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->log_email_id->IsDetailKey) {
            $this->log_email_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->log_email_id->CurrentValue = $this->log_email_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->log_email_id->Required) {
            if (!$this->log_email_id->IsDetailKey && EmptyValue($this->log_email_id->FormValue)) {
                $this->log_email_id->addErrorMessage(str_replace("%s", $this->log_email_id->caption(), $this->log_email_id->RequiredErrorMessage));
            }
        }
        if ($this->cc->Required) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // cc
            $this->cc->setDbValueDef($rsnew, $this->cc->CurrentValue, null, $this->cc->ReadOnly);
            ...
            // email_from
            $this->email_from->setDbValueDef($rsnew, $this->email_from->CurrentValue, "", $this->email_from->ReadOnly);

            // email_to
            $this->email_to->setDbValueDef($rsnew, $this->email_to->CurrentValue, "", $this->email_to->ReadOnly);
