...
class AudittrailList extends Audittrail
{
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`datetime` DESC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->datetime->getSort() != "") {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->user->ViewValue === null) { // Lookup from database
                    $filterWrk = "`users_id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
            ...
            $curVal = trim(strval($this->user->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->user->AdvancedSearch->ViewValue = $this->user->lookupCacheOption($curVal);
            } else {
                $this->user->AdvancedSearch->ViewValue = $this->user->Lookup !== null && is_array($this->user->lookupOptions()) ? $curVal : null;
            }
            if ($this->user->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->user->EditValue = array_values($this->user->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`users_id`" . SearchString("=", $this->user->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
            ...
            $this->_action->EditValue = $this->_action->options(true);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-ew-action=\"highlight\" data-form=\"faudittrailsrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";
    ...
    {
        return $this->datetime->Visible || $this->user->Visible || $this->_action->Visible || $this->_table->Visible || $this->field->Visible;
    }
