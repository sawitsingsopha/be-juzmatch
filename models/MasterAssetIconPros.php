...
/**
 * Table class for master_asset_icon_pros
 */
class MasterAssetIconPros extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'master_asset_icon_pros';
        $this->TableName = 'master_asset_icon_pros';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`master_asset_icon_pros`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->master_asset_icon_pros_id->IsAutoIncrement = true; // Autoincrement field
        $this->master_asset_icon_pros_id->IsPrimaryKey = true; // Primary key field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`master_asset_icon_pros`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->master_asset_icon_pros_id->setDbValue($conn->lastInsertId());
            $rs['master_asset_icon_pros_id'] = $this->master_asset_icon_pros_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('master_asset_icon_pros_id', $rs)) {
                AddFilter($where, QuotedName('master_asset_icon_pros_id', $this->Dbid) . '=' . QuotedValue($rs['master_asset_icon_pros_id'], $this->master_asset_icon_pros_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        return "`master_asset_icon_pros_id` = @master_asset_icon_pros_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->master_asset_icon_pros_id->CurrentValue : $this->master_asset_icon_pros_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->master_asset_icon_pros_id->CurrentValue = $keys[0];
            } else {
                $this->master_asset_icon_pros_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('master_asset_icon_pros_id', $row) ? $row['master_asset_icon_pros_id'] : null;
        } else {
            $val = $this->master_asset_icon_pros_id->OldValue !== null ? $this->master_asset_icon_pros_id->OldValue : $this->master_asset_icon_pros_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@master_asset_icon_pros_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("masterasseticonproslist");
    }
    ...
    {
        global $Language;
        if ($pageName == "masterasseticonprosview") {
            return $Language->phrase("View");
        } elseif ($pageName == "masterasseticonprosedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "masterasseticonprosadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "MasterAssetIconProsView";
            case Config("API_ADD_ACTION"):
                return "MasterAssetIconProsAdd";
            case Config("API_EDIT_ACTION"):
                return "MasterAssetIconProsEdit";
            case Config("API_DELETE_ACTION"):
                return "MasterAssetIconProsDelete";
            case Config("API_LIST_ACTION"):
                return "MasterAssetIconProsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("masterasseticonprosview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("masterasseticonprosview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "masterasseticonprosadd?" . $this->getUrlParm($parm);
        } else {
            $url = "masterasseticonprosadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("masterasseticonprosedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("masterasseticonprosadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("masterasseticonprosdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"master_asset_icon_pros_id\":" . JsonEncode($this->master_asset_icon_pros_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->master_asset_icon_pros_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->master_asset_icon_pros_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("master_asset_icon_pros_id") ?? Route("master_asset_icon_pros_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->master_asset_icon_pros_id->CurrentValue = $key;
            } else {
                $this->master_asset_icon_pros_id->OldValue = $key;
            }
