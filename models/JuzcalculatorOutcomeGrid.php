 ...
 * Page class
 */
class JuzcalculatorOutcomeGrid extends JuzcalculatorOutcome
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JuzcalculatorOutcomeGrid";
    ...
    public $FormName = "fjuzcalculator_outcomegrid";
    ...
    {
        $this->outcome->FormValue = ""; // Clear form value
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->juzcalculator_outcome_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_outcome_title") && $CurrentForm->hasValue("o_outcome_title") && $this->outcome_title->CurrentValue != $this->outcome_title->OldValue) {
            return false;
        }
    ...
    {
        $this->outcome_title->clearErrorMessage();
        $this->outcome->clearErrorMessage();
    ...
    {
        $this->juzcalculator_outcome_id->CurrentValue = null;
        $this->juzcalculator_outcome_id->OldValue = $this->juzcalculator_outcome_id->CurrentValue;
        $this->juzcalculator_id->CurrentValue = null;
        $this->juzcalculator_id->OldValue = $this->juzcalculator_id->CurrentValue;
        $this->outcome_title->CurrentValue = null;
        $this->outcome_title->OldValue = $this->outcome_title->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'outcome_title' first before field var 'x_outcome_title'
        $val = $CurrentForm->hasValue("outcome_title") ? $CurrentForm->getValue("outcome_title") : $CurrentForm->getValue("x_outcome_title");
        if (!$this->outcome_title->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->outcome_title->Visible = false; // Disable update for API request
            } else {
                $this->outcome_title->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_outcome_title")) {
            $this->outcome_title->setOldValue($CurrentForm->getValue("o_outcome_title"));
        }
        ...
        if (!$this->outcome->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->outcome->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'juzcalculator_outcome_id' first before field var 'x_juzcalculator_outcome_id'
        $val = $CurrentForm->hasValue("juzcalculator_outcome_id") ? $CurrentForm->getValue("juzcalculator_outcome_id") : $CurrentForm->getValue("x_juzcalculator_outcome_id");
        if (!$this->juzcalculator_outcome_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->juzcalculator_outcome_id->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->juzcalculator_outcome_id->CurrentValue = $this->juzcalculator_outcome_id->FormValue;
        }
        $this->outcome_title->CurrentValue = $this->outcome_title->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['juzcalculator_outcome_id'] = $this->juzcalculator_outcome_id->CurrentValue;
        $row['juzcalculator_id'] = $this->juzcalculator_id->CurrentValue;
        $row['outcome_title'] = $this->outcome_title->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->outcome_title->EditValue = HtmlEncode($this->outcome_title->CurrentValue);
            ...
            if (strval($this->outcome->EditValue) != "" && is_numeric($this->outcome->EditValue)) {
                ...
                $this->outcome->OldValue = $this->outcome->EditValue;
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->outcome_title->Required) {
            if (!$this->outcome_title->IsDetailKey && EmptyValue($this->outcome_title->FormValue)) {
                $this->outcome_title->addErrorMessage(str_replace("%s", $this->outcome_title->caption(), $this->outcome_title->RequiredErrorMessage));
            }
        }
        ...
        if (!CheckNumber($this->outcome->FormValue)) {
            $this->outcome->addErrorMessage($this->outcome->getErrorMessage(false));
        }
    ...
    protected function deleteRows()
    {
        ...
        foreach ($rsold as $row) {
            ...
            $thisKey .= $row['juzcalculator_outcome_id'];
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // outcome_title
            $this->outcome_title->setDbValueDef($rsnew, $this->outcome_title->CurrentValue, null, $this->outcome_title->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "juzcalculator") {
            $this->juzcalculator_id->CurrentValue = $this->juzcalculator_id->getSessionValue();
        }
        $conn = $this->getConnection();
        ...
        // outcome_title
        $this->outcome_title->setDbValueDef($rsnew, $this->outcome_title->CurrentValue, null, false);
        ...
        if ($this->juzcalculator_id->getSessionValue() != "") {
            $rsnew['juzcalculator_id'] = $this->juzcalculator_id->getSessionValue();
        }
