 ...
 * Page class
 */
class InvestorVerifyView extends InvestorVerify
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InvestorVerifyView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("juzcalculator_id") ?? Route("juzcalculator_id")) !== null) {
            $this->RecKey["juzcalculator_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        $this->juzcalculator_id->setVisibility();
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("juzcalculator_id") ?? Route("juzcalculator_id")) !== null) {
                ...
                $this->RecKey["juzcalculator_id"] = $this->juzcalculator_id->QueryStringValue;
            } elseif (Post("juzcalculator_id") !== null) {
                $this->juzcalculator_id->setFormValue(Post("juzcalculator_id"));
                $this->RecKey["juzcalculator_id"] = $this->juzcalculator_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->juzcalculator_id->setQueryStringValue($keyValue);
                $this->RecKey["juzcalculator_id"] = $this->juzcalculator_id->QueryStringValue;
            } else {
                $returnUrl = "investorverifylist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "investorverifylist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "investorverifylist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
