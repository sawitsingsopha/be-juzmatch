 ...
 * Page class
 */
class MasterBuyerCalculatorList extends MasterBuyerCalculator
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterBuyerCalculatorList";
    ...
    public $FormName = "fmaster_buyer_calculatorlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "masterbuyercalculatoradd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "masterbuyercalculatordelete";
        $this->MultiUpdateUrl = "masterbuyercalculatorupdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->buyer_monthly_payment, $ctrl); // buyer_monthly_payment
            $this->updateSort($this->buyer_monthly_annual_interest, $ctrl); // buyer_monthly_annual_interest
            $this->updateSort($this->buyer_dsr_ratio, $ctrl); // buyer_dsr_ratio
            $this->updateSort($this->buyer_down_payment, $ctrl); // buyer_down_payment
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->master_buyer_calculator_id->setSort("");
                $this->buyer_monthly_payment->setSort("");
                $this->buyer_monthly_annual_interest->setSort("");
                $this->buyer_dsr_ratio->setSort("");
                $this->buyer_down_payment->setSort("");
                $this->cdate->setSort("");
                $this->cip->setSort("");
                $this->cuser->setSort("");
                $this->uip->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmaster_buyer_calculatorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmaster_buyer_calculatorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->master_buyer_calculator_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("buyer_monthly_payment", $this->createColumnOption("buyer_monthly_payment"));
            $option->add("buyer_monthly_annual_interest", $this->createColumnOption("buyer_monthly_annual_interest"));
            $option->add("buyer_dsr_ratio", $this->createColumnOption("buyer_dsr_ratio"));
            $option->add("buyer_down_payment", $this->createColumnOption("buyer_down_payment"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmaster_buyer_calculatorsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmaster_buyer_calculatorsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmaster_buyer_calculatorlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmaster_buyer_calculatorlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
