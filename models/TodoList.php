...
/**
 * Table class for todo_list
 */
class TodoList extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'todo_list';
        $this->TableName = 'todo_list';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`todo_list`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->todo_list_id->InputTextType = "text";
        $this->todo_list_id->IsAutoIncrement = true; // Autoincrement field
        $this->todo_list_id->IsPrimaryKey = true; // Primary key field
        $this->todo_list_id->Sortable = false; // Allow sort
        $this->todo_list_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['todo_list_id'] = &$this->todo_list_id;
        ...
        $this->status->Lookup = new Lookup('status', 'todo_list', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`todo_list`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->todo_list_id->setDbValue($conn->lastInsertId());
            $rs['todo_list_id'] = $this->todo_list_id->DbValue;
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('todo_list_id', $rs)) {
                AddFilter($where, QuotedName('todo_list_id', $this->Dbid) . '=' . QuotedValue($rs['todo_list_id'], $this->todo_list_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->todo_list_id->DbValue = $row['todo_list_id'];
    ...
    {
        $keys = [];
        $val = $current ? $this->todo_list_id->CurrentValue : $this->todo_list_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->todo_list_id->CurrentValue = $keys[0];
            } else {
                $this->todo_list_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('todo_list_id', $row) ? $row['todo_list_id'] : null;
        } else {
            $val = $this->todo_list_id->OldValue !== null ? $this->todo_list_id->OldValue : $this->todo_list_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@todo_list_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("todolistlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "todolistview") {
            return $Language->phrase("View");
        } elseif ($pageName == "todolistedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "todolistadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("todolistview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("todolistview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "todolistadd?" . $this->getUrlParm($parm);
        } else {
            $url = "todolistadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("todolistedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("todolistadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("todolistdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"todo_list_id\":" . JsonEncode($this->todo_list_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->todo_list_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->todo_list_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("todo_list_id") ?? Route("todo_list_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->todo_list_id->CurrentValue = $key;
            } else {
                $this->todo_list_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->todo_list_id->setDbValue($row['todo_list_id']);
    ...
    public function renderListRow()
    {
        ...
        // todo_list_id
        $this->todo_list_id->CellCssStyle = "white-space: nowrap;";
        ...
        // todo_list_id
        $this->todo_list_id->ViewValue = $this->todo_list_id->CurrentValue;
        $this->todo_list_id->ViewCustomAttributes = "";
        ...
        // todo_list_id
        $this->todo_list_id->LinkCustomAttributes = "";
        $this->todo_list_id->HrefValue = "";
        $this->todo_list_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // todo_list_id
        $this->todo_list_id->setupEditAttributes();
        $this->todo_list_id->EditCustomAttributes = "";
        $this->todo_list_id->EditValue = $this->todo_list_id->CurrentValue;
        $this->todo_list_id->ViewCustomAttributes = "";
