 ...
 * Page class
 */
class SaveSearchList extends SaveSearch
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SaveSearchList";
    ...
    public $FormName = "fsave_searchlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "savesearchdelete";
        $this->MultiUpdateUrl = "savesearchupdate";
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fsave_searchlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fsave_searchlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->save_search_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("min_down", $this->createColumnOption("min_down"));
            $option->add("max_down", $this->createColumnOption("max_down"));
            $option->add("min_price", $this->createColumnOption("min_price"));
            $option->add("max_price", $this->createColumnOption("max_price"));
            $option->add("usable_area_min", $this->createColumnOption("usable_area_min"));
            $option->add("usable_area_max", $this->createColumnOption("usable_area_max"));
            $option->add("land_size_area_min", $this->createColumnOption("land_size_area_min"));
            $option->add("land_size_area_max", $this->createColumnOption("land_size_area_max"));
            $option->add("bedroom", $this->createColumnOption("bedroom"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsave_searchsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsave_searchsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsave_searchlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        ...
        $row['min_size'] = null;
        $row['max_size'] = null;
        $row['usable_area_min'] = null;
        $row['usable_area_max'] = null;
        $row['land_size_area_min'] = null;
        $row['land_size_area_max'] = null;
        $row['yer_installment_max'] = null;
        $row['bedroom'] = null;
        $row['latitude'] = null;
        $row['longitude'] = null;
        $row['group_type'] = null;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fsave_searchlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
