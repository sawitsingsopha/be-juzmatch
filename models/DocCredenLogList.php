 ...
 * Page class
 */
class DocCredenLogList extends DocCredenLog
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'doc_creden_log';

    // Page object name
    public $PageObjName = "DocCredenLogList";
    ...
    public $FormName = "fdoc_creden_loglist";
    ...
    public function __construct()
    {
        ...
        // Table object (doc_creden_log)
        if (!isset($GLOBALS["doc_creden_log"]) || get_class($GLOBALS["doc_creden_log"]) == PROJECT_NAMESPACE . "doc_creden_log") {
            $GLOBALS["doc_creden_log"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "doccredenlogadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "doccredenlogdelete";
        $this->MultiUpdateUrl = "doccredenlogupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'doc_creden_log');
        }
    ...
    public function run()
    {
        ...
        $this->pdfbase64->Visible = false;
        $this->request_header->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fdoc_creden_logsrch");
        }
        ...
        $filterList = Concat($filterList, $this->pdfbase64->AdvancedSearch->toJson(), ","); // Field pdfbase64
        $filterList = Concat($filterList, $this->request_header->AdvancedSearch->toJson(), ","); // Field request_header
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fdoc_creden_logsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field pdfbase64
        $this->pdfbase64->AdvancedSearch->SearchValue = @$filter["x_pdfbase64"];
        $this->pdfbase64->AdvancedSearch->SearchOperator = @$filter["z_pdfbase64"];
        $this->pdfbase64->AdvancedSearch->SearchCondition = @$filter["v_pdfbase64"];
        $this->pdfbase64->AdvancedSearch->SearchValue2 = @$filter["y_pdfbase64"];
        $this->pdfbase64->AdvancedSearch->SearchOperator2 = @$filter["w_pdfbase64"];
        $this->pdfbase64->AdvancedSearch->save();

        // Field request_header
        $this->request_header->AdvancedSearch->SearchValue = @$filter["x_request_header"];
        $this->request_header->AdvancedSearch->SearchOperator = @$filter["z_request_header"];
        $this->request_header->AdvancedSearch->SearchCondition = @$filter["v_request_header"];
        $this->request_header->AdvancedSearch->SearchValue2 = @$filter["y_request_header"];
        $this->request_header->AdvancedSearch->SearchOperator2 = @$filter["w_request_header"];
        $this->request_header->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->url;
        $searchFlds[] = &$this->pdfbase64;
        $searchFlds[] = &$this->request_header;
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->cdate->setSort("");
                $this->url->setSort("");
                $this->pdfbase64->setSort("");
                $this->request_header->setSort("");
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdoc_creden_loglist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdoc_creden_loglist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdoc_creden_logsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdoc_creden_logsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdoc_creden_loglist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->pdfbase64->setDbValue($row['pdfbase64']);
        $this->request_header->setDbValue($row['request_header']);
    }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fdoc_creden_loglist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdoc_creden_logsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
