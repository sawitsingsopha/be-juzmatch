...
/**
 * Table class for asset_appointment
 */
class AssetAppointment extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'asset_appointment';
        $this->TableName = 'asset_appointment';
        $this->TableType = 'LINKTABLE';

        // Update Table
        $this->UpdateTable = "`appointment`";
        $this->Dbid = 'juzmatch1';
        $this->ExportAll = true;
        ...
        $this->date = new DbField(
            ...
            'date',
            '`date`',
            CastDateFieldForLike("`date`", 7, "juzmatch1"),
            133,
            10,
            7,
            false,
        ...
        $this->time->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->time->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->time->Lookup = new Lookup('time', 'asset_appointment', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        $this->time->OptionCount = 10;
        ...
        $this->status->Lookup = new Lookup('status', 'asset_appointment', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`appointment`";
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            ...
            $val = $this->appointment_id->OldValue !== null ? $this->appointment_id->OldValue : $this->appointment_id->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("assetappointmentlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "assetappointmentview") {
            return $Language->phrase("View");
        } elseif ($pageName == "assetappointmentedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "assetappointmentadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "AssetAppointmentView";
            case Config("API_ADD_ACTION"):
                return "AssetAppointmentAdd";
            case Config("API_EDIT_ACTION"):
                return "AssetAppointmentEdit";
            case Config("API_DELETE_ACTION"):
                return "AssetAppointmentDelete";
            case Config("API_LIST_ACTION"):
                return "AssetAppointmentList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("assetappointmentview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("assetappointmentview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "assetappointmentadd?" . $this->getUrlParm($parm);
        } else {
            $url = "assetappointmentadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetappointmentedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetappointmentadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("assetappointmentdelete", $this->getUrlParm());
    }
    ...
    public function renderListRow()
    {
        ...
        // appointment_id
        $this->appointment_id->LinkCustomAttributes = "";
    ...
    public function renderEditRow()
    {
        ...
        $this->appointment_id->EditCustomAttributes = "";
    ...
    public function rowUpdating($rsold, &$rsnew)
    {
        ...
        // To cancel, set return value to false
            ...
            if ($rsnew['status'] == 1) {
                // print_r($rsold);
                $member_id = (int) $rsold['member_id'];
                $asset_id = (int) $rsold['asset_id'];
                $req_data = array( 
                    "member_id" => $member_id, 
                    "asset_id" => $asset_id
                ); 
                $reqJSON=json_encode($req_data);//returns JSON string

                // print($reqJSON);
                $curl = curl_init();
                curl_setopt_array($curl, array(
                CURLOPT_URL => API_URL.'buyer/comfirmAppointment',
