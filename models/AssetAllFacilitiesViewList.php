 ...
 * Page class
 */
class AssetAllFacilitiesViewList extends AssetAllFacilitiesView
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'asset_all_facilities_view';

    // Page object name
    public $PageObjName = "AssetAllFacilitiesViewList";
    ...
    public $FormName = "fasset_all_facilities_viewlist";
    ...
    public function __construct()
    {
        ...
        // Table object (asset_all_facilities_view)
        if (!isset($GLOBALS["asset_all_facilities_view"]) || get_class($GLOBALS["asset_all_facilities_view"]) == PROJECT_NAMESPACE . "asset_all_facilities_view") {
            $GLOBALS["asset_all_facilities_view"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "assetallfacilitiesviewadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "assetallfacilitiesviewdelete";
        $this->MultiUpdateUrl = "assetallfacilitiesviewupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'asset_all_facilities_view');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("asset_all_facilities_view");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['master_facilities_group_id'] . Config("COMPOSITE_KEY_SEPARATOR");
            $key .= @$ar['master_facilities_id'];
        }
        return $key;
    }
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->master_facilities_group_id->Visible = false;
        }
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->master_facilities_id->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->master_facilities_group_id->Visible = false;
        $this->group_title->setVisibility();
        $this->group_title_en->setVisibility();
        $this->master_facilities_id->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fasset_all_facilities_viewsrch");
        }
        $filterList = Concat($filterList, $this->group_title->AdvancedSearch->toJson(), ","); // Field group_title
        $filterList = Concat($filterList, $this->group_title_en->AdvancedSearch->toJson(), ","); // Field group_title_en
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fasset_all_facilities_viewsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field group_title
        $this->group_title->AdvancedSearch->SearchValue = @$filter["x_group_title"];
        $this->group_title->AdvancedSearch->SearchOperator = @$filter["z_group_title"];
        $this->group_title->AdvancedSearch->SearchCondition = @$filter["v_group_title"];
        $this->group_title->AdvancedSearch->SearchValue2 = @$filter["y_group_title"];
        $this->group_title->AdvancedSearch->SearchOperator2 = @$filter["w_group_title"];
        $this->group_title->AdvancedSearch->save();

        // Field group_title_en
        $this->group_title_en->AdvancedSearch->SearchValue = @$filter["x_group_title_en"];
        $this->group_title_en->AdvancedSearch->SearchOperator = @$filter["z_group_title_en"];
        $this->group_title_en->AdvancedSearch->SearchCondition = @$filter["v_group_title_en"];
        $this->group_title_en->AdvancedSearch->SearchValue2 = @$filter["y_group_title_en"];
        $this->group_title_en->AdvancedSearch->SearchOperator2 = @$filter["w_group_title_en"];
        $this->group_title_en->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->group_title, $default, false); // group_title
        $this->buildSearchSql($where, $this->group_title_en, $default, false); // group_title_en
        ...
        if (!$default && $this->Command == "search") {
            $this->group_title->AdvancedSearch->save(); // group_title
            $this->group_title_en->AdvancedSearch->save(); // group_title_en
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->group_title->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->group_title_en->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->group_title->AdvancedSearch->unsetSession();
        $this->group_title_en->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->group_title->AdvancedSearch->load();
        $this->group_title_en->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->group_title, $ctrl); // group_title
            $this->updateSort($this->group_title_en, $ctrl); // group_title_en
            $this->updateSort($this->_title, $ctrl); // title
            $this->updateSort($this->title_en, $ctrl); // title_en
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`group_title` ASC,`title` ASC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->group_title->getSort() != "") {
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->group_title->setSort("");
                $this->group_title_en->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_all_facilities_viewlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_all_facilities_viewlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->master_facilities_group_id->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->master_facilities_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("group_title", $this->createColumnOption("group_title"));
            $option->add("group_title_en", $this->createColumnOption("group_title_en"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fasset_all_facilities_viewsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fasset_all_facilities_viewsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fasset_all_facilities_viewlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // group_title
        if ($this->group_title->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->group_title->AdvancedSearch->SearchValue != "" || $this->group_title->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // group_title_en
        if ($this->group_title_en->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->group_title_en->AdvancedSearch->SearchValue != "" || $this->group_title_en->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->group_title->Raw) {
                $this->group_title->AdvancedSearch->SearchValue = HtmlDecode($this->group_title->AdvancedSearch->SearchValue);
            }
            $this->group_title->EditValue = HtmlEncode($this->group_title->AdvancedSearch->SearchValue);
            ...
            if (!$this->group_title_en->Raw) {
                $this->group_title_en->AdvancedSearch->SearchValue = HtmlDecode($this->group_title_en->AdvancedSearch->SearchValue);
            }
            $this->group_title_en->EditValue = HtmlEncode($this->group_title_en->AdvancedSearch->SearchValue);
            ...
            if (!$this->title_en->Raw) {
                $this->title_en->AdvancedSearch->SearchValue = HtmlDecode($this->title_en->AdvancedSearch->SearchValue);
            }
            $this->title_en->EditValue = HtmlEncode($this->title_en->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fasset_all_facilities_viewlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fasset_all_facilities_viewsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
