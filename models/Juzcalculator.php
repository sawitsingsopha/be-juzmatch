...
/**
 * Table class for juzcalculator
 */
class Juzcalculator extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'juzcalculator';
        $this->TableName = 'juzcalculator';
        ...
        $this->status->Lookup = new Lookup('status', 'juzcalculator', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->type_invertor->Lookup = new Lookup('type_invertor', 'juzcalculator', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "juzcalculator_income") {
            $detailUrl = Container("juzcalculator_income")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_juzcalculator_id", $this->juzcalculator_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "juzcalculator_outcome") {
            $detailUrl = Container("juzcalculator_outcome")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_juzcalculator_id", $this->juzcalculator_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "juzcalculatorlist";
        }
        return $detailUrl;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("juzcalculatorlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "juzcalculatoredit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "juzcalculatoradd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "JuzcalculatorView";
            case Config("API_ADD_ACTION"):
                return "JuzcalculatorAdd";
            case Config("API_EDIT_ACTION"):
                return "JuzcalculatorEdit";
            case Config("API_DELETE_ACTION"):
                return "JuzcalculatorDelete";
            case Config("API_LIST_ACTION"):
                return "JuzcalculatorList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("juzcalculatorview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("juzcalculatorview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "juzcalculatoradd?" . $this->getUrlParm($parm);
        } else {
            $url = "juzcalculatoradd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("juzcalculatoredit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("juzcalculatoredit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("juzcalculatoradd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("juzcalculatoradd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("juzcalculatordelete", $this->getUrlParm());
    }
    ...
    public function renderListRow()
    {
        ...
        $this->transfer->ViewValue = FormatNumber($this->transfer->ViewValue, $this->transfer->formatPattern());
    ...
    public function renderEditRow()
    {
        ...
        // transfer
        $this->transfer->setupEditAttributes();
        $this->transfer->EditCustomAttributes = "";
        $this->transfer->EditValue = $this->transfer->CurrentValue;
        $this->transfer->PlaceHolder = RemoveHtml($this->transfer->caption());
        if (strval($this->transfer->EditValue) != "" && is_numeric($this->transfer->EditValue)) {
            $this->transfer->EditValue = FormatNumber($this->transfer->EditValue, null);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->income_all);
                    $doc->exportCaption($this->outcome_all);
                    $doc->exportCaption($this->rent_price);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->income_all);
                        $doc->exportField($this->outcome_all);
                        $doc->exportField($this->rent_price);
