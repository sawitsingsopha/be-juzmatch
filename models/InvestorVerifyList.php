 ...
 * Page class
 */
class InvestorVerifyList extends InvestorVerify
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InvestorVerifyList";
    ...
    public $FormName = "finvestor_verifylist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "investorverifydelete";
        $this->MultiUpdateUrl = "investorverifyupdate";
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finvestor_verifylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finvestor_verifylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("investment", $this->createColumnOption("investment"));
            $option->add("credit_limit", $this->createColumnOption("credit_limit"));
            $option->add("monthly_payments", $this->createColumnOption("monthly_payments"));
            $option->add("highest_rental_price", $this->createColumnOption("highest_rental_price"));
            $option->add("transfer", $this->createColumnOption("transfer"));
            $option->add("total_invertor_year", $this->createColumnOption("total_invertor_year"));
            $option->add("invert_payoff_day", $this->createColumnOption("invert_payoff_day"));
            $option->add("type_invertor", $this->createColumnOption("type_invertor"));
            $option->add("invest_amount", $this->createColumnOption("invest_amount"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"finvestor_verifysrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"finvestor_verifysrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="finvestor_verifylist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="finvestor_verifylist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
