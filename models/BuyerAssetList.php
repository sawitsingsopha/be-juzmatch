 ...
 * Page class
 */
class BuyerAssetList extends BuyerAsset
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'buyer_asset';

    // Page object name
    public $PageObjName = "BuyerAssetList";
    ...
    public $FormName = "fbuyer_assetlist";
    ...
    public function __construct()
    {
        ...
        // Table object (buyer_asset)
        if (!isset($GLOBALS["buyer_asset"]) || get_class($GLOBALS["buyer_asset"]) == PROJECT_NAMESPACE . "buyer_asset") {
            $GLOBALS["buyer_asset"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "buyerassetadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "buyerassetdelete";
        $this->MultiUpdateUrl = "buyerassetupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'buyer_asset');
        }
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_buyer_asset_rent"
        $item = &$this->ListOptions->add("detail_buyer_asset_rent");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'buyer_asset_rent');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_todo_list"
        $item = &$this->ListOptions->add("detail_todo_list");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'todo_list');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_plan_loan"
        $item = &$this->ListOptions->add("detail_plan_loan");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'plan_loan');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_buyer_asset_ready_buy"
        $item = &$this->ListOptions->add("detail_buyer_asset_ready_buy");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'buyer_asset_ready_buy');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_buyer_asset_schedule"
        $item = &$this->ListOptions->add("detail_buyer_asset_schedule");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'buyer_asset_schedule');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_buyer_config_asset_schedule"
        $item = &$this->ListOptions->add("detail_buyer_config_asset_schedule");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'buyer_config_asset_schedule');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("buyer_asset_rent");
        $pages->add("todo_list");
        $pages->add("plan_loan");
        $pages->add("buyer_asset_ready_buy");
        $pages->add("buyer_asset_schedule");
        $pages->add("buyer_config_asset_schedule");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuyer_assetlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuyer_assetlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_buyer_asset_rent"
        $opt = $this->ListOptions["detail_buyer_asset_rent"];
        if ($Security->allowList(CurrentProjectID() . 'buyer_asset_rent')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("buyer_asset_rent", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("buyerassetrentlist?" . Config("TABLE_SHOW_MASTER") . "=buyer_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("BuyerAssetRentGrid");
            if ($links != "") {
        ...
        // "detail_todo_list"
        $opt = $this->ListOptions["detail_todo_list"];
        if ($Security->allowList(CurrentProjectID() . 'todo_list')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("todo_list", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("todolistlist?" . Config("TABLE_SHOW_MASTER") . "=buyer_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("TodoListGrid");
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
            }
        ...
        // "detail_plan_loan"
        $opt = $this->ListOptions["detail_plan_loan"];
        if ($Security->allowList(CurrentProjectID() . 'plan_loan')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("plan_loan", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("planloanlist?" . Config("TABLE_SHOW_MASTER") . "=buyer_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("PlanLoanGrid");
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
            }
        ...
        // "detail_buyer_asset_ready_buy"
        $opt = $this->ListOptions["detail_buyer_asset_ready_buy"];
        if ($Security->allowList(CurrentProjectID() . 'buyer_asset_ready_buy')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("buyer_asset_ready_buy", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("buyerassetreadybuylist?" . Config("TABLE_SHOW_MASTER") . "=buyer_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("BuyerAssetReadyBuyGrid");
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
            }
        ...
        // "detail_buyer_asset_schedule"
        $opt = $this->ListOptions["detail_buyer_asset_schedule"];
        if ($Security->allowList(CurrentProjectID() . 'buyer_asset_schedule')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("buyer_asset_schedule", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("buyerassetschedulelist?" . Config("TABLE_SHOW_MASTER") . "=buyer_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        // "detail_buyer_config_asset_schedule"
        $opt = $this->ListOptions["detail_buyer_config_asset_schedule"];
        if ($Security->allowList(CurrentProjectID() . 'buyer_config_asset_schedule')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("buyer_config_asset_schedule", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("buyerconfigassetschedulelist?" . Config("TABLE_SHOW_MASTER") . "=buyer_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("BuyerConfigAssetScheduleGrid");
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbuyer_assetsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbuyer_assetsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbuyer_assetlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fbuyer_assetlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
