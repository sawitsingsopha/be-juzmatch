...
class AssetEdit extends Asset
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("asset_facilities", $detailTblVar) && $detailPage->DetailEdit) {
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("AssetProsDetailGrid");
        if (in_array("asset_pros_detail", $detailTblVar) && $detailPage->DetailEdit) {
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("AssetBannerGrid");
        if (in_array("asset_banner", $detailTblVar) && $detailPage->DetailEdit) {
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        ...
        if (in_array("asset_warning", $detailTblVar) && $detailPage->DetailEdit) {
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // num_buildings
            $this->num_buildings->setDbValueDef($rsnew, $this->num_buildings->CurrentValue, null, $this->num_buildings->ReadOnly);

            // num_unit
            $this->num_unit->setDbValueDef($rsnew, $this->num_unit->CurrentValue, null, $this->num_unit->ReadOnly);

            // num_floors
            $this->num_floors->setDbValueDef($rsnew, $this->num_floors->CurrentValue, null, $this->num_floors->ReadOnly);

            // floors
            $this->floors->setDbValueDef($rsnew, $this->floors->CurrentValue, null, $this->floors->ReadOnly);

            // asset_year_developer
            $this->asset_year_developer->setDbValueDef($rsnew, $this->asset_year_developer->CurrentValue, null, $this->asset_year_developer->ReadOnly);

            // num_parking_spaces
            $this->num_parking_spaces->setDbValueDef($rsnew, $this->num_parking_spaces->CurrentValue, null, $this->num_parking_spaces->ReadOnly);

            // num_bathrooms
            $this->num_bathrooms->setDbValueDef($rsnew, $this->num_bathrooms->CurrentValue, null, $this->num_bathrooms->ReadOnly);

            // num_bedrooms
            $this->num_bedrooms->setDbValueDef($rsnew, $this->num_bedrooms->CurrentValue, null, $this->num_bedrooms->ReadOnly);

            // address
            $this->address->setDbValueDef($rsnew, $this->address->CurrentValue, null, $this->address->ReadOnly);

            // address_en
            $this->address_en->setDbValueDef($rsnew, $this->address_en->CurrentValue, null, $this->address_en->ReadOnly);

            // province_id
            $this->province_id->setDbValueDef($rsnew, $this->province_id->CurrentValue, null, $this->province_id->ReadOnly);

            // amphur_id
            $this->amphur_id->setDbValueDef($rsnew, $this->amphur_id->CurrentValue, null, $this->amphur_id->ReadOnly);

            // district_id
            $this->district_id->setDbValueDef($rsnew, $this->district_id->CurrentValue, null, $this->district_id->ReadOnly);

            // postcode
            $this->postcode->setDbValueDef($rsnew, $this->postcode->CurrentValue, null, $this->postcode->ReadOnly);

            // floor_plan
            if ($this->floor_plan->Visible && !$this->floor_plan->ReadOnly && !$this->floor_plan->Upload->KeepFile) {
                $this->floor_plan->Upload->DbValue = $rsold['floor_plan']; // Get original value
            ...
            // layout_unit
            if ($this->layout_unit->Visible && !$this->layout_unit->ReadOnly && !$this->layout_unit->Upload->KeepFile) {
                $this->layout_unit->Upload->DbValue = $rsold['layout_unit']; // Get original value
            ...
            // asset_website
            $this->asset_website->setDbValueDef($rsnew, $this->asset_website->CurrentValue, null, $this->asset_website->ReadOnly);

            // asset_review
            $this->asset_review->setDbValueDef($rsnew, $this->asset_review->CurrentValue, null, $this->asset_review->ReadOnly);

            // isactive
            $this->isactive->setDbValueDef($rsnew, $this->isactive->CurrentValue, null, $this->isactive->ReadOnly);

            // is_recommend
            $this->is_recommend->setDbValueDef($rsnew, $this->is_recommend->CurrentValue, 0, $this->is_recommend->ReadOnly);

            // order_by
            $this->order_by->setDbValueDef($rsnew, $this->order_by->CurrentValue, null, $this->order_by->ReadOnly);

            // type_pay
            $this->type_pay->setDbValueDef($rsnew, $this->type_pay->CurrentValue, null, $this->type_pay->ReadOnly);

            // type_pay_2
            $this->type_pay_2->setDbValueDef($rsnew, $this->type_pay_2->CurrentValue, null, $this->type_pay_2->ReadOnly);

            // price_mark
            $this->price_mark->setDbValueDef($rsnew, $this->price_mark->CurrentValue, null, $this->price_mark->ReadOnly);

            // holding_property
            $this->holding_property->setDbValueDef($rsnew, $this->holding_property->CurrentValue, null, $this->holding_property->ReadOnly);

            // common_fee
            $this->common_fee->setDbValueDef($rsnew, $this->common_fee->CurrentValue, null, $this->common_fee->ReadOnly);

            // usable_area
            $this->usable_area->setDbValueDef($rsnew, $this->usable_area->CurrentValue, null, $this->usable_area->ReadOnly);

            // usable_area_price
            $this->usable_area_price->setDbValueDef($rsnew, $this->usable_area_price->CurrentValue, null, $this->usable_area_price->ReadOnly);

            // land_size
            $this->land_size->setDbValueDef($rsnew, $this->land_size->CurrentValue, null, $this->land_size->ReadOnly);

            // land_size_price
            $this->land_size_price->setDbValueDef($rsnew, $this->land_size_price->CurrentValue, null, $this->land_size_price->ReadOnly);

            // commission
            $this->commission->setDbValueDef($rsnew, $this->commission->CurrentValue, null, $this->commission->ReadOnly);

            // transfer_day_expenses_with_business_tax
            $this->transfer_day_expenses_with_business_tax->setDbValueDef($rsnew, $this->transfer_day_expenses_with_business_tax->CurrentValue, null, $this->transfer_day_expenses_with_business_tax->ReadOnly);

            // transfer_day_expenses_without_business_tax
            $this->transfer_day_expenses_without_business_tax->setDbValueDef($rsnew, $this->transfer_day_expenses_without_business_tax->CurrentValue, null, $this->transfer_day_expenses_without_business_tax->ReadOnly);
            ...
            // price_special
            $this->price_special->setDbValueDef($rsnew, $this->price_special->CurrentValue, null, $this->price_special->ReadOnly);

            // reservation_price_model_a
            $this->reservation_price_model_a->setDbValueDef($rsnew, $this->reservation_price_model_a->CurrentValue, null, $this->reservation_price_model_a->ReadOnly);

            // minimum_down_payment_model_a
            $this->minimum_down_payment_model_a->setDbValueDef($rsnew, $this->minimum_down_payment_model_a->CurrentValue, null, $this->minimum_down_payment_model_a->ReadOnly);

            // down_price_min_a
            $this->down_price_min_a->setDbValueDef($rsnew, $this->down_price_min_a->CurrentValue, null, $this->down_price_min_a->ReadOnly);

            // down_price_model_a
            $this->down_price_model_a->setDbValueDef($rsnew, $this->down_price_model_a->CurrentValue, null, $this->down_price_model_a->ReadOnly);

            // factor_monthly_installment_over_down
            $this->factor_monthly_installment_over_down->setDbValueDef($rsnew, $this->factor_monthly_installment_over_down->CurrentValue, null, $this->factor_monthly_installment_over_down->ReadOnly);

            // fee_a
            $this->fee_a->setDbValueDef($rsnew, $this->fee_a->CurrentValue, null, $this->fee_a->ReadOnly);

            // monthly_payment_buyer
            $this->monthly_payment_buyer->setDbValueDef($rsnew, $this->monthly_payment_buyer->CurrentValue, null, $this->monthly_payment_buyer->ReadOnly);

            // annual_interest_buyer_model_a
            $this->annual_interest_buyer_model_a->setDbValueDef($rsnew, $this->annual_interest_buyer_model_a->CurrentValue, null, $this->annual_interest_buyer_model_a->ReadOnly);

            // monthly_expenses_a
            $this->monthly_expenses_a->setDbValueDef($rsnew, $this->monthly_expenses_a->CurrentValue, null, $this->monthly_expenses_a->ReadOnly);

            // average_rent_a
            $this->average_rent_a->setDbValueDef($rsnew, $this->average_rent_a->CurrentValue, null, $this->average_rent_a->ReadOnly);

            // average_down_payment_a
            $this->average_down_payment_a->setDbValueDef($rsnew, $this->average_down_payment_a->CurrentValue, null, $this->average_down_payment_a->ReadOnly);

            // transfer_day_expenses_without_business_tax_max_min
            $this->transfer_day_expenses_without_business_tax_max_min->setDbValueDef($rsnew, $this->transfer_day_expenses_without_business_tax_max_min->CurrentValue, null, $this->transfer_day_expenses_without_business_tax_max_min->ReadOnly);

            // transfer_day_expenses_with_business_tax_max_min
            $this->transfer_day_expenses_with_business_tax_max_min->setDbValueDef($rsnew, $this->transfer_day_expenses_with_business_tax_max_min->CurrentValue, null, $this->transfer_day_expenses_with_business_tax_max_min->ReadOnly);

            // bank_appraisal_price
            $this->bank_appraisal_price->setDbValueDef($rsnew, $this->bank_appraisal_price->CurrentValue, null, $this->bank_appraisal_price->ReadOnly);

            // mark_up_price
            $this->mark_up_price->setDbValueDef($rsnew, $this->mark_up_price->CurrentValue, null, $this->mark_up_price->ReadOnly);

            // required_gap
            $this->required_gap->setDbValueDef($rsnew, $this->required_gap->CurrentValue, null, $this->required_gap->ReadOnly);

            // minimum_down_payment
            $this->minimum_down_payment->setDbValueDef($rsnew, $this->minimum_down_payment->CurrentValue, null, $this->minimum_down_payment->ReadOnly);

            // price_down_max
            $this->price_down_max->setDbValueDef($rsnew, $this->price_down_max->CurrentValue, null, $this->price_down_max->ReadOnly);

            // discount_max
            $this->discount_max->setDbValueDef($rsnew, $this->discount_max->CurrentValue, null, $this->discount_max->ReadOnly);

            // price_down_special_max
            $this->price_down_special_max->setDbValueDef($rsnew, $this->price_down_special_max->CurrentValue, null, $this->price_down_special_max->ReadOnly);

            // usable_area_price_max
            $this->usable_area_price_max->setDbValueDef($rsnew, $this->usable_area_price_max->CurrentValue, null, $this->usable_area_price_max->ReadOnly);

            // land_size_price_max
            $this->land_size_price_max->setDbValueDef($rsnew, $this->land_size_price_max->CurrentValue, null, $this->land_size_price_max->ReadOnly);

            // reservation_price_max
            $this->reservation_price_max->setDbValueDef($rsnew, $this->reservation_price_max->CurrentValue, null, $this->reservation_price_max->ReadOnly);

            // minimum_down_payment_max
            $this->minimum_down_payment_max->setDbValueDef($rsnew, $this->minimum_down_payment_max->CurrentValue, null, $this->minimum_down_payment_max->ReadOnly);

            // down_price_max
            $this->down_price_max->setDbValueDef($rsnew, $this->down_price_max->CurrentValue, null, $this->down_price_max->ReadOnly);

            // down_price
            $this->down_price->setDbValueDef($rsnew, $this->down_price->CurrentValue, null, $this->down_price->ReadOnly);

            // factor_monthly_installment_over_down_max
            $this->factor_monthly_installment_over_down_max->setDbValueDef($rsnew, $this->factor_monthly_installment_over_down_max->CurrentValue, null, $this->factor_monthly_installment_over_down_max->ReadOnly);

            // fee_max
            $this->fee_max->setDbValueDef($rsnew, $this->fee_max->CurrentValue, null, $this->fee_max->ReadOnly);

            // monthly_payment_max
            $this->monthly_payment_max->setDbValueDef($rsnew, $this->monthly_payment_max->CurrentValue, null, $this->monthly_payment_max->ReadOnly);

            // annual_interest_buyer
            $this->annual_interest_buyer->setDbValueDef($rsnew, $this->annual_interest_buyer->CurrentValue, null, $this->annual_interest_buyer->ReadOnly);

            // monthly_expense_max
            $this->monthly_expense_max->setDbValueDef($rsnew, $this->monthly_expense_max->CurrentValue, null, $this->monthly_expense_max->ReadOnly);

            // average_rent_max
            $this->average_rent_max->setDbValueDef($rsnew, $this->average_rent_max->CurrentValue, null, $this->average_rent_max->ReadOnly);

            // average_down_payment_max
            $this->average_down_payment_max->setDbValueDef($rsnew, $this->average_down_payment_max->CurrentValue, null, $this->average_down_payment_max->ReadOnly);

            // min_down
            $this->min_down->setDbValueDef($rsnew, $this->min_down->CurrentValue, null, $this->min_down->ReadOnly);

            // remaining_down
            $this->remaining_down->setDbValueDef($rsnew, $this->remaining_down->CurrentValue, null, $this->remaining_down->ReadOnly);

            // factor_financing
            $this->factor_financing->setDbValueDef($rsnew, $this->factor_financing->CurrentValue, null, $this->factor_financing->ReadOnly);

            // credit_limit_down
            $this->credit_limit_down->setDbValueDef($rsnew, $this->credit_limit_down->CurrentValue, null, $this->credit_limit_down->ReadOnly);

            // price_down_min
            $this->price_down_min->setDbValueDef($rsnew, $this->price_down_min->CurrentValue, null, $this->price_down_min->ReadOnly);

            // discount_min
            $this->discount_min->setDbValueDef($rsnew, $this->discount_min->CurrentValue, null, $this->discount_min->ReadOnly);

            // price_down_special_min
            $this->price_down_special_min->setDbValueDef($rsnew, $this->price_down_special_min->CurrentValue, null, $this->price_down_special_min->ReadOnly);

            // usable_area_price_min
            $this->usable_area_price_min->setDbValueDef($rsnew, $this->usable_area_price_min->CurrentValue, null, $this->usable_area_price_min->ReadOnly);

            // land_size_price_min
            $this->land_size_price_min->setDbValueDef($rsnew, $this->land_size_price_min->CurrentValue, null, $this->land_size_price_min->ReadOnly);

            // reservation_price_min
            $this->reservation_price_min->setDbValueDef($rsnew, $this->reservation_price_min->CurrentValue, null, $this->reservation_price_min->ReadOnly);

            // minimum_down_payment_min
            $this->minimum_down_payment_min->setDbValueDef($rsnew, $this->minimum_down_payment_min->CurrentValue, null, $this->minimum_down_payment_min->ReadOnly);

            // down_price_min
            $this->down_price_min->setDbValueDef($rsnew, $this->down_price_min->CurrentValue, null, $this->down_price_min->ReadOnly);

            // remaining_credit_limit_down
            $this->remaining_credit_limit_down->setDbValueDef($rsnew, $this->remaining_credit_limit_down->CurrentValue, null, $this->remaining_credit_limit_down->ReadOnly);

            // fee_min
            $this->fee_min->setDbValueDef($rsnew, $this->fee_min->CurrentValue, null, $this->fee_min->ReadOnly);

            // monthly_payment_min
            $this->monthly_payment_min->setDbValueDef($rsnew, $this->monthly_payment_min->CurrentValue, null, $this->monthly_payment_min->ReadOnly);

            // annual_interest_buyer_model_min
            $this->annual_interest_buyer_model_min->setDbValueDef($rsnew, $this->annual_interest_buyer_model_min->CurrentValue, null, $this->annual_interest_buyer_model_min->ReadOnly);

            // interest_down-payment_financing
            $this->interest_downpayment_financing->setDbValueDef($rsnew, $this->interest_downpayment_financing->CurrentValue, null, $this->interest_downpayment_financing->ReadOnly);

            // monthly_expenses_min
            $this->monthly_expenses_min->setDbValueDef($rsnew, $this->monthly_expenses_min->CurrentValue, null, $this->monthly_expenses_min->ReadOnly);

            // average_rent_min
            $this->average_rent_min->setDbValueDef($rsnew, $this->average_rent_min->CurrentValue, null, $this->average_rent_min->ReadOnly);

            // average_down_payment_min
            $this->average_down_payment_min->setDbValueDef($rsnew, $this->average_down_payment_min->CurrentValue, null, $this->average_down_payment_min->ReadOnly);

            // installment_down_payment_loan
            $this->installment_down_payment_loan->setDbValueDef($rsnew, $this->installment_down_payment_loan->CurrentValue, null, $this->installment_down_payment_loan->ReadOnly);

            // price_invertor
            $this->price_invertor->setDbValueDef($rsnew, $this->price_invertor->CurrentValue, null, $this->price_invertor->ReadOnly);
            ...
            if ($this->floor_plan->Visible && !$this->floor_plan->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->floor_plan->Upload->FileName)) {
                    ...
                    $this->floor_plan->setDbValueDef($rsnew, $this->floor_plan->Upload->FileName, null, $this->floor_plan->ReadOnly);
                }
            }
            if ($this->layout_unit->Visible && !$this->layout_unit->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->layout_unit->Upload->FileName)) {
                    ...
                    $this->layout_unit->setDbValueDef($rsnew, $this->layout_unit->Upload->FileName, null, $this->layout_unit->ReadOnly);
                }
            }
