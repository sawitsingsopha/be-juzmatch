 ...
 * Page class
 */
class PeakExpenseEdit extends PeakExpense
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PeakExpenseEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("peak_expense_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->peak_expense_id->setQueryStringValue($keyValue);
                $this->peak_expense_id->setOldValue($this->peak_expense_id->QueryStringValue);
            } elseif (Post("peak_expense_id") !== null) {
                $this->peak_expense_id->setFormValue(Post("peak_expense_id"));
                $this->peak_expense_id->setOldValue($this->peak_expense_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->peak_expense_id->IsDetailKey) {
            $this->peak_expense_id->setFormValue($val);
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // contactId
            $this->contactId->setDbValueDef($rsnew, $this->contactId->CurrentValue, null, $this->contactId->ReadOnly);
            ...
            // paymentGroupId
            $this->paymentGroupId->setDbValueDef($rsnew, $this->paymentGroupId->CurrentValue, null, $this->paymentGroupId->ReadOnly);

            // paymentTotal
            $this->paymentTotal->setDbValueDef($rsnew, $this->paymentTotal->CurrentValue, null, $this->paymentTotal->ReadOnly);
            ...
            // sync_detail_date
            $this->sync_detail_date->setDbValueDef($rsnew, UnFormatDateTime($this->sync_detail_date->CurrentValue, $this->sync_detail_date->formatPattern()), null, $this->sync_detail_date->ReadOnly);
