 ...
 * Page class
 */
class AllAssetScheduleView extends AllAssetSchedule
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AllAssetScheduleView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("asset_schedule_id") ?? Route("asset_schedule_id")) !== null) {
            $this->RecKey["asset_schedule_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        $this->asset_schedule_id->setVisibility();
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("asset_schedule_id") ?? Route("asset_schedule_id")) !== null) {
                ...
                $this->RecKey["asset_schedule_id"] = $this->asset_schedule_id->QueryStringValue;
            } elseif (Post("asset_schedule_id") !== null) {
                $this->asset_schedule_id->setFormValue(Post("asset_schedule_id"));
                $this->RecKey["asset_schedule_id"] = $this->asset_schedule_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->asset_schedule_id->setQueryStringValue($keyValue);
                $this->RecKey["asset_schedule_id"] = $this->asset_schedule_id->QueryStringValue;
            } else {
                $returnUrl = "allassetschedulelist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "allassetschedulelist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "allassetschedulelist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
