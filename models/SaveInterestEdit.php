 ...
 * Page class
 */
class SaveInterestEdit extends SaveInterest
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SaveInterestEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("save_interest_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->save_interest_id->setQueryStringValue($keyValue);
                $this->save_interest_id->setOldValue($this->save_interest_id->QueryStringValue);
            } elseif (Post("save_interest_id") !== null) {
                $this->save_interest_id->setFormValue(Post("save_interest_id"));
                $this->save_interest_id->setOldValue($this->save_interest_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->save_interest_id->IsDetailKey) {
            $this->save_interest_id->setFormValue($val);
        }
    }
    ...
    {
        global $CurrentForm;
        $this->save_interest_id->CurrentValue = $this->save_interest_id->FormValue;
