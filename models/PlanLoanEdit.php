 ...
 * Page class
 */
class PlanLoanEdit extends PlanLoan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PlanLoanEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("plan_loan_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->plan_loan_id->setQueryStringValue($keyValue);
                $this->plan_loan_id->setOldValue($this->plan_loan_id->QueryStringValue);
            } elseif (Post("plan_loan_id") !== null) {
                $this->plan_loan_id->setFormValue(Post("plan_loan_id"));
                $this->plan_loan_id->setOldValue($this->plan_loan_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("plan_loan_id") ?? Route("plan_loan_id")) !== null) {
                    $this->plan_loan_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->plan_loan_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("planloanlist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "planloanlist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'plan_loan_id' first before field var 'x_plan_loan_id'
        $val = $CurrentForm->hasValue("plan_loan_id") ? $CurrentForm->getValue("plan_loan_id") : $CurrentForm->getValue("x_plan_loan_id");
        if (!$this->plan_loan_id->IsDetailKey) {
            $this->plan_loan_id->setFormValue($val);
        }
    }
    ...
    {
        global $CurrentForm;
        $this->plan_loan_id->CurrentValue = $this->plan_loan_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // plan_loan_id
        $this->plan_loan_id->RowCssClass = "row";
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // time
            $this->time->setDbValueDef($rsnew, UnFormatDateTime($this->time->CurrentValue, $this->time->formatPattern()), CurrentTime(), $this->time->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("planloanlist"), "", $this->TableVar, true);
        $pageId = "edit";
