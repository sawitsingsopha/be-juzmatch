...
/**
 * Table class for sale_asset
 */
class SaleAsset extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sale_asset';
        $this->TableName = 'sale_asset';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`sale_asset`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // sale_asset_id
        $this->sale_asset_id = new DbField(
            'sale_asset',
            'sale_asset',
            'x_sale_asset_id',
            'sale_asset_id',
        ...
        $this->sale_asset_id->InputTextType = "text";
        $this->sale_asset_id->IsAutoIncrement = true; // Autoincrement field
        $this->sale_asset_id->IsPrimaryKey = true; // Primary key field
        $this->sale_asset_id->Sortable = false; // Allow sort
        $this->sale_asset_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['sale_asset_id'] = &$this->sale_asset_id;
        ...
        $this->status_live->InputTextType = "text";
        $this->status_live->Nullable = false; // NOT NULL field
        $this->status_live->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['status_live'] = &$this->status_live;
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "seller") {
            if ($this->member_id->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`member_id`", $this->member_id->getSessionValue(), DATATYPE_NUMBER, "juzmatch3");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "seller") {
            if ($this->member_id->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`member_id`", $this->member_id->getSessionValue(), DATATYPE_NUMBER, "juzmatch3");
            } else {
                return "";
            }
        }
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "asset") {
            $detailUrl = Container("asset")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "saleassetlist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`sale_asset`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->sale_asset_id->setDbValue($conn->lastInsertId());
            $rs['sale_asset_id'] = $this->sale_asset_id->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("appointment")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("asset_warning")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('sale_asset_id', $rs)) {
                AddFilter($where, QuotedName('sale_asset_id', $this->Dbid) . '=' . QuotedValue($rs['sale_asset_id'], $this->sale_asset_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("appointment")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB") . " AND " . "`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        ...
        $dtlrows = Container("asset_warning")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB") . " AND " . "`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->sale_asset_id->DbValue = $row['sale_asset_id'];
        $this->asset_id->DbValue = $row['asset_id'];
        $this->member_id->DbValue = $row['member_id'];
        $this->status_live->DbValue = $row['status_live'];
    }
    ...
    {
        return "`sale_asset_id` = @sale_asset_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->sale_asset_id->CurrentValue : $this->sale_asset_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->sale_asset_id->CurrentValue = $keys[0];
            } else {
                $this->sale_asset_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('sale_asset_id', $row) ? $row['sale_asset_id'] : null;
        } else {
            $val = $this->sale_asset_id->OldValue !== null ? $this->sale_asset_id->OldValue : $this->sale_asset_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@sale_asset_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("saleassetlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "saleassetedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "saleassetadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "SaleAssetView";
            case Config("API_ADD_ACTION"):
                return "SaleAssetAdd";
            case Config("API_EDIT_ACTION"):
                return "SaleAssetEdit";
            case Config("API_DELETE_ACTION"):
                return "SaleAssetDelete";
            case Config("API_LIST_ACTION"):
                return "SaleAssetList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("saleassetview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("saleassetview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "saleassetadd?" . $this->getUrlParm($parm);
        } else {
            $url = "saleassetadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("saleassetedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("saleassetedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("saleassetadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("saleassetadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("saleassetdelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "seller" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
    ...
    {
        $json = "";
        $json .= "\"sale_asset_id\":" . JsonEncode($this->sale_asset_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->sale_asset_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->sale_asset_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("sale_asset_id") ?? Route("sale_asset_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->sale_asset_id->CurrentValue = $key;
            } else {
                $this->sale_asset_id->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // sale_asset_id
        $this->sale_asset_id->ViewValue = $this->sale_asset_id->CurrentValue;
        $this->sale_asset_id->ViewCustomAttributes = "";
        ...
        // sale_asset_id
        $this->sale_asset_id->LinkCustomAttributes = "";
        $this->sale_asset_id->HrefValue = "";
        $this->sale_asset_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // sale_asset_id
        $this->sale_asset_id->setupEditAttributes();
        $this->sale_asset_id->EditCustomAttributes = "";
        $this->sale_asset_id->EditValue = $this->sale_asset_id->CurrentValue;
        $this->sale_asset_id->ViewCustomAttributes = "";
        ...
        $this->status_live->EditValue = $this->status_live->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->status_live);
                } else {
                    $doc->exportCaption($this->asset_id);
                    $doc->exportCaption($this->member_id);
                    $doc->exportCaption($this->status_live);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->status_live);
                    } else {
                        $doc->exportField($this->asset_id);
                        $doc->exportField($this->member_id);
                        $doc->exportField($this->status_live);
