 ...
 * Page class
 */
class AssetProsDetailList extends AssetProsDetail
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'asset_pros_detail';

    // Page object name
    public $PageObjName = "AssetProsDetailList";
    ...
    public $FormName = "fasset_pros_detaillist";
    ...
    public function __construct()
    {
        ...
        // Table object (asset_pros_detail)
        if (!isset($GLOBALS["asset_pros_detail"]) || get_class($GLOBALS["asset_pros_detail"]) == PROJECT_NAMESPACE . "asset_pros_detail") {
            $GLOBALS["asset_pros_detail"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "assetprosdetailadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "assetprosdetaildelete";
        $this->MultiUpdateUrl = "assetprosdetailupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'asset_pros_detail');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("asset_pros_detail");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->asset_pros_detail_id->Visible = false;
        }
        if ($this->isAddOrEdit()) {
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->group_type);
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "asset") {
            $masterTbl = Container("asset");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("assetlist"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fasset_pros_detailsrch");
        }
        $filterList = Concat($filterList, $this->asset_pros_detail_id->AdvancedSearch->toJson(), ","); // Field asset_pros_detail_id
        $filterList = Concat($filterList, $this->asset_pros_id->AdvancedSearch->toJson(), ","); // Field asset_pros_id
        ...
        $filterList = Concat($filterList, $this->group_type->AdvancedSearch->toJson(), ","); // Field group_type
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fasset_pros_detailsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field asset_pros_detail_id
        $this->asset_pros_detail_id->AdvancedSearch->SearchValue = @$filter["x_asset_pros_detail_id"];
        $this->asset_pros_detail_id->AdvancedSearch->SearchOperator = @$filter["z_asset_pros_detail_id"];
        $this->asset_pros_detail_id->AdvancedSearch->SearchCondition = @$filter["v_asset_pros_detail_id"];
        $this->asset_pros_detail_id->AdvancedSearch->SearchValue2 = @$filter["y_asset_pros_detail_id"];
        $this->asset_pros_detail_id->AdvancedSearch->SearchOperator2 = @$filter["w_asset_pros_detail_id"];
        $this->asset_pros_detail_id->AdvancedSearch->save();

        // Field asset_pros_id
        $this->asset_pros_id->AdvancedSearch->SearchValue = @$filter["x_asset_pros_id"];
        $this->asset_pros_id->AdvancedSearch->SearchOperator = @$filter["z_asset_pros_id"];
        $this->asset_pros_id->AdvancedSearch->SearchCondition = @$filter["v_asset_pros_id"];
        $this->asset_pros_id->AdvancedSearch->SearchValue2 = @$filter["y_asset_pros_id"];
        $this->asset_pros_id->AdvancedSearch->SearchOperator2 = @$filter["w_asset_pros_id"];
        $this->asset_pros_id->AdvancedSearch->save();
        ...
        // Field group_type
        $this->group_type->AdvancedSearch->SearchValue = @$filter["x_group_type"];
        $this->group_type->AdvancedSearch->SearchOperator = @$filter["z_group_type"];
        $this->group_type->AdvancedSearch->SearchCondition = @$filter["v_group_type"];
        $this->group_type->AdvancedSearch->SearchValue2 = @$filter["y_group_type"];
        $this->group_type->AdvancedSearch->SearchOperator2 = @$filter["w_group_type"];
        $this->group_type->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->asset_pros_detail_id, $default, false); // asset_pros_detail_id
        $this->buildSearchSql($where, $this->asset_pros_id, $default, false); // asset_pros_id
        ...
        $this->buildSearchSql($where, $this->group_type, $default, false); // group_type
        ...
        if (!$default && $this->Command == "search") {
            $this->asset_pros_detail_id->AdvancedSearch->save(); // asset_pros_detail_id
            $this->asset_pros_id->AdvancedSearch->save(); // asset_pros_id
            ...
            $this->group_type->AdvancedSearch->save(); // group_type
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->detail;
        $searchFlds[] = &$this->detail_en;
        $searchFlds[] = &$this->latitude;
        $searchFlds[] = &$this->longitude;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->asset_pros_detail_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->asset_pros_id->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->group_type->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->asset_pros_detail_id->AdvancedSearch->unsetSession();
        $this->asset_pros_id->AdvancedSearch->unsetSession();
        ...
        $this->group_type->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->asset_pros_detail_id->AdvancedSearch->load();
        $this->asset_pros_id->AdvancedSearch->load();
        $this->asset_id->AdvancedSearch->load();
        $this->detail->AdvancedSearch->load();
        $this->detail_en->AdvancedSearch->load();
        $this->group_type->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->detail, $ctrl); // detail
            $this->updateSort($this->group_type, $ctrl); // group_type
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`detail` ASC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->detail->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->detail->setSort("ASC");
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->asset_pros_detail_id->setSort("");
                $this->asset_pros_id->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_pros_detaillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_pros_detaillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->asset_pros_detail_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("detail", $this->createColumnOption("detail"));
            $option->add("group_type", $this->createColumnOption("group_type"));
            $option->add("latitude", $this->createColumnOption("latitude"));
            $option->add("longitude", $this->createColumnOption("longitude"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fasset_pros_detailsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fasset_pros_detailsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fasset_pros_detaillist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // asset_pros_detail_id
        if ($this->asset_pros_detail_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->asset_pros_detail_id->AdvancedSearch->SearchValue != "" || $this->asset_pros_detail_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // asset_pros_id
        if ($this->asset_pros_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->asset_pros_id->AdvancedSearch->SearchValue != "" || $this->asset_pros_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // group_type
        if ($this->group_type->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->group_type->AdvancedSearch->SearchValue != "" || $this->group_type->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->asset_pros_detail_id->setDbValue($row['asset_pros_detail_id']);
        $this->asset_pros_id->setDbValue($row['asset_pros_id']);
    ...
    {
        $row = [];
        $row['asset_pros_detail_id'] = null;
        $row['asset_pros_id'] = null;
        $row['asset_id'] = null;
        $row['detail'] = null;
        $row['detail_en'] = null;
        $row['group_type'] = null;
    ...
    public function renderRow()
    {
        ...
        // asset_pros_detail_id
        $this->asset_pros_detail_id->CellCssStyle = "white-space: nowrap;";

        // asset_pros_id
        $this->asset_pros_id->CellCssStyle = "white-space: nowrap;";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // group_type
            if (strval($this->group_type->CurrentValue) != "") {
                $this->group_type->ViewValue = $this->group_type->optionCaption($this->group_type->CurrentValue);
            } else {
                $this->group_type->ViewValue = null;
            }
            ...
            if (!$this->isExport()) {
                $this->detail->ViewValue = $this->highlightValue($this->detail);
            }
            ...
            $this->detail->EditValue = HtmlEncode($this->detail->AdvancedSearch->SearchValue);
            ...
            $this->group_type->EditValue = $this->group_type->options(true);
            ...
            if (!$this->latitude->Raw) {
                $this->latitude->AdvancedSearch->SearchValue = HtmlDecode($this->latitude->AdvancedSearch->SearchValue);
            }
            ...
            if (!$this->longitude->Raw) {
                $this->longitude->AdvancedSearch->SearchValue = HtmlDecode($this->longitude->AdvancedSearch->SearchValue);
            }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fasset_pros_detailsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-ew-action=\"highlight\" data-form=\"fasset_pros_detailsrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "asset") {
            $asset = Container("asset");
            $rsmaster = $asset->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $asset;
                    $asset->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
