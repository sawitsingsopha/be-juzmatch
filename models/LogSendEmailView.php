 ...
 * Page class
 */
class LogSendEmailView extends LogSendEmail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogSendEmailView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("log_email_id") ?? Route("log_email_id")) !== null) {
            $this->RecKey["log_email_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("log_email_id") ?? Route("log_email_id")) !== null) {
                ...
                $this->RecKey["log_email_id"] = $this->log_email_id->QueryStringValue;
            } elseif (Post("log_email_id") !== null) {
                $this->log_email_id->setFormValue(Post("log_email_id"));
                $this->RecKey["log_email_id"] = $this->log_email_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->log_email_id->setQueryStringValue($keyValue);
                $this->RecKey["log_email_id"] = $this->log_email_id->QueryStringValue;
            } else {
                $returnUrl = "logsendemaillist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "logsendemaillist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "logsendemaillist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
