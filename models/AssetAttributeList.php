 ...
 * Page class
 */
class AssetAttributeList extends AssetAttribute
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'asset_attribute';

    // Page object name
    public $PageObjName = "AssetAttributeList";
    ...
    public $FormName = "fasset_attributelist";
    ...
    public function __construct()
    {
        ...
        // Table object (asset_attribute)
        if (!isset($GLOBALS["asset_attribute"]) || get_class($GLOBALS["asset_attribute"]) == PROJECT_NAMESPACE . "asset_attribute") {
            $GLOBALS["asset_attribute"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "assetattributeadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "assetattributedelete";
        $this->MultiUpdateUrl = "assetattributeupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'asset_attribute');
        }
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->asset_attribute_id->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->asset_attribute_id->setVisibility();
        $this->asset_id->setVisibility();
        $this->attribute_id->setVisibility();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->asset_attribute_id, $ctrl); // asset_attribute_id
            $this->updateSort($this->asset_id, $ctrl); // asset_id
            $this->updateSort($this->attribute_id, $ctrl); // attribute_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->asset_attribute_id->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_attributelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_attributelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->asset_attribute_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("asset_attribute_id", $this->createColumnOption("asset_attribute_id"));
            $option->add("asset_id", $this->createColumnOption("asset_id"));
            $option->add("attribute_id", $this->createColumnOption("attribute_id"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fasset_attributesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fasset_attributesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fasset_attributelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
