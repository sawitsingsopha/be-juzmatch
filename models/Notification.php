 ...
 */
class Notification extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'notification';
        $this->TableName = 'notification';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`notification`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // notification_id
        $this->notification_id = new DbField(
            'notification',
            'notification',
            'x_notification_id',
            'notification_id',
        ...
        $this->notification_id->InputTextType = "text";
        $this->notification_id->IsAutoIncrement = true; // Autoincrement field
        $this->notification_id->IsPrimaryKey = true; // Primary key field
        $this->notification_id->Sortable = false; // Allow sort
        $this->notification_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['notification_id'] = &$this->notification_id;
        ...
        $this->is_read->Nullable = false; // NOT NULL field
        $this->is_read->Required = true; // Required field
        $this->is_read->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`notification`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->notification_id->setDbValue($conn->lastInsertId());
            $rs['notification_id'] = $this->notification_id->DbValue;
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('notification_id', $rs)) {
                AddFilter($where, QuotedName('notification_id', $this->Dbid) . '=' . QuotedValue($rs['notification_id'], $this->notification_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->notification_id->DbValue = $row['notification_id'];
        ...
        $this->is_read->DbValue = $row['is_read'];
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->notification_id->CurrentValue : $this->notification_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->notification_id->CurrentValue = $keys[0];
            } else {
                $this->notification_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('notification_id', $row) ? $row['notification_id'] : null;
        } else {
            $val = $this->notification_id->OldValue !== null ? $this->notification_id->OldValue : $this->notification_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@notification_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("notificationlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "notificationview") {
            return $Language->phrase("View");
        } elseif ($pageName == "notificationedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "notificationadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("notificationview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("notificationview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "notificationadd?" . $this->getUrlParm($parm);
        } else {
            $url = "notificationadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("notificationedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("notificationadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("notificationdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"notification_id\":" . JsonEncode($this->notification_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->notification_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->notification_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("notification_id") ?? Route("notification_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->notification_id->CurrentValue = $key;
            } else {
                $this->notification_id->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // notification_id
        $this->notification_id->ViewValue = $this->notification_id->CurrentValue;
        $this->notification_id->ViewCustomAttributes = "";
        ...
        // notification_id
        $this->notification_id->LinkCustomAttributes = "";
        $this->notification_id->HrefValue = "";
        $this->notification_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // notification_id
        $this->notification_id->setupEditAttributes();
        $this->notification_id->EditCustomAttributes = "";
        $this->notification_id->EditValue = $this->notification_id->CurrentValue;
        $this->notification_id->ViewCustomAttributes = "";
        ...
        // is_read
        $this->is_read->setupEditAttributes();
        $this->is_read->EditCustomAttributes = "";
        $this->is_read->EditValue = $this->is_read->CurrentValue;
        $this->is_read->PlaceHolder = RemoveHtml($this->is_read->caption());
        if (strval($this->is_read->EditValue) != "" && is_numeric($this->is_read->EditValue)) {
            $this->is_read->EditValue = FormatNumber($this->is_read->EditValue, null);
        }
