 ...
 * Page class
 */
class AllBuyerConfigAssetScheduleView extends AllBuyerConfigAssetSchedule
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AllBuyerConfigAssetScheduleView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("buyer_config_asset_schedule_id") ?? Route("buyer_config_asset_schedule_id")) !== null) {
            $this->RecKey["buyer_config_asset_schedule_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("buyer_config_asset_schedule_id") ?? Route("buyer_config_asset_schedule_id")) !== null) {
                ...
                $this->RecKey["buyer_config_asset_schedule_id"] = $this->buyer_config_asset_schedule_id->QueryStringValue;
                ...
                $this->RecKey["buyer_config_asset_schedule_id"] = $this->buyer_config_asset_schedule_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->buyer_config_asset_schedule_id->setQueryStringValue($keyValue);
                $this->RecKey["buyer_config_asset_schedule_id"] = $this->buyer_config_asset_schedule_id->QueryStringValue;
            } else {
                $returnUrl = "allbuyerconfigassetschedulelist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "allbuyerconfigassetschedulelist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "allbuyerconfigassetschedulelist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item = &$option->add("detail_all_buyer_asset_schedule");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("all_buyer_asset_schedule", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("allbuyerassetschedulelist?" . Config("TABLE_SHOW_MASTER") . "=all_buyer_config_asset_schedule&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("AllBuyerAssetScheduleGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'all_buyer_config_asset_schedule')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=all_buyer_asset_schedule"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "all_buyer_asset_schedule";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'all_buyer_config_asset_schedule')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=all_buyer_asset_schedule"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
