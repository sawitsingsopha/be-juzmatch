 ...
 * Page class
 */
class AssetStockReportList extends AssetStockReport
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'assetStockReport';

    // Page object name
    public $PageObjName = "AssetStockReportList";
    ...
    public $FormName = "fassetStockReportlist";
    ...
    public function __construct()
    {
        ...
        // Table object (assetStockReport)
        if (!isset($GLOBALS["assetStockReport"]) || get_class($GLOBALS["assetStockReport"]) == PROJECT_NAMESPACE . "assetStockReport") {
            $GLOBALS["assetStockReport"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "assetstockreportadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "assetstockreportdelete";
        $this->MultiUpdateUrl = "assetstockreportupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'assetStockReport');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("assetStockReport");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->consignment_period->setVisibility();
        $this->member_favorite->setVisibility();
        $this->member_appointment->setVisibility();
        $this->member_appointment_done->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fassetStockReportsrch");
        }
        ...
        $filterList = Concat($filterList, $this->consignment_period->AdvancedSearch->toJson(), ","); // Field consignment_period
        $filterList = Concat($filterList, $this->member_favorite->AdvancedSearch->toJson(), ","); // Field member_favorite
        $filterList = Concat($filterList, $this->member_appointment->AdvancedSearch->toJson(), ","); // Field member_appointment
        $filterList = Concat($filterList, $this->member_appointment_done->AdvancedSearch->toJson(), ","); // Field member_appointment_done
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fassetStockReportsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field consignment_period
        $this->consignment_period->AdvancedSearch->SearchValue = @$filter["x_consignment_period"];
        $this->consignment_period->AdvancedSearch->SearchOperator = @$filter["z_consignment_period"];
        $this->consignment_period->AdvancedSearch->SearchCondition = @$filter["v_consignment_period"];
        $this->consignment_period->AdvancedSearch->SearchValue2 = @$filter["y_consignment_period"];
        $this->consignment_period->AdvancedSearch->SearchOperator2 = @$filter["w_consignment_period"];
        $this->consignment_period->AdvancedSearch->save();

        // Field member_favorite
        $this->member_favorite->AdvancedSearch->SearchValue = @$filter["x_member_favorite"];
        $this->member_favorite->AdvancedSearch->SearchOperator = @$filter["z_member_favorite"];
        $this->member_favorite->AdvancedSearch->SearchCondition = @$filter["v_member_favorite"];
        $this->member_favorite->AdvancedSearch->SearchValue2 = @$filter["y_member_favorite"];
        $this->member_favorite->AdvancedSearch->SearchOperator2 = @$filter["w_member_favorite"];
        $this->member_favorite->AdvancedSearch->save();

        // Field member_appointment
        $this->member_appointment->AdvancedSearch->SearchValue = @$filter["x_member_appointment"];
        $this->member_appointment->AdvancedSearch->SearchOperator = @$filter["z_member_appointment"];
        $this->member_appointment->AdvancedSearch->SearchCondition = @$filter["v_member_appointment"];
        $this->member_appointment->AdvancedSearch->SearchValue2 = @$filter["y_member_appointment"];
        $this->member_appointment->AdvancedSearch->SearchOperator2 = @$filter["w_member_appointment"];
        $this->member_appointment->AdvancedSearch->save();

        // Field member_appointment_done
        $this->member_appointment_done->AdvancedSearch->SearchValue = @$filter["x_member_appointment_done"];
        $this->member_appointment_done->AdvancedSearch->SearchOperator = @$filter["z_member_appointment_done"];
        $this->member_appointment_done->AdvancedSearch->SearchCondition = @$filter["v_member_appointment_done"];
        $this->member_appointment_done->AdvancedSearch->SearchValue2 = @$filter["y_member_appointment_done"];
        $this->member_appointment_done->AdvancedSearch->SearchOperator2 = @$filter["w_member_appointment_done"];
        $this->member_appointment_done->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->consignment_period, $default, false); // consignment_period
        $this->buildSearchSql($where, $this->member_favorite, $default, false); // member_favorite
        $this->buildSearchSql($where, $this->member_appointment, $default, false); // member_appointment
        $this->buildSearchSql($where, $this->member_appointment_done, $default, false); // member_appointment_done
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->consignment_period->AdvancedSearch->save(); // consignment_period
            $this->member_favorite->AdvancedSearch->save(); // member_favorite
            $this->member_appointment->AdvancedSearch->save(); // member_appointment
            $this->member_appointment_done->AdvancedSearch->save(); // member_appointment_done
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->consignment_period->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->member_favorite->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->member_appointment->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->member_appointment_done->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->consignment_period->AdvancedSearch->unsetSession();
        $this->member_favorite->AdvancedSearch->unsetSession();
        $this->member_appointment->AdvancedSearch->unsetSession();
        $this->member_appointment_done->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->consignment_period->AdvancedSearch->load();
        $this->member_favorite->AdvancedSearch->load();
        $this->member_appointment->AdvancedSearch->load();
        $this->member_appointment_done->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->consignment_period, $ctrl); // consignment_period
            $this->updateSort($this->member_favorite, $ctrl); // member_favorite
            $this->updateSort($this->member_appointment, $ctrl); // member_appointment
            $this->updateSort($this->member_appointment_done, $ctrl); // member_appointment_done
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->price->setSort("");
                $this->consignment_period->setSort("");
                $this->member_favorite->setSort("");
                $this->member_appointment->setSort("");
                $this->member_appointment_done->setSort("");
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fassetStockReportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fassetStockReportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("consignment_period", $this->createColumnOption("consignment_period"));
            $option->add("member_favorite", $this->createColumnOption("member_favorite"));
            $option->add("member_appointment", $this->createColumnOption("member_appointment"));
            $option->add("member_appointment_done", $this->createColumnOption("member_appointment_done"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fassetStockReportsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fassetStockReportsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fassetStockReportlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // consignment_period
        if ($this->consignment_period->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->consignment_period->AdvancedSearch->SearchValue != "" || $this->consignment_period->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // member_favorite
        if ($this->member_favorite->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->member_favorite->AdvancedSearch->SearchValue != "" || $this->member_favorite->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // member_appointment
        if ($this->member_appointment->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->member_appointment->AdvancedSearch->SearchValue != "" || $this->member_appointment->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // member_appointment_done
        if ($this->member_appointment_done->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->member_appointment_done->AdvancedSearch->SearchValue != "" || $this->member_appointment_done->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->consignment_period->setDbValue($row['consignment_period']);
        $this->member_favorite->setDbValue($row['member_favorite']);
        $this->member_appointment->setDbValue($row['member_appointment']);
        $this->member_appointment_done->setDbValue($row['member_appointment_done']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['price'] = null;
        $row['consignment_period'] = null;
        $row['member_favorite'] = null;
        $row['member_appointment'] = null;
        $row['member_appointment_done'] = null;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // consignment_period
            $this->consignment_period->ViewValue = $this->consignment_period->CurrentValue;
            $this->consignment_period->ViewValue = FormatNumber($this->consignment_period->ViewValue, $this->consignment_period->formatPattern());
            $this->consignment_period->ViewCustomAttributes = "";

            // member_favorite
            $this->member_favorite->ViewValue = $this->member_favorite->CurrentValue;
            $this->member_favorite->ViewValue = FormatNumber($this->member_favorite->ViewValue, $this->member_favorite->formatPattern());
            $this->member_favorite->ViewCustomAttributes = "";

            // member_appointment
            $this->member_appointment->ViewValue = $this->member_appointment->CurrentValue;
            $this->member_appointment->ViewValue = FormatNumber($this->member_appointment->ViewValue, $this->member_appointment->formatPattern());
            $this->member_appointment->ViewCustomAttributes = "";

            // member_appointment_done
            $this->member_appointment_done->ViewValue = $this->member_appointment_done->CurrentValue;
            $this->member_appointment_done->ViewValue = FormatNumber($this->member_appointment_done->ViewValue, $this->member_appointment_done->formatPattern());
            $this->member_appointment_done->ViewCustomAttributes = "";
            ...
            // consignment_period
            $this->consignment_period->LinkCustomAttributes = "";
            $this->consignment_period->HrefValue = "";
            $this->consignment_period->TooltipValue = "";

            // member_favorite
            $this->member_favorite->LinkCustomAttributes = "";
            $this->member_favorite->HrefValue = "";
            $this->member_favorite->TooltipValue = "";

            // member_appointment
            $this->member_appointment->LinkCustomAttributes = "";
            $this->member_appointment->HrefValue = "";
            $this->member_appointment->TooltipValue = "";

            // member_appointment_done
            $this->member_appointment_done->LinkCustomAttributes = "";
            $this->member_appointment_done->HrefValue = "";
            $this->member_appointment_done->TooltipValue = "";
            ...
            // consignment_period
            $this->consignment_period->setupEditAttributes();
            $this->consignment_period->EditCustomAttributes = "";
            $this->consignment_period->EditValue = HtmlEncode($this->consignment_period->AdvancedSearch->SearchValue);
            $this->consignment_period->PlaceHolder = RemoveHtml($this->consignment_period->caption());

            // member_favorite
            $this->member_favorite->setupEditAttributes();
            $this->member_favorite->EditCustomAttributes = "";
            $this->member_favorite->EditValue = HtmlEncode($this->member_favorite->AdvancedSearch->SearchValue);
            $this->member_favorite->PlaceHolder = RemoveHtml($this->member_favorite->caption());

            // member_appointment
            $this->member_appointment->setupEditAttributes();
            $this->member_appointment->EditCustomAttributes = "";
            $this->member_appointment->EditValue = HtmlEncode($this->member_appointment->AdvancedSearch->SearchValue);
            $this->member_appointment->PlaceHolder = RemoveHtml($this->member_appointment->caption());

            // member_appointment_done
            $this->member_appointment_done->setupEditAttributes();
            $this->member_appointment_done->EditCustomAttributes = "";
            $this->member_appointment_done->EditValue = HtmlEncode($this->member_appointment_done->AdvancedSearch->SearchValue);
            $this->member_appointment_done->PlaceHolder = RemoveHtml($this->member_appointment_done->caption());
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fassetStockReportsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
