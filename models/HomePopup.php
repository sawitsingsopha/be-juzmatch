...
/**
 * Table class for home_popup
 */
class HomePopup extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'home_popup';
        $this->TableName = 'home_popup';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`home_popup`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // home_popup_id
        $this->home_popup_id = new DbField(
            'home_popup',
            'home_popup',
            'x_home_popup_id',
            'home_popup_id',
        ...
        $this->home_popup_id->InputTextType = "text";
        $this->home_popup_id->IsAutoIncrement = true; // Autoincrement field
        $this->home_popup_id->IsPrimaryKey = true; // Primary key field
        $this->home_popup_id->Sortable = false; // Allow sort
        $this->home_popup_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['home_popup_id'] = &$this->home_popup_id;
        ...
        $this->active_status->Lookup = new Lookup('active_status', 'home_popup', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`home_popup`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->home_popup_id->setDbValue($conn->lastInsertId());
            $rs['home_popup_id'] = $this->home_popup_id->DbValue;
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('home_popup_id', $rs)) {
                AddFilter($where, QuotedName('home_popup_id', $this->Dbid) . '=' . QuotedValue($rs['home_popup_id'], $this->home_popup_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->home_popup_id->DbValue = $row['home_popup_id'];
    ...
    {
        return "`home_popup_id` = @home_popup_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->home_popup_id->CurrentValue : $this->home_popup_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->home_popup_id->CurrentValue = $keys[0];
            } else {
                $this->home_popup_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('home_popup_id', $row) ? $row['home_popup_id'] : null;
        } else {
            $val = $this->home_popup_id->OldValue !== null ? $this->home_popup_id->OldValue : $this->home_popup_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@home_popup_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("homepopuplist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "homepopupedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "homepopupadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_EDIT_ACTION"):
                return "HomePopupEdit";
            case Config("API_DELETE_ACTION"):
                return "HomePopupDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("homepopupview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("homepopupview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "homepopupadd?" . $this->getUrlParm($parm);
        } else {
            $url = "homepopupadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("homepopupedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("homepopupadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("homepopupdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"home_popup_id\":" . JsonEncode($this->home_popup_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->home_popup_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->home_popup_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("home_popup_id") ?? Route("home_popup_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->home_popup_id->CurrentValue = $key;
            } else {
                $this->home_popup_id->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // home_popup_id
        $this->home_popup_id->CellCssStyle = "white-space: nowrap;";
        ...
        // home_popup_id
        $this->home_popup_id->ViewValue = $this->home_popup_id->CurrentValue;
        $this->home_popup_id->ViewCustomAttributes = "";
        ...
        // home_popup_id
        $this->home_popup_id->LinkCustomAttributes = "";
        $this->home_popup_id->HrefValue = "";
        $this->home_popup_id->TooltipValue = "";
        ...
        if ($this->image->UseColorbox) {
            ...
            $this->image->LinkAttrs["data-rel"] = "home_popup_x_image";
    ...
    public function renderEditRow()
    {
        ...
        // home_popup_id
        $this->home_popup_id->setupEditAttributes();
        $this->home_popup_id->EditCustomAttributes = "";
        $this->home_popup_id->EditValue = $this->home_popup_id->CurrentValue;
        $this->home_popup_id->ViewCustomAttributes = "";
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        if (count($ar) == 1) {
            $this->home_popup_id->CurrentValue = $ar[0];
        } else {
