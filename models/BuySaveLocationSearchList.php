 ...
 * Page class
 */
class BuySaveLocationSearchList extends BuySaveLocationSearch
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'buy_save_location_search';

    // Page object name
    public $PageObjName = "BuySaveLocationSearchList";
    ...
    public $FormName = "fbuy_save_location_searchlist";
    ...
    public function __construct()
    {
        ...
        // Table object (buy_save_location_search)
        if (!isset($GLOBALS["buy_save_location_search"]) || get_class($GLOBALS["buy_save_location_search"]) == PROJECT_NAMESPACE . "buy_save_location_search") {
            $GLOBALS["buy_save_location_search"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "buysavelocationsearchadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "buysavelocationsearchdelete";
        $this->MultiUpdateUrl = "buysavelocationsearchupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'buy_save_location_search');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("buy_save_location_search");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->buy_save_location_search_id->setVisibility();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->buy_save_location_search_id, $ctrl); // buy_save_location_search_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->buy_save_location_search_id->setSort("");
                $this->name->setSort("");
                $this->cdate->setSort("");
                $this->cip->setSort("");
                $this->cuser->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuy_save_location_searchlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuy_save_location_searchlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("buy_save_location_search_id", $this->createColumnOption("buy_save_location_search_id"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbuy_save_location_searchsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbuy_save_location_searchsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbuy_save_location_searchlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->buy_save_location_search_id->setDbValue($row['buy_save_location_search_id']);
    ...
    {
        $row = [];
        $row['buy_save_location_search_id'] = null;
        $row['name'] = null;
        $row['cdate'] = null;
        $row['cip'] = null;
        $row['cuser'] = null;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            // buy_save_location_search_id
            $this->buy_save_location_search_id->ViewValue = $this->buy_save_location_search_id->CurrentValue;
            $this->buy_save_location_search_id->ViewValue = FormatNumber($this->buy_save_location_search_id->ViewValue, $this->buy_save_location_search_id->formatPattern());
            $this->buy_save_location_search_id->ViewCustomAttributes = "";
            ...
            // buy_save_location_search_id
            $this->buy_save_location_search_id->LinkCustomAttributes = "";
            $this->buy_save_location_search_id->HrefValue = "";
            $this->buy_save_location_search_id->TooltipValue = "";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fbuy_save_location_searchlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
