...
/**
 * Table class for check_login
 */
class CheckLogin extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'check_login';
        $this->TableName = 'check_login';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`check_login`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // check_login_id
        $this->check_login_id = new DbField(
            'check_login',
            'check_login',
            'x_check_login_id',
            'check_login_id',
        ...
        $this->check_login_id->InputTextType = "text";
        $this->check_login_id->IsAutoIncrement = true; // Autoincrement field
        $this->check_login_id->IsPrimaryKey = true; // Primary key field
        $this->check_login_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['check_login_id'] = &$this->check_login_id;
        ...
        // lock_time
        $this->lock_time = new DbField(
            'check_login',
            'check_login',
            'x_lock_time',
            'lock_time',
            '`lock_time`',
            CastDateFieldForLike("`lock_time`", 0, "DB"),
            135,
            19,
            0,
            false,
        ...
        $this->lock_time->InputTextType = "text";
        $this->lock_time->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
        $this->Fields['lock_time'] = &$this->lock_time;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`check_login`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->check_login_id->setDbValue($conn->lastInsertId());
            $rs['check_login_id'] = $this->check_login_id->DbValue;
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('check_login_id', $rs)) {
                AddFilter($where, QuotedName('check_login_id', $this->Dbid) . '=' . QuotedValue($rs['check_login_id'], $this->check_login_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->check_login_id->DbValue = $row['check_login_id'];
        ...
        $this->lock_time->DbValue = $row['lock_time'];
    }
    ...
    {
        return "`check_login_id` = @check_login_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->check_login_id->CurrentValue : $this->check_login_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->check_login_id->CurrentValue = $keys[0];
            } else {
                $this->check_login_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('check_login_id', $row) ? $row['check_login_id'] : null;
        } else {
            $val = $this->check_login_id->OldValue !== null ? $this->check_login_id->OldValue : $this->check_login_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@check_login_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("checkloginlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "checkloginview") {
            return $Language->phrase("View");
        } elseif ($pageName == "checkloginedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "checkloginadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "CheckLoginView";
            case Config("API_ADD_ACTION"):
                return "CheckLoginAdd";
            case Config("API_EDIT_ACTION"):
                return "CheckLoginEdit";
            case Config("API_DELETE_ACTION"):
                return "CheckLoginDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("checkloginview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("checkloginview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "checkloginadd?" . $this->getUrlParm($parm);
        } else {
            $url = "checkloginadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("checkloginedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("checkloginadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("checklogindelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"check_login_id\":" . JsonEncode($this->check_login_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->check_login_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->check_login_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("check_login_id") ?? Route("check_login_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->check_login_id->CurrentValue = $key;
            } else {
                $this->check_login_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->check_login_id->setDbValue($row['check_login_id']);
        ...
        $this->lock_time->setDbValue($row['lock_time']);
    }
    ...
    public function renderListRow()
    {
        ...
        // check_login_id
        $this->check_login_id->ViewValue = $this->check_login_id->CurrentValue;
        $this->check_login_id->ViewCustomAttributes = "";
        ...
        // lock_time
        $this->lock_time->ViewValue = $this->lock_time->CurrentValue;
        $this->lock_time->ViewValue = FormatDateTime($this->lock_time->ViewValue, $this->lock_time->formatPattern());
        $this->lock_time->ViewCustomAttributes = "";

        // check_login_id
        $this->check_login_id->LinkCustomAttributes = "";
        $this->check_login_id->HrefValue = "";
        $this->check_login_id->TooltipValue = "";
        ...
        // lock_time
        $this->lock_time->LinkCustomAttributes = "";
        $this->lock_time->HrefValue = "";
        $this->lock_time->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // check_login_id
        $this->check_login_id->setupEditAttributes();
        $this->check_login_id->EditCustomAttributes = "";
        $this->check_login_id->EditValue = $this->check_login_id->CurrentValue;
        $this->check_login_id->ViewCustomAttributes = "";
        ...
        // lock_time
        $this->lock_time->setupEditAttributes();
        $this->lock_time->EditCustomAttributes = "";
        $this->lock_time->EditValue = FormatDateTime($this->lock_time->CurrentValue, $this->lock_time->formatPattern());
        $this->lock_time->PlaceHolder = RemoveHtml($this->lock_time->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->check_login_id);
                    ...
                    $doc->exportCaption($this->lock_time);
                } else {
                    $doc->exportCaption($this->check_login_id);
                    $doc->exportCaption($this->_email);
                    $doc->exportCaption($this->cip);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->check_login_id);
                        ...
                        $doc->exportField($this->lock_time);
                    } else {
                        $doc->exportField($this->check_login_id);
                        $doc->exportField($this->_email);
                        $doc->exportField($this->cip);
