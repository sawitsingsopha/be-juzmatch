 ...
 * Page class
 */
class SaleAssetEdit extends SaleAsset
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SaleAssetEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("sale_asset_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->sale_asset_id->setQueryStringValue($keyValue);
                $this->sale_asset_id->setOldValue($this->sale_asset_id->QueryStringValue);
            } elseif (Post("sale_asset_id") !== null) {
                $this->sale_asset_id->setFormValue(Post("sale_asset_id"));
                $this->sale_asset_id->setOldValue($this->sale_asset_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("sale_asset_id") ?? Route("sale_asset_id")) !== null) {
                    $this->sale_asset_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("saleassetlist"); // No matching record, return to list
                        return;
                    }
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "saleassetlist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->sale_asset_id->IsDetailKey) {
    ...
    public function renderRow()
    {
        ...
        // sale_asset_id
        $this->sale_asset_id->RowCssClass = "row";
        ...
        // status_live
        $this->status_live->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("asset", $detailTblVar) && $detailPage->DetailEdit) {
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("asset", $detailTblVar)) {
                $detailPageObj = Container("AssetGrid");
                if ($detailPageObj->DetailEdit) {
                    $detailPageObj->CurrentMode = "edit";
                    $detailPageObj->CurrentAction = "gridedit";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("saleassetlist"), "", $this->TableVar, true);
        $pageId = "edit";
