 ...
 * Page class
 */
class BuyerAssetRentList extends BuyerAssetRent
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BuyerAssetRentList";
    ...
    public $FormName = "fbuyer_asset_rentlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "buyerassetrentdelete";
        $this->MultiUpdateUrl = "buyerassetrentupdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->pay_number_half_price_1, $ctrl); // pay_number_half_price_1
            $this->updateSort($this->date_pay_half_price_1, $ctrl); // date_pay_half_price_1
            ...
            $this->updateSort($this->pay_number_half_price_2, $ctrl); // pay_number_half_price_2
            $this->updateSort($this->date_pay_half_price_2, $ctrl); // date_pay_half_price_2
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuyer_asset_rentlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuyer_asset_rentlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("pay_number_half_price_1", $this->createColumnOption("pay_number_half_price_1"));
            $option->add("date_pay_half_price_1", $this->createColumnOption("date_pay_half_price_1"));
            ...
            $option->add("pay_number_half_price_2", $this->createColumnOption("pay_number_half_price_2"));
            $option->add("date_pay_half_price_2", $this->createColumnOption("date_pay_half_price_2"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbuyer_asset_rentsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbuyer_asset_rentsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbuyer_asset_rentlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fbuyer_asset_rentlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
