 ...
 * Page class
 */
class MemberScbDetailList extends MemberScbDetail
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'member_scb_detail';

    // Page object name
    public $PageObjName = "MemberScbDetailList";
    ...
    public $FormName = "fmember_scb_detaillist";
    ...
    public function __construct()
    {
        ...
        // Table object (member_scb_detail)
        if (!isset($GLOBALS["member_scb_detail"]) || get_class($GLOBALS["member_scb_detail"]) == PROJECT_NAMESPACE . "member_scb_detail") {
            $GLOBALS["member_scb_detail"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "memberscbdetailadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "memberscbdetaildelete";
        $this->MultiUpdateUrl = "memberscbdetailupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'member_scb_detail');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("member_scb_detail");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->member_scb_detail_id->Visible = false;
        }
        if ($this->isAddOrEdit()) {
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fmember_scb_detailsrch");
        }
        $filterList = Concat($filterList, $this->member_scb_detail_id->AdvancedSearch->toJson(), ","); // Field member_scb_detail_id
        $filterList = Concat($filterList, $this->paid_amt->AdvancedSearch->toJson(), ","); // Field paid_amt
        $filterList = Concat($filterList, $this->pay_date->AdvancedSearch->toJson(), ","); // Field pay_date
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmember_scb_detailsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field member_scb_detail_id
        $this->member_scb_detail_id->AdvancedSearch->SearchValue = @$filter["x_member_scb_detail_id"];
        $this->member_scb_detail_id->AdvancedSearch->SearchOperator = @$filter["z_member_scb_detail_id"];
        $this->member_scb_detail_id->AdvancedSearch->SearchCondition = @$filter["v_member_scb_detail_id"];
        $this->member_scb_detail_id->AdvancedSearch->SearchValue2 = @$filter["y_member_scb_detail_id"];
        $this->member_scb_detail_id->AdvancedSearch->SearchOperator2 = @$filter["w_member_scb_detail_id"];
        $this->member_scb_detail_id->AdvancedSearch->save();
        ...
        // Field pay_date
        $this->pay_date->AdvancedSearch->SearchValue = @$filter["x_pay_date"];
        $this->pay_date->AdvancedSearch->SearchOperator = @$filter["z_pay_date"];
        $this->pay_date->AdvancedSearch->SearchCondition = @$filter["v_pay_date"];
        $this->pay_date->AdvancedSearch->SearchValue2 = @$filter["y_pay_date"];
        $this->pay_date->AdvancedSearch->SearchOperator2 = @$filter["w_pay_date"];
        $this->pay_date->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->member_scb_detail_id, $default, false); // member_scb_detail_id
        $this->buildSearchSql($where, $this->paid_amt, $default, false); // paid_amt
        $this->buildSearchSql($where, $this->pay_date, $default, false); // pay_date
        ...
        if (!$default && $this->Command == "search") {
            $this->member_scb_detail_id->AdvancedSearch->save(); // member_scb_detail_id
            $this->paid_amt->AdvancedSearch->save(); // paid_amt
            $this->pay_date->AdvancedSearch->save(); // pay_date
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->member_scb_id;
        $searchFlds[] = &$this->paid_amt;
        $searchFlds[] = &$this->pay_date;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->member_scb_detail_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->paid_amt->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->pay_date->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->member_scb_detail_id->AdvancedSearch->unsetSession();
        $this->paid_amt->AdvancedSearch->unsetSession();
        $this->pay_date->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->member_scb_detail_id->AdvancedSearch->load();
        $this->paid_amt->AdvancedSearch->load();
        $this->pay_date->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->member_scb_id, $ctrl); // member_scb_id
            $this->updateSort($this->paid_amt, $ctrl); // paid_amt
            $this->updateSort($this->pay_date, $ctrl); // pay_date
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->member_scb_detail_id->setSort("");
                $this->member_scb_id->setSort("");
                $this->paid_amt->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmember_scb_detaillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmember_scb_detaillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->member_scb_detail_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("member_scb_id", $this->createColumnOption("member_scb_id"));
            $option->add("paid_amt", $this->createColumnOption("paid_amt"));
            $option->add("pay_date", $this->createColumnOption("pay_date"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmember_scb_detailsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmember_scb_detailsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmember_scb_detaillist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // member_scb_detail_id
        if ($this->member_scb_detail_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->member_scb_detail_id->AdvancedSearch->SearchValue != "" || $this->member_scb_detail_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // paid_amt
        if ($this->paid_amt->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->paid_amt->AdvancedSearch->SearchValue != "" || $this->paid_amt->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // pay_date
        if ($this->pay_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->pay_date->AdvancedSearch->SearchValue != "" || $this->pay_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->member_scb_detail_id->setDbValue($row['member_scb_detail_id']);
    ...
    public function renderRow()
    {
        ...
        // member_scb_detail_id
        $this->member_scb_detail_id->CellCssStyle = "white-space: nowrap;";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->member_scb_id->EditValue = HtmlEncode($this->member_scb_id->AdvancedSearch->SearchValue);
            ...
            $this->paid_amt->EditValue = HtmlEncode($this->paid_amt->AdvancedSearch->SearchValue);
            ...
            $this->pay_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->pay_date->AdvancedSearch->SearchValue, $this->pay_date->formatPattern()), $this->pay_date->formatPattern()));
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmember_scb_detaillist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmember_scb_detailsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
