...
/**
 * Table class for log_notification
 */
class LogNotification extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'log_notification';
        $this->TableName = 'log_notification';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`log_notification`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // log_notification_id
        $this->log_notification_id = new DbField(
            'log_notification',
            'log_notification',
            'x_log_notification_id',
            'log_notification_id',
            '`log_notification_id`',
            '`log_notification_id`',
            3,
        ...
        $this->log_notification_id->InputTextType = "text";
        $this->log_notification_id->IsAutoIncrement = true; // Autoincrement field
        $this->log_notification_id->IsPrimaryKey = true; // Primary key field
        $this->log_notification_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['log_notification_id'] = &$this->log_notification_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`log_notification`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->log_notification_id->setDbValue($conn->lastInsertId());
            $rs['log_notification_id'] = $this->log_notification_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('log_notification_id', $rs)) {
                AddFilter($where, QuotedName('log_notification_id', $this->Dbid) . '=' . QuotedValue($rs['log_notification_id'], $this->log_notification_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->log_notification_id->DbValue = $row['log_notification_id'];
    ...
    {
        return "`log_notification_id` = @log_notification_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->log_notification_id->CurrentValue : $this->log_notification_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->log_notification_id->CurrentValue = $keys[0];
            } else {
                $this->log_notification_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('log_notification_id', $row) ? $row['log_notification_id'] : null;
        } else {
            $val = $this->log_notification_id->OldValue !== null ? $this->log_notification_id->OldValue : $this->log_notification_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@log_notification_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("lognotificationlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "lognotificationview") {
            return $Language->phrase("View");
        } elseif ($pageName == "lognotificationedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "lognotificationadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "LogNotificationView";
            case Config("API_ADD_ACTION"):
                return "LogNotificationAdd";
            case Config("API_EDIT_ACTION"):
                return "LogNotificationEdit";
            case Config("API_DELETE_ACTION"):
                return "LogNotificationDelete";
            case Config("API_LIST_ACTION"):
                return "LogNotificationList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("lognotificationview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("lognotificationview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "lognotificationadd?" . $this->getUrlParm($parm);
        } else {
            $url = "lognotificationadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("lognotificationedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("lognotificationadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("lognotificationdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"log_notification_id\":" . JsonEncode($this->log_notification_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->log_notification_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->log_notification_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("log_notification_id") ?? Route("log_notification_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->log_notification_id->CurrentValue = $key;
            } else {
                $this->log_notification_id->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        // log_notification_id
        $this->log_notification_id->setupEditAttributes();
        $this->log_notification_id->EditCustomAttributes = "";
        $this->log_notification_id->EditValue = $this->log_notification_id->CurrentValue;
        $this->log_notification_id->ViewCustomAttributes = "";
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->log_notification_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->log_notification_id);
