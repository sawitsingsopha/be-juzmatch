 ...
 * Page class
 */
class LogNotificationEdit extends LogNotification
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogNotificationEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("log_notification_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->log_notification_id->setQueryStringValue($keyValue);
                $this->log_notification_id->setOldValue($this->log_notification_id->QueryStringValue);
            } elseif (Post("log_notification_id") !== null) {
                $this->log_notification_id->setFormValue(Post("log_notification_id"));
                $this->log_notification_id->setOldValue($this->log_notification_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("log_notification_id") ?? Route("log_notification_id")) !== null) {
                    $this->log_notification_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->log_notification_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("lognotificationlist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "lognotificationlist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'log_notification_id' first before field var 'x_log_notification_id'
        $val = $CurrentForm->hasValue("log_notification_id") ? $CurrentForm->getValue("log_notification_id") : $CurrentForm->getValue("x_log_notification_id");
        if (!$this->log_notification_id->IsDetailKey) {
            $this->log_notification_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->log_notification_id->CurrentValue = $this->log_notification_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // log_notification_id
        $this->log_notification_id->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->log_notification_id->Required) {
            if (!$this->log_notification_id->IsDetailKey && EmptyValue($this->log_notification_id->FormValue)) {
                $this->log_notification_id->addErrorMessage(str_replace("%s", $this->log_notification_id->caption(), $this->log_notification_id->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // cdate
            $this->cdate->setDbValueDef($rsnew, UnFormatDateTime($this->cdate->CurrentValue, $this->cdate->formatPattern()), CurrentDate(), $this->cdate->ReadOnly);

            // cuser
            $this->cuser->setDbValueDef($rsnew, $this->cuser->CurrentValue, 0, $this->cuser->ReadOnly);

            // cip
            $this->cip->setDbValueDef($rsnew, $this->cip->CurrentValue, "", $this->cip->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("lognotificationlist"), "", $this->TableVar, true);
        $pageId = "edit";
