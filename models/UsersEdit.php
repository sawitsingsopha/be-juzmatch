...
class UsersEdit extends Users
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("users_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->users_id->setQueryStringValue($keyValue);
                $this->users_id->setOldValue($this->users_id->QueryStringValue);
            } elseif (Post("users_id") !== null) {
                $this->users_id->setFormValue(Post("users_id"));
                $this->users_id->setOldValue($this->users_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->users_id->IsDetailKey) {
            $this->users_id->setFormValue($val);
        }
		$this->image->OldUploadPath = 'upload/user';
		$this->image->UploadPath = $this->image->OldUploadPath;
    ...
    {
        global $CurrentForm;
        $this->users_id->CurrentValue = $this->users_id->FormValue;
