...
/**
 * Table class for doc_creden
 */
class DocCreden extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'doc_creden';
        $this->TableName = 'doc_creden';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`doc_creden`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->doc_creden_id->IsAutoIncrement = true; // Autoincrement field
        $this->doc_creden_id->IsPrimaryKey = true; // Primary key field
        ...
        $this->send_email->InputTextType = "text";
        $this->Fields['send_email'] = &$this->send_email;

        // redirect_url
        $this->redirect_url = new DbField(
            'doc_creden',
            'doc_creden',
            'x_redirect_url',
            'redirect_url',
        ...
        $this->redirect_url->InputTextType = "text";
        $this->Fields['redirect_url'] = &$this->redirect_url;
        ...
        $this->doc_url->InputTextType = "text";
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`doc_creden`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->doc_creden_id->setDbValue($conn->lastInsertId());
            $rs['doc_creden_id'] = $this->doc_creden_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('doc_creden_id', $rs)) {
                AddFilter($where, QuotedName('doc_creden_id', $this->Dbid) . '=' . QuotedValue($rs['doc_creden_id'], $this->doc_creden_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->send_email->DbValue = $row['send_email'];
        $this->redirect_url->DbValue = $row['redirect_url'];
        ...
        $this->doc_url->DbValue = $row['doc_url'];
    }
    ...
    {
        return "`doc_creden_id` = @doc_creden_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->doc_creden_id->CurrentValue : $this->doc_creden_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->doc_creden_id->CurrentValue = $keys[0];
            } else {
                $this->doc_creden_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('doc_creden_id', $row) ? $row['doc_creden_id'] : null;
        } else {
            $val = $this->doc_creden_id->OldValue !== null ? $this->doc_creden_id->OldValue : $this->doc_creden_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@doc_creden_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("doccredenlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "doccredenedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "doccredenadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "DocCredenView";
            case Config("API_ADD_ACTION"):
                return "DocCredenAdd";
            case Config("API_EDIT_ACTION"):
                return "DocCredenEdit";
            case Config("API_DELETE_ACTION"):
                return "DocCredenDelete";
            case Config("API_LIST_ACTION"):
                return "DocCredenList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("doccredenview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("doccredenview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "doccredenadd?" . $this->getUrlParm($parm);
        } else {
            $url = "doccredenadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("doccredenedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("doccredenadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("doccredendelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"doc_creden_id\":" . JsonEncode($this->doc_creden_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->doc_creden_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->doc_creden_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("doc_creden_id") ?? Route("doc_creden_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->doc_creden_id->CurrentValue = $key;
            } else {
                $this->doc_creden_id->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->send_email->EditValue = $this->send_email->CurrentValue;
        ...
        $this->redirect_url->EditValue = $this->redirect_url->CurrentValue;
        ...
        $this->doc_url->EditValue = $this->doc_url->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->doc_temp_id);
                    $doc->exportCaption($this->txid);
                    $doc->exportCaption($this->subject);
                    $doc->exportCaption($this->send_email);
                    $doc->exportCaption($this->redirect_url);
                    ...
                    $doc->exportCaption($this->doc_url);
                } else {
                    $doc->exportCaption($this->document_id);
                    $doc->exportCaption($this->doc_creden_id);
                    $doc->exportCaption($this->doc_temp_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->doc_temp_id);
                        $doc->exportField($this->txid);
                        $doc->exportField($this->subject);
                        $doc->exportField($this->send_email);
                        $doc->exportField($this->redirect_url);
