...
/**
 * Table class for log_test_payment
 */
class LogTestPayment extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'log_test_payment';
        $this->TableName = 'log_test_payment';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`log_test_payment`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // log_test_payment_id
        $this->log_test_payment_id = new DbField(
            'log_test_payment',
            'log_test_payment',
            'x_log_test_payment_id',
            'log_test_payment_id',
            '`log_test_payment_id`',
            '`log_test_payment_id`',
            3,
        ...
        $this->log_test_payment_id->InputTextType = "text";
        $this->log_test_payment_id->IsAutoIncrement = true; // Autoincrement field
        $this->log_test_payment_id->IsPrimaryKey = true; // Primary key field
        $this->log_test_payment_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['log_test_payment_id'] = &$this->log_test_payment_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`log_test_payment`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->log_test_payment_id->setDbValue($conn->lastInsertId());
            $rs['log_test_payment_id'] = $this->log_test_payment_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('log_test_payment_id', $rs)) {
                AddFilter($where, QuotedName('log_test_payment_id', $this->Dbid) . '=' . QuotedValue($rs['log_test_payment_id'], $this->log_test_payment_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->log_test_payment_id->DbValue = $row['log_test_payment_id'];
    ...
    {
        return "`log_test_payment_id` = @log_test_payment_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->log_test_payment_id->CurrentValue : $this->log_test_payment_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->log_test_payment_id->CurrentValue = $keys[0];
            } else {
                $this->log_test_payment_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('log_test_payment_id', $row) ? $row['log_test_payment_id'] : null;
        } else {
            $val = $this->log_test_payment_id->OldValue !== null ? $this->log_test_payment_id->OldValue : $this->log_test_payment_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@log_test_payment_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("logtestpaymentlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "logtestpaymentedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "logtestpaymentadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "LogTestPaymentView";
            case Config("API_ADD_ACTION"):
                return "LogTestPaymentAdd";
            case Config("API_EDIT_ACTION"):
                return "LogTestPaymentEdit";
            case Config("API_DELETE_ACTION"):
                return "LogTestPaymentDelete";
            case Config("API_LIST_ACTION"):
                return "LogTestPaymentList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("logtestpaymentview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("logtestpaymentview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "logtestpaymentadd?" . $this->getUrlParm($parm);
        } else {
            $url = "logtestpaymentadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logtestpaymentedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logtestpaymentadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("logtestpaymentdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"log_test_payment_id\":" . JsonEncode($this->log_test_payment_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->log_test_payment_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->log_test_payment_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("log_test_payment_id") ?? Route("log_test_payment_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->log_test_payment_id->CurrentValue = $key;
            } else {
                $this->log_test_payment_id->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->log_test_payment_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->log_test_payment_id);
