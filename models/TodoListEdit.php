 ...
 * Page class
 */
class TodoListEdit extends TodoList
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TodoListEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("todo_list_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->todo_list_id->setQueryStringValue($keyValue);
                $this->todo_list_id->setOldValue($this->todo_list_id->QueryStringValue);
            } elseif (Post("todo_list_id") !== null) {
                $this->todo_list_id->setFormValue(Post("todo_list_id"));
                $this->todo_list_id->setOldValue($this->todo_list_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("todo_list_id") ?? Route("todo_list_id")) !== null) {
                    $this->todo_list_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("todolistlist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "todolistlist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->todo_list_id->IsDetailKey) {
    ...
    public function renderRow()
    {
        ...
        // todo_list_id
        $this->todo_list_id->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("todolistlist"), "", $this->TableVar, true);
        $pageId = "edit";
