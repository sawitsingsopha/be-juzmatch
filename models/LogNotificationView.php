 ...
 * Page class
 */
class LogNotificationView extends LogNotification
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogNotificationView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("log_notification_id") ?? Route("log_notification_id")) !== null) {
            $this->RecKey["log_notification_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("log_notification_id") ?? Route("log_notification_id")) !== null) {
                ...
                $this->RecKey["log_notification_id"] = $this->log_notification_id->QueryStringValue;
            } elseif (Post("log_notification_id") !== null) {
                $this->log_notification_id->setFormValue(Post("log_notification_id"));
                $this->RecKey["log_notification_id"] = $this->log_notification_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->log_notification_id->setQueryStringValue($keyValue);
                $this->RecKey["log_notification_id"] = $this->log_notification_id->QueryStringValue;
            } else {
                $returnUrl = "lognotificationlist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "lognotificationlist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "lognotificationlist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
