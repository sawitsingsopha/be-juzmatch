...
class BuyerList extends Buyer
{
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "buyeradd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "buyerdelete";
        $this->MultiUpdateUrl = "buyerupdate";
    ...
    public function run()
    {
        ...
        $this->isbuyer->Visible = false;
        $this->isinvertor->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fbuyersrch");
        }
        ...
        $filterList = Concat($filterList, $this->verify_phone_status->AdvancedSearch->toJson(), ","); // Field verify_phone_status
        $filterList = Concat($filterList, $this->verify_phone_date->AdvancedSearch->toJson(), ","); // Field verify_phone_date
        $filterList = Concat($filterList, $this->verify_phone_ip->AdvancedSearch->toJson(), ","); // Field verify_phone_ip
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fbuyersrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field verify_phone_status
        $this->verify_phone_status->AdvancedSearch->SearchValue = @$filter["x_verify_phone_status"];
        $this->verify_phone_status->AdvancedSearch->SearchOperator = @$filter["z_verify_phone_status"];
        $this->verify_phone_status->AdvancedSearch->SearchCondition = @$filter["v_verify_phone_status"];
        $this->verify_phone_status->AdvancedSearch->SearchValue2 = @$filter["y_verify_phone_status"];
        $this->verify_phone_status->AdvancedSearch->SearchOperator2 = @$filter["w_verify_phone_status"];
        $this->verify_phone_status->AdvancedSearch->save();

        // Field verify_phone_date
        $this->verify_phone_date->AdvancedSearch->SearchValue = @$filter["x_verify_phone_date"];
        $this->verify_phone_date->AdvancedSearch->SearchOperator = @$filter["z_verify_phone_date"];
        $this->verify_phone_date->AdvancedSearch->SearchCondition = @$filter["v_verify_phone_date"];
        $this->verify_phone_date->AdvancedSearch->SearchValue2 = @$filter["y_verify_phone_date"];
        $this->verify_phone_date->AdvancedSearch->SearchOperator2 = @$filter["w_verify_phone_date"];
        $this->verify_phone_date->AdvancedSearch->save();

        // Field verify_phone_ip
        $this->verify_phone_ip->AdvancedSearch->SearchValue = @$filter["x_verify_phone_ip"];
        $this->verify_phone_ip->AdvancedSearch->SearchOperator = @$filter["z_verify_phone_ip"];
        $this->verify_phone_ip->AdvancedSearch->SearchCondition = @$filter["v_verify_phone_ip"];
        $this->verify_phone_ip->AdvancedSearch->SearchValue2 = @$filter["y_verify_phone_ip"];
        $this->verify_phone_ip->AdvancedSearch->SearchOperator2 = @$filter["w_verify_phone_ip"];
        $this->verify_phone_ip->AdvancedSearch->save();
    }
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->last_login, $default, false); // last_login
        ...
        $this->buildSearchSql($where, $this->verify_phone_status, $default, false); // verify_phone_status
        $this->buildSearchSql($where, $this->verify_phone_date, $default, false); // verify_phone_date
        $this->buildSearchSql($where, $this->verify_phone_ip, $default, false); // verify_phone_ip
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->last_login->AdvancedSearch->save(); // last_login
            ...
            $this->verify_phone_status->AdvancedSearch->save(); // verify_phone_status
            $this->verify_phone_date->AdvancedSearch->save(); // verify_phone_date
            $this->verify_phone_ip->AdvancedSearch->save(); // verify_phone_ip
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->last_login->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->verify_phone_status->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->verify_phone_date->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->verify_phone_ip->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->last_login->AdvancedSearch->unsetSession();
        ...
        $this->verify_phone_status->AdvancedSearch->unsetSession();
        $this->verify_phone_date->AdvancedSearch->unsetSession();
        $this->verify_phone_ip->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->verify_phone_status->AdvancedSearch->load();
        $this->verify_phone_date->AdvancedSearch->load();
        $this->verify_phone_ip->AdvancedSearch->load();
    }
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_buyer_verify"
        $item = &$this->ListOptions->add("detail_buyer_verify");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'buyer_verify');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_save_search"
        $item = &$this->ListOptions->add("detail_save_search");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'save_search');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_juzcalculator"
        $item = &$this->ListOptions->add("detail_juzcalculator");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'juzcalculator');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;
        ...
        // "detail_buyer_booking_asset"
        $item = &$this->ListOptions->add("detail_buyer_booking_asset");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'buyer_booking_asset');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_buyer_asset"
        $item = &$this->ListOptions->add("detail_buyer_asset");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'buyer_asset');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("buyer_verify");
        $pages->add("save_search");
        $pages->add("juzcalculator");
        $pages->add("appointment");
        $pages->add("buyer_booking_asset");
        $pages->add("buyer_asset");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuyerlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuyerlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_buyer_verify"
        $opt = $this->ListOptions["detail_buyer_verify"];
        if ($Security->allowList(CurrentProjectID() . 'buyer_verify')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("buyer_verify", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("buyerverifylist?" . Config("TABLE_SHOW_MASTER") . "=buyer&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("BuyerVerifyGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'buyer')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=buyer_verify");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "buyer_verify";
            }
            if ($links != "") {
        ...
        // "detail_save_search"
        $opt = $this->ListOptions["detail_save_search"];
        if ($Security->allowList(CurrentProjectID() . 'save_search')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("save_search", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("savesearchlist?" . Config("TABLE_SHOW_MASTER") . "=buyer&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("SaveSearchGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'buyer')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=save_search");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "save_search";
            }
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        // "detail_juzcalculator"
        $opt = $this->ListOptions["detail_juzcalculator"];
        if ($Security->allowList(CurrentProjectID() . 'juzcalculator')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("juzcalculator", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("juzcalculatorlist?" . Config("TABLE_SHOW_MASTER") . "=buyer&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("JuzcalculatorGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'buyer')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=juzcalculator");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "juzcalculator";
            }
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        if ($Security->allowList(CurrentProjectID() . 'appointment')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("appointmentlist?" . Config("TABLE_SHOW_MASTER") . "=buyer&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        // "detail_buyer_booking_asset"
        $opt = $this->ListOptions["detail_buyer_booking_asset"];
        if ($Security->allowList(CurrentProjectID() . 'buyer_booking_asset')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("buyer_booking_asset", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("buyerbookingassetlist?" . Config("TABLE_SHOW_MASTER") . "=buyer&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("BuyerBookingAssetGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'buyer')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=buyer_booking_asset");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "buyer_booking_asset";
            }
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        // "detail_buyer_asset"
        $opt = $this->ListOptions["detail_buyer_asset"];
        if ($Security->allowList(CurrentProjectID() . 'buyer_asset')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("buyer_asset", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("buyerassetlist?" . Config("TABLE_SHOW_MASTER") . "=buyer&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("BuyerAssetGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'buyer')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=buyer_asset");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "buyer_asset";
            }
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbuyersrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbuyersrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbuyerlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->last_login->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->last_login->AdvancedSearch->SearchValue != "" || $this->last_login->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // verify_phone_status
        if ($this->verify_phone_status->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->verify_phone_status->AdvancedSearch->SearchValue != "" || $this->verify_phone_status->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // verify_phone_date
        if ($this->verify_phone_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->verify_phone_date->AdvancedSearch->SearchValue != "" || $this->verify_phone_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // verify_phone_ip
        if ($this->verify_phone_ip->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->verify_phone_ip->AdvancedSearch->SearchValue != "" || $this->verify_phone_ip->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->image_profile->UseColorbox) {
                ...
                $this->image_profile->LinkAttrs["data-rel"] = "buyer_x" . $this->RowCount . "_image_profile";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fbuyerlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fbuyersrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->first_name->Visible || $this->last_name->Visible || $this->idcardnumber->Visible || $this->_email->Visible || $this->phone->Visible;
    }
