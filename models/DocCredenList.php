 ...
 * Page class
 */
class DocCredenList extends DocCreden
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocCredenList";
    ...
    public $FormName = "fdoc_credenlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "doccredenadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "doccredendelete";
        $this->MultiUpdateUrl = "doccredenupdate";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fdoc_credensrch");
        }
        ...
        $filterList = Concat($filterList, $this->doc_temp_id->AdvancedSearch->toJson(), ","); // Field doc_temp_id
        ...
        $filterList = Concat($filterList, $this->send_email->AdvancedSearch->toJson(), ","); // Field send_email
        $filterList = Concat($filterList, $this->redirect_url->AdvancedSearch->toJson(), ","); // Field redirect_url
        ...
        $filterList = Concat($filterList, $this->doc_url->AdvancedSearch->toJson(), ","); // Field doc_url
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fdoc_credensrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field doc_temp_id
        $this->doc_temp_id->AdvancedSearch->SearchValue = @$filter["x_doc_temp_id"];
        $this->doc_temp_id->AdvancedSearch->SearchOperator = @$filter["z_doc_temp_id"];
        $this->doc_temp_id->AdvancedSearch->SearchCondition = @$filter["v_doc_temp_id"];
        $this->doc_temp_id->AdvancedSearch->SearchValue2 = @$filter["y_doc_temp_id"];
        $this->doc_temp_id->AdvancedSearch->SearchOperator2 = @$filter["w_doc_temp_id"];
        $this->doc_temp_id->AdvancedSearch->save();
        ...
        // Field send_email
        $this->send_email->AdvancedSearch->SearchValue = @$filter["x_send_email"];
        $this->send_email->AdvancedSearch->SearchOperator = @$filter["z_send_email"];
        $this->send_email->AdvancedSearch->SearchCondition = @$filter["v_send_email"];
        $this->send_email->AdvancedSearch->SearchValue2 = @$filter["y_send_email"];
        $this->send_email->AdvancedSearch->SearchOperator2 = @$filter["w_send_email"];
        $this->send_email->AdvancedSearch->save();

        // Field redirect_url
        $this->redirect_url->AdvancedSearch->SearchValue = @$filter["x_redirect_url"];
        $this->redirect_url->AdvancedSearch->SearchOperator = @$filter["z_redirect_url"];
        $this->redirect_url->AdvancedSearch->SearchCondition = @$filter["v_redirect_url"];
        $this->redirect_url->AdvancedSearch->SearchValue2 = @$filter["y_redirect_url"];
        $this->redirect_url->AdvancedSearch->SearchOperator2 = @$filter["w_redirect_url"];
        $this->redirect_url->AdvancedSearch->save();
        ...
        // Field doc_url
        $this->doc_url->AdvancedSearch->SearchValue = @$filter["x_doc_url"];
        $this->doc_url->AdvancedSearch->SearchOperator = @$filter["z_doc_url"];
        $this->doc_url->AdvancedSearch->SearchCondition = @$filter["v_doc_url"];
        $this->doc_url->AdvancedSearch->SearchValue2 = @$filter["y_doc_url"];
        $this->doc_url->AdvancedSearch->SearchOperator2 = @$filter["w_doc_url"];
        $this->doc_url->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->document_id;
        $searchFlds[] = &$this->txid;
        $searchFlds[] = &$this->subject;
        $searchFlds[] = &$this->send_email;
        $searchFlds[] = &$this->redirect_url;
        $searchFlds[] = &$this->cip;
        $searchFlds[] = &$this->uip;
        $searchFlds[] = &$this->doc_url;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->document_id, $ctrl); // document_id
            $this->updateSort($this->doc_creden_id, $ctrl); // doc_creden_id
            $this->updateSort($this->doc_temp_id, $ctrl); // doc_temp_id
            $this->updateSort($this->txid, $ctrl); // txid
            $this->updateSort($this->subject, $ctrl); // subject
            $this->updateSort($this->send_email, $ctrl); // send_email
            $this->updateSort($this->redirect_url, $ctrl); // redirect_url
            ...
            $this->updateSort($this->doc_url, $ctrl); // doc_url
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->doc_temp_id->setSort("");
                $this->txid->setSort("");
                $this->subject->setSort("");
                $this->send_email->setSort("");
                $this->redirect_url->setSort("");
                $this->status->setSort("");
                $this->cdate->setSort("");
                $this->cuser->setSort("");
                $this->cip->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdoc_credenlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdoc_credenlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->doc_creden_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("document_id", $this->createColumnOption("document_id"));
            $option->add("doc_creden_id", $this->createColumnOption("doc_creden_id"));
            $option->add("doc_temp_id", $this->createColumnOption("doc_temp_id"));
            $option->add("txid", $this->createColumnOption("txid"));
            $option->add("subject", $this->createColumnOption("subject"));
            $option->add("send_email", $this->createColumnOption("send_email"));
            $option->add("redirect_url", $this->createColumnOption("redirect_url"));
            ...
            $option->add("doc_url", $this->createColumnOption("doc_url"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdoc_credensrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdoc_credensrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdoc_credenlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fdoc_credenlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdoc_credensrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
