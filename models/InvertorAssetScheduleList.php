 ...
 * Page class
 */
class InvertorAssetScheduleList extends InvertorAssetSchedule
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'invertor_asset_schedule';

    // Page object name
    public $PageObjName = "InvertorAssetScheduleList";
    ...
    public $FormName = "finvertor_asset_schedulelist";
    ...
    public function __construct()
    {
        ...
        // Table object (invertor_asset_schedule)
        if (!isset($GLOBALS["invertor_asset_schedule"]) || get_class($GLOBALS["invertor_asset_schedule"]) == PROJECT_NAMESPACE . "invertor_asset_schedule") {
            $GLOBALS["invertor_asset_schedule"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "invertorassetscheduleadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "invertorassetscheduledelete";
        $this->MultiUpdateUrl = "invertorassetscheduleupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'invertor_asset_schedule');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("invertor_asset_schedule");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['invertor_asset_schedule_id'];
        }
        return $key;
    }
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->invertor_asset_schedule_id->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->invertor_asset_schedule_id->setVisibility();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->invertor_asset_schedule_id, $ctrl); // invertor_asset_schedule_id
            ...
            $this->updateSort($this->receive_per_installment, $ctrl); // receive_per_installment
            $this->updateSort($this->receive_per_installment_invertor, $ctrl); // receive_per_installment_invertor
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->invertor_asset_schedule_id->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finvertor_asset_schedulelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finvertor_asset_schedulelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->invertor_asset_schedule_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("invertor_asset_schedule_id", $this->createColumnOption("invertor_asset_schedule_id"));
            ...
            $option->add("receive_per_installment", $this->createColumnOption("receive_per_installment"));
            $option->add("receive_per_installment_invertor", $this->createColumnOption("receive_per_installment_invertor"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"finvertor_asset_schedulesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"finvertor_asset_schedulesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="finvertor_asset_schedulelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    {
        $row = [];
        $row['invertor_asset_schedule_id'] = null;
        $row['asset_id'] = null;
        $row['member_id'] = null;
