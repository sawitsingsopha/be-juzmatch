 ...
 * Page class
 */
class MasterFacilitiesEdit extends MasterFacilities
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterFacilitiesEdit";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "masterfacilitiesview") {
                        $row["view"] = "1";
                    }
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("master_facilities_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->master_facilities_id->setQueryStringValue($keyValue);
                $this->master_facilities_id->setOldValue($this->master_facilities_id->QueryStringValue);
            } elseif (Post("master_facilities_id") !== null) {
                $this->master_facilities_id->setFormValue(Post("master_facilities_id"));
                $this->master_facilities_id->setOldValue($this->master_facilities_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("master_facilities_id") ?? Route("master_facilities_id")) !== null) {
                    $this->master_facilities_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("masterfacilitieslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "masterfacilitieslist") {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // master_facilities_group_id
            $this->master_facilities_group_id->setDbValueDef($rsnew, $this->master_facilities_group_id->CurrentValue, 0, $this->master_facilities_group_id->ReadOnly);
            ...
            // udate
            $this->udate->CurrentValue = CurrentDate();
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("masterfacilitieslist"), "", $this->TableVar, true);
        $pageId = "edit";
