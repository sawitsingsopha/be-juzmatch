...
/**
 * Table class for buyer_all_booking_asset
 */
class BuyerAllBookingAsset extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'buyer_all_booking_asset';
        $this->TableName = 'buyer_all_booking_asset';
        ...
        $this->status_payment->Lookup = new Lookup('status_payment', 'buyer_all_booking_asset', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->date_payment = new DbField(
            ...
            'date_payment',
            '`date_payment`',
            CastDateFieldForLike("`date_payment`", 7, "juzmatch1"),
            135,
            19,
            7,
            false,
        ...
        $this->due_date = new DbField(
            ...
            'due_date',
            '`due_date`',
            CastDateFieldForLike("`due_date`", 7, "juzmatch1"),
            135,
            19,
            7,
            false,
        ...
        $this->status_expire->Lookup = new Lookup('status_expire', 'buyer_all_booking_asset', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->transaction_datetime = new DbField(
            ...
            'transaction_datetime',
            '`transaction_datetime`',
            CastDateFieldForLike("`transaction_datetime`", 0, "juzmatch1"),
            135,
            19,
            0,
            false,
        ...
        // authorize_url
        $this->authorize_url = new DbField(
            'buyer_all_booking_asset',
            'buyer_all_booking_asset',
            'x_authorize_url',
            'authorize_url',
        ...
        $this->authorize_url->InputTextType = "text";
        $this->authorize_url->Sortable = false; // Allow sort
        $this->Fields['authorize_url'] = &$this->authorize_url;

        // amount_net
        $this->amount_net = new DbField(
            'buyer_all_booking_asset',
            'buyer_all_booking_asset',
            'x_amount_net',
            'amount_net',
        ...
        $this->amount_net->InputTextType = "text";
        $this->amount_net->Sortable = false; // Allow sort
        $this->amount_net->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->Fields['amount_net'] = &$this->amount_net;

        // amount_cust_fee
        $this->amount_cust_fee = new DbField(
            'buyer_all_booking_asset',
            'buyer_all_booking_asset',
            'x_amount_cust_fee',
            'amount_cust_fee',
        ...
        $this->amount_cust_fee->InputTextType = "text";
        $this->amount_cust_fee->Sortable = false; // Allow sort
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "buyer_all_asset_rent") {
            $detailUrl = Container("buyer_all_asset_rent")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "all_buyer_config_asset_schedule") {
            $detailUrl = Container("all_buyer_config_asset_schedule")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "doc_juzmatch1") {
            $detailUrl = Container("doc_juzmatch1")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_buyer_booking_asset_id", $this->buyer_booking_asset_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "doc_juzmatch3") {
            $detailUrl = Container("doc_juzmatch3")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_buyer_booking_asset_id", $this->buyer_booking_asset_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "buyerallbookingassetlist";
        }
        return $detailUrl;
    }
    ...
    {
        // Cascade Update detail table 'buyer_all_asset_rent'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("buyer_all_asset_rent")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'juzmatch1'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("buyer_all_asset_rent")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("buyer_all_asset_rent")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("buyer_all_asset_rent")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'all_buyer_config_asset_schedule'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['asset_id']) && $rsold['asset_id'] != $rs['asset_id'])) { // Update detail field 'asset_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("all_buyer_config_asset_schedule")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'juzmatch1'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                ...
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("all_buyer_config_asset_schedule")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("all_buyer_config_asset_schedule")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("all_buyer_config_asset_schedule")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'doc_juzmatch1'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['buyer_booking_asset_id']) && $rsold['buyer_booking_asset_id'] != $rs['buyer_booking_asset_id'])) { // Update detail field 'buyer_booking_asset_id'
            $cascadeUpdate = true;
            $rscascade['buyer_booking_asset_id'] = $rs['buyer_booking_asset_id'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("doc_juzmatch1")->loadRs("`buyer_booking_asset_id` = " . QuotedValue($rsold['buyer_booking_asset_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("doc_juzmatch1")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("doc_juzmatch1")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("doc_juzmatch1")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'doc_juzmatch3'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['buyer_booking_asset_id']) && $rsold['buyer_booking_asset_id'] != $rs['buyer_booking_asset_id'])) { // Update detail field 'buyer_booking_asset_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("doc_juzmatch3")->loadRs("`buyer_booking_asset_id` = " . QuotedValue($rsold['buyer_booking_asset_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("doc_juzmatch3")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("doc_juzmatch3")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("doc_juzmatch3")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'buyer_all_asset_rent'
        $dtlrows = Container("buyer_all_asset_rent")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "juzmatch1"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("buyer_all_asset_rent")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("buyer_all_asset_rent")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("buyer_all_asset_rent")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'all_buyer_config_asset_schedule'
        $dtlrows = Container("all_buyer_config_asset_schedule")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "juzmatch1"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("all_buyer_config_asset_schedule")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("all_buyer_config_asset_schedule")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("all_buyer_config_asset_schedule")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'doc_juzmatch1'
        $dtlrows = Container("doc_juzmatch1")->loadRs("`buyer_booking_asset_id` = " . QuotedValue($rs['buyer_booking_asset_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("doc_juzmatch1")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("doc_juzmatch1")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("doc_juzmatch1")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'doc_juzmatch3'
        $dtlrows = Container("doc_juzmatch3")->loadRs("`buyer_booking_asset_id` = " . QuotedValue($rs['buyer_booking_asset_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("doc_juzmatch3")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("doc_juzmatch3")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("doc_juzmatch3")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("buyerallbookingassetlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "buyerallbookingassetedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "buyerallbookingassetadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "BuyerAllBookingAssetView";
            case Config("API_ADD_ACTION"):
                return "BuyerAllBookingAssetAdd";
            case Config("API_EDIT_ACTION"):
                return "BuyerAllBookingAssetEdit";
            case Config("API_DELETE_ACTION"):
                return "BuyerAllBookingAssetDelete";
            case Config("API_LIST_ACTION"):
                return "BuyerAllBookingAssetList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("buyerallbookingassetview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("buyerallbookingassetview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "buyerallbookingassetadd?" . $this->getUrlParm($parm);
        } else {
            $url = "buyerallbookingassetadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("buyerallbookingassetedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("buyerallbookingassetedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("buyerallbookingassetadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("buyerallbookingassetadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("buyerallbookingassetdelete", $this->getUrlParm());
    }
    ...
    public function renderListRow()
    {
        ...
        // authorize_url
        $this->authorize_url->CellCssStyle = "white-space: nowrap;";

        // amount_net
        $this->amount_net->CellCssStyle = "white-space: nowrap;";
        ...
        // authorize_url
        $this->authorize_url->ViewValue = $this->authorize_url->CurrentValue;
        $this->authorize_url->ViewCustomAttributes = "";

        // amount_net
        $this->amount_net->ViewValue = $this->amount_net->CurrentValue;
        $this->amount_net->ViewValue = FormatNumber($this->amount_net->ViewValue, $this->amount_net->formatPattern());
        $this->amount_net->ViewCustomAttributes = "";

        // amount_cust_fee
        $this->amount_cust_fee->ViewValue = $this->amount_cust_fee->CurrentValue;
        ...
        // authorize_url
        $this->authorize_url->LinkCustomAttributes = "";
        $this->authorize_url->HrefValue = "";
        $this->authorize_url->TooltipValue = "";

        // amount_net
        $this->amount_net->LinkCustomAttributes = "";
        $this->amount_net->HrefValue = "";
        $this->amount_net->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // authorize_url
        $this->authorize_url->setupEditAttributes();
        $this->authorize_url->EditCustomAttributes = "";
        if (!$this->authorize_url->Raw) {
            $this->authorize_url->CurrentValue = HtmlDecode($this->authorize_url->CurrentValue);
        }
        $this->authorize_url->EditValue = $this->authorize_url->CurrentValue;
        $this->authorize_url->PlaceHolder = RemoveHtml($this->authorize_url->caption());

        // amount_net
        $this->amount_net->setupEditAttributes();
        $this->amount_net->EditCustomAttributes = "";
        $this->amount_net->EditValue = $this->amount_net->CurrentValue;
        $this->amount_net->PlaceHolder = RemoveHtml($this->amount_net->caption());
        if (strval($this->amount_net->EditValue) != "" && is_numeric($this->amount_net->EditValue)) {
            $this->amount_net->EditValue = FormatNumber($this->amount_net->EditValue, null);
        }

        // amount_cust_fee
        $this->amount_cust_fee->setupEditAttributes();
        $this->amount_cust_fee->EditCustomAttributes = "";
        $this->amount_cust_fee->EditValue = $this->amount_cust_fee->CurrentValue;
