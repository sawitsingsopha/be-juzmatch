 ...
 * Page class
 */
class PeakExpenseItemList extends PeakExpenseItem
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PeakExpenseItemList";
    ...
    public $FormName = "fpeak_expense_itemlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "peakexpenseitemadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "peakexpenseitemdelete";
        $this->MultiUpdateUrl = "peakexpenseitemupdate";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fpeak_expense_itemsrch");
        }
        $filterList = Concat($filterList, $this->peak_expense_item_id->AdvancedSearch->toJson(), ","); // Field peak_expense_item_id
        ...
        $filterList = Concat($filterList, $this->isdelete->AdvancedSearch->toJson(), ","); // Field isdelete
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fpeak_expense_itemsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field peak_expense_item_id
        $this->peak_expense_item_id->AdvancedSearch->SearchValue = @$filter["x_peak_expense_item_id"];
        $this->peak_expense_item_id->AdvancedSearch->SearchOperator = @$filter["z_peak_expense_item_id"];
        $this->peak_expense_item_id->AdvancedSearch->SearchCondition = @$filter["v_peak_expense_item_id"];
        $this->peak_expense_item_id->AdvancedSearch->SearchValue2 = @$filter["y_peak_expense_item_id"];
        $this->peak_expense_item_id->AdvancedSearch->SearchOperator2 = @$filter["w_peak_expense_item_id"];
        $this->peak_expense_item_id->AdvancedSearch->save();
        ...
        // Field isdelete
        $this->isdelete->AdvancedSearch->SearchValue = @$filter["x_isdelete"];
        $this->isdelete->AdvancedSearch->SearchOperator = @$filter["z_isdelete"];
        $this->isdelete->AdvancedSearch->SearchCondition = @$filter["v_isdelete"];
        $this->isdelete->AdvancedSearch->SearchValue2 = @$filter["y_isdelete"];
        $this->isdelete->AdvancedSearch->SearchOperator2 = @$filter["w_isdelete"];
        $this->isdelete->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->peak_expense_item_id, $ctrl); // peak_expense_item_id
            ...
            $this->updateSort($this->isdelete, $ctrl); // isdelete
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->peak_expense_item_id->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fpeak_expense_itemlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fpeak_expense_itemlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->peak_expense_item_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("peak_expense_item_id", $this->createColumnOption("peak_expense_item_id"));
            ...
            $option->add("isdelete", $this->createColumnOption("isdelete"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpeak_expense_itemsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpeak_expense_itemsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpeak_expense_itemlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fpeak_expense_itemlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpeak_expense_itemsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
