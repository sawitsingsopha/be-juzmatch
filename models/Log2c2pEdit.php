 ...
 * Page class
 */
class Log2c2pEdit extends Log2c2p
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Log2c2pEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("log_2c2p_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->log_2c2p_id->setQueryStringValue($keyValue);
                $this->log_2c2p_id->setOldValue($this->log_2c2p_id->QueryStringValue);
            } elseif (Post("log_2c2p_id") !== null) {
                $this->log_2c2p_id->setFormValue(Post("log_2c2p_id"));
                $this->log_2c2p_id->setOldValue($this->log_2c2p_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("log_2c2p_id") ?? Route("log_2c2p_id")) !== null) {
                    $this->log_2c2p_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->log_2c2p_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("log2c2plist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "log2c2plist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'log_2c2p_id' first before field var 'x_log_2c2p_id'
        $val = $CurrentForm->hasValue("log_2c2p_id") ? $CurrentForm->getValue("log_2c2p_id") : $CurrentForm->getValue("x_log_2c2p_id");
        if (!$this->log_2c2p_id->IsDetailKey) {
            $this->log_2c2p_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->log_2c2p_id->CurrentValue = $this->log_2c2p_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // log_2c2p_id
        $this->log_2c2p_id->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->log_2c2p_id->Required) {
            if (!$this->log_2c2p_id->IsDetailKey && EmptyValue($this->log_2c2p_id->FormValue)) {
                $this->log_2c2p_id->addErrorMessage(str_replace("%s", $this->log_2c2p_id->caption(), $this->log_2c2p_id->RequiredErrorMessage));
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("log2c2plist"), "", $this->TableVar, true);
        $pageId = "edit";
