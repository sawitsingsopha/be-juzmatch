...
/**
 * Table class for asset_config_schedule
 */
class AssetConfigSchedule extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'asset_config_schedule';
        $this->TableName = 'asset_config_schedule';
        ...
        $this->status_approve->Lookup = new Lookup('status_approve', 'asset_config_schedule', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    public function getMasterFilterFromSession()
    {
        // Master filter
        $masterFilter = "";
        if ($this->getCurrentMasterTable() == "inverter_asset") {
            ...
            if ($this->asset_id->getSessionValue() != "") {
                $masterFilter .= " AND " . GetForeignKeySql("`asset_id`", $this->asset_id->getSessionValue(), DATATYPE_NUMBER, "juzmatch1");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "inverter_asset") {
            ...
            if ($this->asset_id->getSessionValue() != "") {
                $detailFilter .= " AND " . GetForeignKeySql("`asset_id`", $this->asset_id->getSessionValue(), DATATYPE_NUMBER, "juzmatch1");
            } else {
                return "";
            }
        }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("assetconfigschedulelist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "assetconfigscheduleedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "assetconfigscheduleadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "AssetConfigScheduleView";
            case Config("API_ADD_ACTION"):
                return "AssetConfigScheduleAdd";
            case Config("API_EDIT_ACTION"):
                return "AssetConfigScheduleEdit";
            case Config("API_DELETE_ACTION"):
                return "AssetConfigScheduleDelete";
            case Config("API_LIST_ACTION"):
                return "AssetConfigScheduleList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("assetconfigscheduleview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("assetconfigscheduleview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "assetconfigscheduleadd?" . $this->getUrlParm($parm);
        } else {
            $url = "assetconfigscheduleadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetconfigscheduleedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetconfigscheduleadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("assetconfigscheduledelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "inverter_asset" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        $key .= $rs['asset_config_schedule_id'];
    ...
    public function writeAuditTrailOnEdit(&$rsold, &$rsnew)
    {
        ...
        $key .= $rsold['asset_config_schedule_id'];
