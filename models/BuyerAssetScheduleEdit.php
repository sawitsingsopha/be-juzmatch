 ...
 * Page class
 */
class BuyerAssetScheduleEdit extends BuyerAssetSchedule
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BuyerAssetScheduleEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("buyer_asset_schedule_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->buyer_asset_schedule_id->setQueryStringValue($keyValue);
                $this->buyer_asset_schedule_id->setOldValue($this->buyer_asset_schedule_id->QueryStringValue);
            } elseif (Post("buyer_asset_schedule_id") !== null) {
                $this->buyer_asset_schedule_id->setFormValue(Post("buyer_asset_schedule_id"));
                $this->buyer_asset_schedule_id->setOldValue($this->buyer_asset_schedule_id->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("buyerassetschedulelist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "buyerassetschedulelist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->buyer_asset_schedule_id->IsDetailKey) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // Check referential integrity for master table 'buyer_config_asset_schedule'
            $detailKeys = [];
            $keyValue = $rsnew['member_id'] ?? $rsold['member_id'];
            $detailKeys['member_id'] = $keyValue;
            $keyValue = $rsnew['asset_id'] ?? $rsold['asset_id'];
            $detailKeys['asset_id'] = $keyValue;
            $masterTable = Container("buyer_config_asset_schedule");
            $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
            if (!EmptyValue($masterFilter)) {
                $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
                $validMasterRecord = $rsmaster !== false;
            ...
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "buyer_config_asset_schedule", $Language->phrase("RelatedRecordRequired"));
                $this->setFailureMessage($relatedRecordMsg);
                return false;
            }
