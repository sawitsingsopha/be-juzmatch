 ...
 * Page class
 */
class MasterFacilitiesGroupAdd extends MasterFacilitiesGroup
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'master_facilities_group';

    // Page object name
    public $PageObjName = "MasterFacilitiesGroupAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (master_facilities_group)
        if (!isset($GLOBALS["master_facilities_group"]) || get_class($GLOBALS["master_facilities_group"]) == PROJECT_NAMESPACE . "master_facilities_group") {
            $GLOBALS["master_facilities_group"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'master_facilities_group');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("master_facilities_group");
                $doc = new $class($tbl);
                $doc->Text = @$content;
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "masterfacilitiesgroupview") {
                        $row["view"] = "1";
                    }
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['master_facilities_group_id'];
        }
        return $key;
    }
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("master_facilities_group_id") ?? Route("master_facilities_group_id")) !== null) {
                $this->master_facilities_group_id->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("masterfacilitiesgrouplist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "masterfacilitiesgrouplist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "masterfacilitiesgroupview") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            // master_facilities_group_id
            $arwrk = [];
            $arwrk["lf"] = $this->_title->CurrentValue;
            $arwrk["df"] = $this->_title->CurrentValue;
            $arwrk = $this->master_facilities_group_id->Lookup->renderViewRow($arwrk, $this);
            $dispVal = $this->master_facilities_group_id->displayValue($arwrk);
            if ($dispVal != "") {
                $this->master_facilities_group_id->ViewValue = $dispVal;
            }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("masterfacilitiesgrouplist"), "", $this->TableVar, true);
