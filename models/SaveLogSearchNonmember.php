...
/**
 * Table class for save_log_search_nonmember
 */
class SaveLogSearchNonmember extends DbTable
{
    protected $SqlFrom = "";
    ...
    // Fields
    public $save_log_search_nonmember_id;
    public $category_id;
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'save_log_search_nonmember';
        $this->TableName = 'save_log_search_nonmember';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`save_log_search_nonmember`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->save_log_search_nonmember_id = new DbField(
            'save_log_search_nonmember',
            'save_log_search_nonmember',
            'x_save_log_search_nonmember_id',
            'save_log_search_nonmember_id',
            '`save_log_search_nonmember_id`',
            '`save_log_search_nonmember_id`',
            3,
        ...
        $this->save_log_search_nonmember_id->InputTextType = "text";
        $this->save_log_search_nonmember_id->IsAutoIncrement = true; // Autoincrement field
        $this->save_log_search_nonmember_id->IsPrimaryKey = true; // Primary key field
        $this->save_log_search_nonmember_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['save_log_search_nonmember_id'] = &$this->save_log_search_nonmember_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`save_log_search_nonmember`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->save_log_search_nonmember_id->setDbValue($conn->lastInsertId());
            $rs['save_log_search_nonmember_id'] = $this->save_log_search_nonmember_id->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success && $this->AuditTrailOnEdit && $rsold) {
            $rsaudit = $rs;
            $fldname = 'save_log_search_nonmember_id';
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('save_log_search_nonmember_id', $rs)) {
                AddFilter($where, QuotedName('save_log_search_nonmember_id', $this->Dbid) . '=' . QuotedValue($rs['save_log_search_nonmember_id'], $this->save_log_search_nonmember_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->save_log_search_nonmember_id->DbValue = $row['save_log_search_nonmember_id'];
    ...
    {
        return "`save_log_search_nonmember_id` = @save_log_search_nonmember_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->save_log_search_nonmember_id->CurrentValue : $this->save_log_search_nonmember_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->save_log_search_nonmember_id->CurrentValue = $keys[0];
            } else {
                $this->save_log_search_nonmember_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('save_log_search_nonmember_id', $row) ? $row['save_log_search_nonmember_id'] : null;
        } else {
            $val = $this->save_log_search_nonmember_id->OldValue !== null ? $this->save_log_search_nonmember_id->OldValue : $this->save_log_search_nonmember_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@save_log_search_nonmember_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("savelogsearchnonmemberlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "savelogsearchnonmemberedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "savelogsearchnonmemberadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "SaveLogSearchNonmemberView";
            case Config("API_ADD_ACTION"):
                return "SaveLogSearchNonmemberAdd";
            case Config("API_EDIT_ACTION"):
                return "SaveLogSearchNonmemberEdit";
            case Config("API_DELETE_ACTION"):
                return "SaveLogSearchNonmemberDelete";
            case Config("API_LIST_ACTION"):
                return "SaveLogSearchNonmemberList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("savelogsearchnonmemberview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("savelogsearchnonmemberview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "savelogsearchnonmemberadd?" . $this->getUrlParm($parm);
        } else {
            $url = "savelogsearchnonmemberadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("savelogsearchnonmemberedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("savelogsearchnonmemberadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("savelogsearchnonmemberdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"save_log_search_nonmember_id\":" . JsonEncode($this->save_log_search_nonmember_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->save_log_search_nonmember_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->save_log_search_nonmember_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("save_log_search_nonmember_id") ?? Route("save_log_search_nonmember_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->save_log_search_nonmember_id->CurrentValue = $key;
            } else {
                $this->save_log_search_nonmember_id->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->save_log_search_nonmember_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->save_log_search_nonmember_id);
    ...
    {
        $table = 'save_log_search_nonmember';
        $usr = CurrentUserID();
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        $key .= $rs['save_log_search_nonmember_id'];
    ...
    public function writeAuditTrailOnEdit(&$rsold, &$rsnew)
    {
        ...
        $key .= $rsold['save_log_search_nonmember_id'];
