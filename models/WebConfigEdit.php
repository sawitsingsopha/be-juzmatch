 ...
 * Page class
 */
class WebConfigEdit extends WebConfig
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "WebConfigEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("web_config_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->web_config_id->setQueryStringValue($keyValue);
                $this->web_config_id->setOldValue($this->web_config_id->QueryStringValue);
            } elseif (Post("web_config_id") !== null) {
                $this->web_config_id->setFormValue(Post("web_config_id"));
                $this->web_config_id->setOldValue($this->web_config_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("web_config_id") ?? Route("web_config_id")) !== null) {
                    $this->web_config_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->web_config_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("webconfiglist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "webconfiglist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'web_config_id' first before field var 'x_web_config_id'
        $val = $CurrentForm->hasValue("web_config_id") ? $CurrentForm->getValue("web_config_id") : $CurrentForm->getValue("x_web_config_id");
        if (!$this->web_config_id->IsDetailKey) {
            $this->web_config_id->setFormValue($val);
        }
    }
    ...
    {
        global $CurrentForm;
        $this->web_config_id->CurrentValue = $this->web_config_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // web_config_id
        $this->web_config_id->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("webconfiglist"), "", $this->TableVar, true);
        $pageId = "edit";
