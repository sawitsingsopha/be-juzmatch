 ...
 * Page class
 */
class BuyerAssetReadyBuyGrid extends BuyerAssetReadyBuy
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'buyer_asset_ready_buy';

    // Page object name
    public $PageObjName = "BuyerAssetReadyBuyGrid";
    ...
    public $FormName = "fbuyer_asset_ready_buygrid";
    ...
    public function __construct()
    {
        ...
        // Table object (buyer_asset_ready_buy)
        if (!isset($GLOBALS["buyer_asset_ready_buy"]) || get_class($GLOBALS["buyer_asset_ready_buy"]) == PROJECT_NAMESPACE . "buyer_asset_ready_buy") {
            $GLOBALS["buyer_asset_ready_buy"] = &$this;
        }
        $this->AddUrl = "buyerassetreadybuyadd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'buyer_asset_ready_buy');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("buyer_asset_ready_buy");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['buyer_asset_ready_buy_id'];
        }
        return $key;
    }
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->buyer_asset_ready_buy_id->Visible = false;
        }
        if ($this->isAddOrEdit()) {
    ...
    public function run()
    {
        ...
        $this->price_payment->setVisibility();
    ...
    {
        $this->price_payment->FormValue = ""; // Clear form value
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->buyer_asset_ready_buy_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_price_payment") && $CurrentForm->hasValue("o_price_payment") && $this->price_payment->CurrentValue != $this->price_payment->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_duedate") && $CurrentForm->hasValue("o_duedate") && $this->duedate->CurrentValue != $this->duedate->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->buyer_asset_ready_buy_id->CurrentValue = null;
        $this->buyer_asset_ready_buy_id->OldValue = $this->buyer_asset_ready_buy_id->CurrentValue;
        ...
        $this->price_payment->CurrentValue = null;
        $this->price_payment->OldValue = $this->price_payment->CurrentValue;
        $this->pay_number->CurrentValue = null;
        $this->pay_number->OldValue = $this->pay_number->CurrentValue;
        $this->status_payment->CurrentValue = null;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'price_payment' first before field var 'x_price_payment'
        $val = $CurrentForm->hasValue("price_payment") ? $CurrentForm->getValue("price_payment") : $CurrentForm->getValue("x_price_payment");
        if (!$this->price_payment->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->price_payment->Visible = false; // Disable update for API request
            } else {
                $this->price_payment->setFormValue($val, true, $validate);
            }
        }
        if ($CurrentForm->hasValue("o_price_payment")) {
            $this->price_payment->setOldValue($CurrentForm->getValue("o_price_payment"));
        }
        ...
        // Check field name 'due date' first before field var 'x_duedate'
        $val = $CurrentForm->hasValue("due date") ? $CurrentForm->getValue("due date") : $CurrentForm->getValue("x_duedate");
        ...
        if ($CurrentForm->hasValue("o_duedate")) {
            $this->duedate->setOldValue($CurrentForm->getValue("o_duedate"));
        }
        ...
        // Check field name 'buyer_asset_ready_buy_id' first before field var 'x_buyer_asset_ready_buy_id'
        $val = $CurrentForm->hasValue("buyer_asset_ready_buy_id") ? $CurrentForm->getValue("buyer_asset_ready_buy_id") : $CurrentForm->getValue("x_buyer_asset_ready_buy_id");
        if (!$this->buyer_asset_ready_buy_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->buyer_asset_ready_buy_id->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->buyer_asset_ready_buy_id->CurrentValue = $this->buyer_asset_ready_buy_id->FormValue;
        }
        $this->price_payment->CurrentValue = $this->price_payment->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['buyer_asset_ready_buy_id'] = $this->buyer_asset_ready_buy_id->CurrentValue;
        $row['asset_id'] = $this->asset_id->CurrentValue;
        $row['member_id'] = $this->member_id->CurrentValue;
        $row['price_payment'] = $this->price_payment->CurrentValue;
        ...
        $row['due date'] = $this->duedate->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->price_payment->EditValue = HtmlEncode($this->price_payment->CurrentValue);
            ...
            if (strval($this->price_payment->EditValue) != "" && is_numeric($this->price_payment->EditValue)) {
                ...
                $this->price_payment->OldValue = $this->price_payment->EditValue;
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->price_payment->Required) {
            if (!$this->price_payment->IsDetailKey && EmptyValue($this->price_payment->FormValue)) {
                $this->price_payment->addErrorMessage(str_replace("%s", $this->price_payment->caption(), $this->price_payment->RequiredErrorMessage));
            }
        }
        if (!CheckNumber($this->price_payment->FormValue)) {
            $this->price_payment->addErrorMessage($this->price_payment->getErrorMessage(false));
        }
    ...
    protected function deleteRows()
    {
        ...
        foreach ($rsold as $row) {
            ...
            $thisKey .= $row['buyer_asset_ready_buy_id'];
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // price_payment
            $this->price_payment->setDbValueDef($rsnew, $this->price_payment->CurrentValue, null, $this->price_payment->ReadOnly);
            ...
            // Check referential integrity for master table 'buyer_asset'
            $detailKeys = [];
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'buyer_asset_ready_buy'
        ...
        // price_payment
        $this->price_payment->setDbValueDef($rsnew, $this->price_payment->CurrentValue, null, false);

        // pay_number
        $this->pay_number->setDbValueDef($rsnew, $this->pay_number->CurrentValue, null, false);

        // status_payment
        $this->status_payment->setDbValueDef($rsnew, $this->status_payment->CurrentValue, null, false);
