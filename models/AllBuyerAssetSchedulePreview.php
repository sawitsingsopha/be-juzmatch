 ...
 * Page class
 */
class AllBuyerAssetSchedulePreview extends AllBuyerAssetSchedule
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AllBuyerAssetSchedulePreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "all_buyer_config_asset_schedule") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=all_buyer_config_asset_schedule&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->QueryStringValue) . "";
        }
        if ($masterTblVar == "number_deals_available") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=number_deals_available&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->QueryStringValue) . "";
        }
        if ($masterTblVar == "number_of_accrued") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=number_of_accrued&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->QueryStringValue) . "";
        }
        if ($masterTblVar == "number_of_unpaid_units") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=number_of_unpaid_units&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->QueryStringValue) . "";
        }
        if ($masterTblVar == "outstanding_amount") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=outstanding_amount&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeysFromFilter($f)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "all_buyer_config_asset_schedule") {
            $find = "`buyer_config_asset_schedule_id`=";
            $x = strpos($f, $find);
            if ($x !== false) {
                $x += strlen($find);
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "juzmatch1");
                $this->buyer_config_asset_schedule_id->setQueryStringValue($val);
            }
        }
        if ($masterTblVar == "number_deals_available") {
            $find = "`buyer_config_asset_schedule_id`=";
