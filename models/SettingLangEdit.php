 ...
 * Page class
 */
class SettingLangEdit extends SettingLang
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SettingLangEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("setting_lang_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->setting_lang_id->setQueryStringValue($keyValue);
                $this->setting_lang_id->setOldValue($this->setting_lang_id->QueryStringValue);
            } elseif (Post("setting_lang_id") !== null) {
                $this->setting_lang_id->setFormValue(Post("setting_lang_id"));
                $this->setting_lang_id->setOldValue($this->setting_lang_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->setting_lang_id->IsDetailKey) {
            $this->setting_lang_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->setting_lang_id->CurrentValue = $this->setting_lang_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->setting_lang_id->Required) {
            if (!$this->setting_lang_id->IsDetailKey && EmptyValue($this->setting_lang_id->FormValue)) {
                $this->setting_lang_id->addErrorMessage(str_replace("%s", $this->setting_lang_id->caption(), $this->setting_lang_id->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // lang_en
            $this->lang_en->setDbValueDef($rsnew, $this->lang_en->CurrentValue, "", $this->lang_en->ReadOnly);

            // lang
            $this->lang->setDbValueDef($rsnew, $this->lang->CurrentValue, "", $this->lang->ReadOnly);

            // param
            $this->_param->setDbValueDef($rsnew, $this->_param->CurrentValue, "", $this->_param->ReadOnly);

            // cip
            $this->cip->setDbValueDef($rsnew, $this->cip->CurrentValue, "", $this->cip->ReadOnly);

            // udate
            $this->udate->setDbValueDef($rsnew, UnFormatDateTime($this->udate->CurrentValue, $this->udate->formatPattern()), CurrentDate(), $this->udate->ReadOnly);

            // uip
            $this->uip->setDbValueDef($rsnew, $this->uip->CurrentValue, "", $this->uip->ReadOnly);

            // uuser
            $this->uuser->setDbValueDef($rsnew, $this->uuser->CurrentValue, "", $this->uuser->ReadOnly);
