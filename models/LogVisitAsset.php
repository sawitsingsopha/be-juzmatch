...
/**
 * Table class for log_visit_asset
 */
class LogVisitAsset extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'log_visit_asset';
        $this->TableName = 'log_visit_asset';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`log_visit_asset`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // log_visit_asset_id
        $this->log_visit_asset_id = new DbField(
            'log_visit_asset',
            'log_visit_asset',
            'x_log_visit_asset_id',
            'log_visit_asset_id',
            '`log_visit_asset_id`',
            '`log_visit_asset_id`',
            3,
        ...
        $this->log_visit_asset_id->InputTextType = "text";
        $this->log_visit_asset_id->IsAutoIncrement = true; // Autoincrement field
        $this->log_visit_asset_id->IsPrimaryKey = true; // Primary key field
        $this->log_visit_asset_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['log_visit_asset_id'] = &$this->log_visit_asset_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`log_visit_asset`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->log_visit_asset_id->setDbValue($conn->lastInsertId());
            $rs['log_visit_asset_id'] = $this->log_visit_asset_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('log_visit_asset_id', $rs)) {
                AddFilter($where, QuotedName('log_visit_asset_id', $this->Dbid) . '=' . QuotedValue($rs['log_visit_asset_id'], $this->log_visit_asset_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->log_visit_asset_id->DbValue = $row['log_visit_asset_id'];
    ...
    {
        return "`log_visit_asset_id` = @log_visit_asset_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->log_visit_asset_id->CurrentValue : $this->log_visit_asset_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->log_visit_asset_id->CurrentValue = $keys[0];
            } else {
                $this->log_visit_asset_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('log_visit_asset_id', $row) ? $row['log_visit_asset_id'] : null;
        } else {
            $val = $this->log_visit_asset_id->OldValue !== null ? $this->log_visit_asset_id->OldValue : $this->log_visit_asset_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@log_visit_asset_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("logvisitassetlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "logvisitassetview") {
            return $Language->phrase("View");
        } elseif ($pageName == "logvisitassetedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "logvisitassetadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "LogVisitAssetView";
            case Config("API_ADD_ACTION"):
                return "LogVisitAssetAdd";
            case Config("API_EDIT_ACTION"):
                return "LogVisitAssetEdit";
            case Config("API_DELETE_ACTION"):
                return "LogVisitAssetDelete";
            case Config("API_LIST_ACTION"):
                return "LogVisitAssetList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("logvisitassetview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("logvisitassetview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "logvisitassetadd?" . $this->getUrlParm($parm);
        } else {
            $url = "logvisitassetadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logvisitassetedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logvisitassetadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("logvisitassetdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"log_visit_asset_id\":" . JsonEncode($this->log_visit_asset_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->log_visit_asset_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->log_visit_asset_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("log_visit_asset_id") ?? Route("log_visit_asset_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->log_visit_asset_id->CurrentValue = $key;
            } else {
                $this->log_visit_asset_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->log_visit_asset_id->setDbValue($row['log_visit_asset_id']);
    ...
    public function renderListRow()
    {
        ...
        // log_visit_asset_id
        $this->log_visit_asset_id->ViewValue = $this->log_visit_asset_id->CurrentValue;
        $this->log_visit_asset_id->ViewCustomAttributes = "";
        ...
        // log_visit_asset_id
        $this->log_visit_asset_id->LinkCustomAttributes = "";
        $this->log_visit_asset_id->HrefValue = "";
        $this->log_visit_asset_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // log_visit_asset_id
        $this->log_visit_asset_id->setupEditAttributes();
        $this->log_visit_asset_id->EditCustomAttributes = "";
        $this->log_visit_asset_id->EditValue = $this->log_visit_asset_id->CurrentValue;
        $this->log_visit_asset_id->ViewCustomAttributes = "";
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->log_visit_asset_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->log_visit_asset_id);
