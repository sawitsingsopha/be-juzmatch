...
/**
 * Table class for asset_banner
 */
class AssetBanner extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'asset_banner';
        $this->TableName = 'asset_banner';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`asset_banner`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // asset_banner_id
        $this->asset_banner_id = new DbField(
            'asset_banner',
            'asset_banner',
            'x_asset_banner_id',
            'asset_banner_id',
        ...
        $this->asset_banner_id->InputTextType = "text";
        $this->asset_banner_id->IsAutoIncrement = true; // Autoincrement field
        $this->asset_banner_id->IsPrimaryKey = true; // Primary key field
        $this->asset_banner_id->Sortable = false; // Allow sort
        $this->asset_banner_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['asset_banner_id'] = &$this->asset_banner_id;
        ...
        $this->image->UploadPath = './upload/asset_banner';
        ...
        $this->isactive->Lookup = new Lookup('isactive', 'asset_banner', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`asset_banner`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->asset_banner_id->setDbValue($conn->lastInsertId());
            $rs['asset_banner_id'] = $this->asset_banner_id->DbValue;
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('asset_banner_id', $rs)) {
                AddFilter($where, QuotedName('asset_banner_id', $this->Dbid) . '=' . QuotedValue($rs['asset_banner_id'], $this->asset_banner_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->asset_banner_id->DbValue = $row['asset_banner_id'];
    ...
    public function deleteUploadedFiles($row)
    {
        ...
        $this->image->OldUploadPath = './upload/asset_banner';
    ...
    {
        return "`asset_banner_id` = @asset_banner_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->asset_banner_id->CurrentValue : $this->asset_banner_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->asset_banner_id->CurrentValue = $keys[0];
            } else {
                $this->asset_banner_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('asset_banner_id', $row) ? $row['asset_banner_id'] : null;
        } else {
            $val = $this->asset_banner_id->OldValue !== null ? $this->asset_banner_id->OldValue : $this->asset_banner_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@asset_banner_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("assetbannerlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "assetbannerview") {
            return $Language->phrase("View");
        } elseif ($pageName == "assetbanneredit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "assetbanneradd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "AssetBannerView";
            case Config("API_ADD_ACTION"):
                return "AssetBannerAdd";
            case Config("API_EDIT_ACTION"):
                return "AssetBannerEdit";
            case Config("API_DELETE_ACTION"):
                return "AssetBannerDelete";
            case Config("API_LIST_ACTION"):
                return "AssetBannerList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("assetbannerview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("assetbannerview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "assetbanneradd?" . $this->getUrlParm($parm);
        } else {
            $url = "assetbanneradd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("assetbanneredit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetbanneradd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("assetbannerdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"asset_banner_id\":" . JsonEncode($this->asset_banner_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->asset_banner_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->asset_banner_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("asset_banner_id") ?? Route("asset_banner_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->asset_banner_id->CurrentValue = $key;
            } else {
                $this->asset_banner_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->asset_banner_id->setDbValue($row['asset_banner_id']);
    ...
    public function renderListRow()
    {
        ...
        // asset_banner_id
        $this->asset_banner_id->CellCssStyle = "white-space: nowrap;";
        ...
        // asset_banner_id
        $this->asset_banner_id->ViewValue = $this->asset_banner_id->CurrentValue;
        $this->asset_banner_id->ViewCustomAttributes = "";
        ...
        // asset_banner_id
        $this->asset_banner_id->LinkCustomAttributes = "";
        $this->asset_banner_id->HrefValue = "";
        $this->asset_banner_id->TooltipValue = "";
        ...
        if ($this->image->UseColorbox) {
            ...
            $this->image->LinkAttrs["data-rel"] = "asset_banner_x_image";
    ...
    public function renderEditRow()
    {
        ...
        // asset_banner_id
        $this->asset_banner_id->setupEditAttributes();
        $this->asset_banner_id->EditCustomAttributes = "";
        $this->asset_banner_id->EditValue = $this->asset_banner_id->CurrentValue;
        $this->asset_banner_id->ViewCustomAttributes = "";
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        if (count($ar) == 1) {
            $this->asset_banner_id->CurrentValue = $ar[0];
        } else {
