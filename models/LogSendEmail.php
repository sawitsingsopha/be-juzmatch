...
/**
 * Table class for log_send_email
 */
class LogSendEmail extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'log_send_email';
        $this->TableName = 'log_send_email';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`log_send_email`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->log_email_id->InputTextType = "text";
        $this->log_email_id->IsAutoIncrement = true; // Autoincrement field
        $this->log_email_id->IsPrimaryKey = true; // Primary key field
        $this->log_email_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['log_email_id'] = &$this->log_email_id;
        ...
        $this->email_from->InputTextType = "text";
        $this->email_from->Nullable = false; // NOT NULL field
        $this->email_from->Required = true; // Required field
        $this->Fields['email_from'] = &$this->email_from;

        // email_to
        $this->email_to = new DbField(
            'log_send_email',
            'log_send_email',
            'x_email_to',
            'email_to',
        ...
        $this->email_to->InputTextType = "text";
        $this->email_to->Nullable = false; // NOT NULL field
        $this->email_to->Required = true; // Required field
        $this->Fields['email_to'] = &$this->email_to;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`log_send_email`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->log_email_id->setDbValue($conn->lastInsertId());
            $rs['log_email_id'] = $this->log_email_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('log_email_id', $rs)) {
                AddFilter($where, QuotedName('log_email_id', $this->Dbid) . '=' . QuotedValue($rs['log_email_id'], $this->log_email_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->log_email_id->DbValue = $row['log_email_id'];
        $this->cc->DbValue = $row['cc'];
        ...
        $this->email_from->DbValue = $row['email_from'];
        $this->email_to->DbValue = $row['email_to'];
    }
    ...
    {
        return "`log_email_id` = @log_email_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->log_email_id->CurrentValue : $this->log_email_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->log_email_id->CurrentValue = $keys[0];
            } else {
                $this->log_email_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('log_email_id', $row) ? $row['log_email_id'] : null;
        } else {
            $val = $this->log_email_id->OldValue !== null ? $this->log_email_id->OldValue : $this->log_email_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@log_email_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("logsendemaillist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "logsendemailedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "logsendemailadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "LogSendEmailView";
            case Config("API_ADD_ACTION"):
                return "LogSendEmailAdd";
            case Config("API_EDIT_ACTION"):
                return "LogSendEmailEdit";
            case Config("API_DELETE_ACTION"):
                return "LogSendEmailDelete";
            case Config("API_LIST_ACTION"):
                return "LogSendEmailList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("logsendemailview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("logsendemailview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "logsendemailadd?" . $this->getUrlParm($parm);
        } else {
            $url = "logsendemailadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("logsendemailedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logsendemailadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("logsendemaildelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"log_email_id\":" . JsonEncode($this->log_email_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->log_email_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->log_email_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("log_email_id") ?? Route("log_email_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->log_email_id->CurrentValue = $key;
            } else {
                $this->log_email_id->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        // log_email_id
        $this->log_email_id->setupEditAttributes();
        $this->log_email_id->EditCustomAttributes = "";
        $this->log_email_id->EditValue = $this->log_email_id->CurrentValue;
        $this->log_email_id->ViewCustomAttributes = "";
        ...
        $this->email_from->EditValue = $this->email_from->CurrentValue;
        ...
        $this->email_to->EditValue = $this->email_to->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->log_email_id);
                    $doc->exportCaption($this->cc);
                    ...
                    $doc->exportCaption($this->email_from);
                    $doc->exportCaption($this->email_to);
                } else {
                    $doc->exportCaption($this->log_email_id);
                    $doc->exportCaption($this->cc);
                    $doc->exportCaption($this->subject);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->log_email_id);
                        $doc->exportField($this->cc);
                        ...
                        $doc->exportField($this->email_from);
                        $doc->exportField($this->email_to);
                    } else {
                        $doc->exportField($this->log_email_id);
                        $doc->exportField($this->cc);
                        $doc->exportField($this->subject);
