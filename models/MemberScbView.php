 ...
 * Page class
 */
class MemberScbView extends MemberScb
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MemberScbView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("member_scb_id") ?? Route("member_scb_id")) !== null) {
            $this->RecKey["member_scb_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("member_scb_id") ?? Route("member_scb_id")) !== null) {
                $this->member_scb_id->setQueryStringValue($keyValue);
                $this->RecKey["member_scb_id"] = $this->member_scb_id->QueryStringValue;
            } elseif (Post("member_scb_id") !== null) {
                $this->member_scb_id->setFormValue(Post("member_scb_id"));
                $this->RecKey["member_scb_id"] = $this->member_scb_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->member_scb_id->setQueryStringValue($keyValue);
                $this->RecKey["member_scb_id"] = $this->member_scb_id->QueryStringValue;
            } else {
                $returnUrl = "memberscblist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "memberscblist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "memberscblist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("memberscblist"), "", $this->TableVar, true);
        $pageId = "view";
