...
/**
 * Table class for log_login
 */
class LogLogin extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'log_login';
        $this->TableName = 'log_login';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`log_login`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->log_login_id->InputTextType = "text";
        $this->log_login_id->IsPrimaryKey = true; // Primary key field
        $this->log_login_id->Nullable = false; // NOT NULL field
        $this->log_login_id->Required = true; // Required field
        $this->log_login_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['log_login_id'] = &$this->log_login_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`log_login`";
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('log_login_id', $rs)) {
                AddFilter($where, QuotedName('log_login_id', $this->Dbid) . '=' . QuotedValue($rs['log_login_id'], $this->log_login_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->log_login_id->DbValue = $row['log_login_id'];
    ...
    {
        return "`log_login_id` = @log_login_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->log_login_id->CurrentValue : $this->log_login_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->log_login_id->CurrentValue = $keys[0];
            } else {
                $this->log_login_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('log_login_id', $row) ? $row['log_login_id'] : null;
        } else {
            $val = $this->log_login_id->OldValue !== null ? $this->log_login_id->OldValue : $this->log_login_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@log_login_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("logloginlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "logloginview") {
            return $Language->phrase("View");
        } elseif ($pageName == "logloginedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "logloginadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("logloginview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("logloginview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "logloginadd?" . $this->getUrlParm($parm);
        } else {
            $url = "logloginadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("logloginedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logloginadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("loglogindelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"log_login_id\":" . JsonEncode($this->log_login_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->log_login_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->log_login_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("log_login_id") ?? Route("log_login_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->log_login_id->CurrentValue = $key;
            } else {
                $this->log_login_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->log_login_id->setDbValue($row['log_login_id']);
    ...
    public function renderListRow()
    {
        ...
        // log_login_id
        $this->log_login_id->ViewValue = $this->log_login_id->CurrentValue;
        $this->log_login_id->ViewValue = FormatNumber($this->log_login_id->ViewValue, $this->log_login_id->formatPattern());
        $this->log_login_id->ViewCustomAttributes = "";
        ...
        // log_login_id
        $this->log_login_id->LinkCustomAttributes = "";
        $this->log_login_id->HrefValue = "";
        $this->log_login_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // log_login_id
        $this->log_login_id->setupEditAttributes();
        $this->log_login_id->EditCustomAttributes = "";
        $this->log_login_id->EditValue = $this->log_login_id->CurrentValue;
        $this->log_login_id->PlaceHolder = RemoveHtml($this->log_login_id->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->log_login_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->log_login_id);
