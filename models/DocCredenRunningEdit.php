 ...
 * Page class
 */
class DocCredenRunningEdit extends DocCredenRunning
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocCredenRunningEdit";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "doccredenrunningview") {
                        $row["view"] = "1";
                    }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("doccredenrunninglist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "doccredenrunninglist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'running' first before field var 'x_running'
        $val = $CurrentForm->hasValue("running") ? $CurrentForm->getValue("running") : $CurrentForm->getValue("x_running");
        if (!$this->running->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->running->Visible = false; // Disable update for API request
            } else {
                $this->running->setFormValue($val, true, $validate);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->running->CurrentValue = $this->running->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // running
            $this->running->setupEditAttributes();
            $this->running->EditCustomAttributes = "";
            $this->running->EditValue = HtmlEncode($this->running->CurrentValue);
            $this->running->PlaceHolder = RemoveHtml($this->running->caption());
            if (strval($this->running->EditValue) != "" && is_numeric($this->running->EditValue)) {
                $this->running->EditValue = FormatNumber($this->running->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->running->Required) {
            if (!$this->running->IsDetailKey && EmptyValue($this->running->FormValue)) {
                $this->running->addErrorMessage(str_replace("%s", $this->running->caption(), $this->running->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->running->FormValue)) {
            $this->running->addErrorMessage($this->running->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // running
            $this->running->setDbValueDef($rsnew, $this->running->CurrentValue, 0, $this->running->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("doccredenrunninglist"), "", $this->TableVar, true);
        $pageId = "edit";
