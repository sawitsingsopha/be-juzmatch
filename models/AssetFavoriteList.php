 ...
 * Page class
 */
class AssetFavoriteList extends AssetFavorite
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'asset_favorite';

    // Page object name
    public $PageObjName = "AssetFavoriteList";
    ...
    public $FormName = "fasset_favoritelist";
    ...
    public function __construct()
    {
        ...
        // Table object (asset_favorite)
        if (!isset($GLOBALS["asset_favorite"]) || get_class($GLOBALS["asset_favorite"]) == PROJECT_NAMESPACE . "asset_favorite") {
            $GLOBALS["asset_favorite"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "assetfavoriteadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "assetfavoritedelete";
        $this->MultiUpdateUrl = "assetfavoriteupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'asset_favorite');
        }
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->asset_favorite_id->Visible = false;
        }
        if ($this->isAddOrEdit()) {
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->isfavorite);
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "member") {
            $masterTbl = Container("member");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("memberlist"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fasset_favoritesrch");
        }
        $filterList = Concat($filterList, $this->asset_favorite_id->AdvancedSearch->toJson(), ","); // Field asset_favorite_id
        ...
        $filterList = Concat($filterList, $this->isfavorite->AdvancedSearch->toJson(), ","); // Field isfavorite
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fasset_favoritesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field asset_favorite_id
        $this->asset_favorite_id->AdvancedSearch->SearchValue = @$filter["x_asset_favorite_id"];
        $this->asset_favorite_id->AdvancedSearch->SearchOperator = @$filter["z_asset_favorite_id"];
        $this->asset_favorite_id->AdvancedSearch->SearchCondition = @$filter["v_asset_favorite_id"];
        $this->asset_favorite_id->AdvancedSearch->SearchValue2 = @$filter["y_asset_favorite_id"];
        $this->asset_favorite_id->AdvancedSearch->SearchOperator2 = @$filter["w_asset_favorite_id"];
        $this->asset_favorite_id->AdvancedSearch->save();
        ...
        // Field isfavorite
        $this->isfavorite->AdvancedSearch->SearchValue = @$filter["x_isfavorite"];
        $this->isfavorite->AdvancedSearch->SearchOperator = @$filter["z_isfavorite"];
        $this->isfavorite->AdvancedSearch->SearchCondition = @$filter["v_isfavorite"];
        $this->isfavorite->AdvancedSearch->SearchValue2 = @$filter["y_isfavorite"];
        $this->isfavorite->AdvancedSearch->SearchOperator2 = @$filter["w_isfavorite"];
        $this->isfavorite->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->asset_favorite_id, $default, false); // asset_favorite_id
        ...
        $this->buildSearchSql($where, $this->isfavorite, $default, false); // isfavorite
        ...
        if (!$default && $this->Command == "search") {
            $this->asset_favorite_id->AdvancedSearch->save(); // asset_favorite_id
            $this->asset_id->AdvancedSearch->save(); // asset_id
            $this->member_id->AdvancedSearch->save(); // member_id
            $this->isfavorite->AdvancedSearch->save(); // isfavorite
    ...
    {
        if ($this->asset_favorite_id->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->isfavorite->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->asset_favorite_id->AdvancedSearch->unsetSession();
        $this->asset_id->AdvancedSearch->unsetSession();
        $this->member_id->AdvancedSearch->unsetSession();
        $this->isfavorite->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->asset_favorite_id->AdvancedSearch->load();
        $this->asset_id->AdvancedSearch->load();
        $this->member_id->AdvancedSearch->load();
        $this->isfavorite->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->isfavorite, $ctrl); // isfavorite
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->asset_favorite_id->setSort("");
                $this->asset_id->setSort("");
                $this->member_id->setSort("");
                $this->isfavorite->setSort("");
                $this->cdate->setSort("");
                $this->cip->setSort("");
                $this->cuser->setSort("");
                $this->udate->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_favoritelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_favoritelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->asset_favorite_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("isfavorite", $this->createColumnOption("isfavorite"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fasset_favoritesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fasset_favoritesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fasset_favoritelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // asset_favorite_id
        if ($this->asset_favorite_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->asset_favorite_id->AdvancedSearch->SearchValue != "" || $this->asset_favorite_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // isfavorite
        if ($this->isfavorite->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->isfavorite->AdvancedSearch->SearchValue != "" || $this->isfavorite->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fasset_favoritelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fasset_favoritesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->asset_id->Visible || $this->isfavorite->Visible;
    }
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "member") {
            $member = Container("member");
            $rsmaster = $member->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $member;
                    $member->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
