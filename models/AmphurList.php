 ...
 * Page class
 */
class AmphurList extends Amphur
{
    use MessagesTrait;
    ...
    public function __construct()
    {
        ...
        // Table object (amphur)
        if (!isset($GLOBALS["amphur"]) || get_class($GLOBALS["amphur"]) == PROJECT_NAMESPACE . "amphur") {
            $GLOBALS["amphur"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "amphuradd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "amphurdelete";
        $this->MultiUpdateUrl = "amphurupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'amphur');
        }
    ...
    public function run()
    {
        ...
        $this->amphur_code->setVisibility();
        $this->amphur_name->setVisibility();
        $this->amphur_name_en->setVisibility();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->amphur_id, $ctrl); // amphur_id
            $this->updateSort($this->amphur_code, $ctrl); // amphur_code
            $this->updateSort($this->amphur_name, $ctrl); // amphur_name
            $this->updateSort($this->amphur_name_en, $ctrl); // amphur_name_en
            $this->updateSort($this->geo_id, $ctrl); // geo_id
            $this->updateSort($this->province_id, $ctrl); // province_id
            $this->updateSort($this->postcode, $ctrl); // postcode
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"famphurlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"famphurlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->amphur_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("amphur_id", $this->createColumnOption("amphur_id"));
            $option->add("amphur_code", $this->createColumnOption("amphur_code"));
            $option->add("amphur_name", $this->createColumnOption("amphur_name"));
            $option->add("amphur_name_en", $this->createColumnOption("amphur_name_en"));
            $option->add("geo_id", $this->createColumnOption("geo_id"));
            $option->add("province_id", $this->createColumnOption("province_id"));
            $option->add("postcode", $this->createColumnOption("postcode"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"famphursrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"famphursrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="famphurlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        $row = [];
        $row['amphur_id'] = null;
        $row['amphur_code'] = null;
        $row['amphur_name'] = null;
        $row['amphur_name_en'] = null;
        $row['geo_id'] = null;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="famphurlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
