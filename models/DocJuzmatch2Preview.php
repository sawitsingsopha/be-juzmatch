 ...
 * Page class
 */
class DocJuzmatch2Preview extends DocJuzmatch2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocJuzmatch2Preview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "invertor_all_booking") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=invertor_all_booking&" . GetForeignKeyUrl("fk_invertor_booking_id", $this->investor_booking_id->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeysFromFilter($f)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "invertor_all_booking") {
            $find = "`investor_booking_id`=";
            $x = strpos($f, $find);
            if ($x !== false) {
                $x += strlen($find);
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                $this->investor_booking_id->setQueryStringValue($val);
            }
        }
    }
