...
/**
 * Table class for asset_warning
 */
class AssetWarning extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'asset_warning';
        $this->TableName = 'asset_warning';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`asset_warning`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // asset_warning_id
        $this->asset_warning_id = new DbField(
            'asset_warning',
            'asset_warning',
            'x_asset_warning_id',
            'asset_warning_id',
        ...
        $this->asset_warning_id->InputTextType = "text";
        $this->asset_warning_id->IsAutoIncrement = true; // Autoincrement field
        $this->asset_warning_id->IsPrimaryKey = true; // Primary key field
        $this->asset_warning_id->Sortable = false; // Allow sort
        $this->asset_warning_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['asset_warning_id'] = &$this->asset_warning_id;
        ...
        $this->note->UseFilter = true; // Table header filter
        $this->note->Lookup = new Lookup('note', 'asset_warning', true, 'note', ["note","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->status->Lookup = new Lookup('status', 'asset_warning', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "sale_asset") {
            ...
            if ($this->member_id->getSessionValue() != "") {
                $masterFilter .= " AND " . GetForeignKeySql("`member_id`", $this->member_id->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "sale_asset") {
            ...
            if ($this->member_id->getSessionValue() != "") {
                $detailFilter .= " AND " . GetForeignKeySql("`member_id`", $this->member_id->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`asset_warning`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->asset_warning_id->setDbValue($conn->lastInsertId());
            $rs['asset_warning_id'] = $this->asset_warning_id->DbValue;
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('asset_warning_id', $rs)) {
                AddFilter($where, QuotedName('asset_warning_id', $this->Dbid) . '=' . QuotedValue($rs['asset_warning_id'], $this->asset_warning_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->asset_warning_id->DbValue = $row['asset_warning_id'];
    ...
    {
        return "`asset_warning_id` = @asset_warning_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->asset_warning_id->CurrentValue : $this->asset_warning_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->asset_warning_id->CurrentValue = $keys[0];
            } else {
                $this->asset_warning_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('asset_warning_id', $row) ? $row['asset_warning_id'] : null;
        } else {
            $val = $this->asset_warning_id->OldValue !== null ? $this->asset_warning_id->OldValue : $this->asset_warning_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@asset_warning_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("assetwarninglist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "assetwarningedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "assetwarningadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "AssetWarningView";
            case Config("API_ADD_ACTION"):
                return "AssetWarningAdd";
            case Config("API_EDIT_ACTION"):
                return "AssetWarningEdit";
            case Config("API_DELETE_ACTION"):
                return "AssetWarningDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("assetwarningview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("assetwarningview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "assetwarningadd?" . $this->getUrlParm($parm);
        } else {
            $url = "assetwarningadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("assetwarningedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetwarningadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("assetwarningdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"asset_warning_id\":" . JsonEncode($this->asset_warning_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->asset_warning_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->asset_warning_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("asset_warning_id") ?? Route("asset_warning_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->asset_warning_id->CurrentValue = $key;
            } else {
                $this->asset_warning_id->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // asset_warning_id
        $this->asset_warning_id->ViewValue = $this->asset_warning_id->CurrentValue;
        $this->asset_warning_id->ViewCustomAttributes = "";
        ...
        // asset_warning_id
        $this->asset_warning_id->LinkCustomAttributes = "";
        $this->asset_warning_id->HrefValue = "";
        $this->asset_warning_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // asset_warning_id
        $this->asset_warning_id->setupEditAttributes();
        $this->asset_warning_id->EditCustomAttributes = "";
        $this->asset_warning_id->EditValue = $this->asset_warning_id->CurrentValue;
        $this->asset_warning_id->ViewCustomAttributes = "";
