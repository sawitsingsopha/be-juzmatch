 ...
 * Page class
 */
class LogMemberScbEdit extends LogMemberScb
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogMemberScbEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("log_member_scb_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->log_member_scb_id->setQueryStringValue($keyValue);
                $this->log_member_scb_id->setOldValue($this->log_member_scb_id->QueryStringValue);
            } elseif (Post("log_member_scb_id") !== null) {
                $this->log_member_scb_id->setFormValue(Post("log_member_scb_id"));
                $this->log_member_scb_id->setOldValue($this->log_member_scb_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->log_member_scb_id->IsDetailKey) {
            $this->log_member_scb_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->log_member_scb_id->CurrentValue = $this->log_member_scb_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // log_member_scb_id
            $this->log_member_scb_id->setupEditAttributes();
            $this->log_member_scb_id->EditCustomAttributes = "";
            $this->log_member_scb_id->EditValue = $this->log_member_scb_id->CurrentValue;
            $this->log_member_scb_id->ViewCustomAttributes = "";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->log_member_scb_id->Required) {
            if (!$this->log_member_scb_id->IsDetailKey && EmptyValue($this->log_member_scb_id->FormValue)) {
                $this->log_member_scb_id->addErrorMessage(str_replace("%s", $this->log_member_scb_id->caption(), $this->log_member_scb_id->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // reference_url
            $this->reference_url->setDbValueDef($rsnew, $this->reference_url->CurrentValue, null, $this->reference_url->ReadOnly);

            // member_id
            $this->member_id->setDbValueDef($rsnew, $this->member_id->CurrentValue, null, $this->member_id->ReadOnly);

            // refreshtoken
            $this->refreshtoken->setDbValueDef($rsnew, $this->refreshtoken->CurrentValue, null, $this->refreshtoken->ReadOnly);

            // auth_code
            $this->auth_code->setDbValueDef($rsnew, $this->auth_code->CurrentValue, null, $this->auth_code->ReadOnly);
            ...
            // at_expire_in
            $this->at_expire_in->setDbValueDef($rsnew, $this->at_expire_in->CurrentValue, null, $this->at_expire_in->ReadOnly);

            // rt_expire_in
            $this->rt_expire_in->setDbValueDef($rsnew, $this->rt_expire_in->CurrentValue, null, $this->rt_expire_in->ReadOnly);

            // asset_id
            $this->asset_id->setDbValueDef($rsnew, $this->asset_id->CurrentValue, null, $this->asset_id->ReadOnly);

            // decision_status
            $this->decision_status->setDbValueDef($rsnew, $this->decision_status->CurrentValue, null, $this->decision_status->ReadOnly);

            // decision_timestamp
            $this->decision_timestamp->setDbValueDef($rsnew, UnFormatDateTime($this->decision_timestamp->CurrentValue, $this->decision_timestamp->formatPattern()), null, $this->decision_timestamp->ReadOnly);

            // deposit_amount
            $this->deposit_amount->setDbValueDef($rsnew, $this->deposit_amount->CurrentValue, null, $this->deposit_amount->ReadOnly);

            // due_date
            $this->due_date->setDbValueDef($rsnew, $this->due_date->CurrentValue, null, $this->due_date->ReadOnly);

            // rental_fee
            $this->rental_fee->setDbValueDef($rsnew, $this->rental_fee->CurrentValue, null, $this->rental_fee->ReadOnly);
            ...
            // noOfChildren
            $this->noOfChildren->setDbValueDef($rsnew, $this->noOfChildren->CurrentValue, null, $this->noOfChildren->ReadOnly);

            // educationLevel
            $this->educationLevel->setDbValueDef($rsnew, $this->educationLevel->CurrentValue, null, $this->educationLevel->ReadOnly);

            // workplace
            $this->workplace->setDbValueDef($rsnew, $this->workplace->CurrentValue, null, $this->workplace->ReadOnly);

            // occupation
            $this->occupation->setDbValueDef($rsnew, $this->occupation->CurrentValue, null, $this->occupation->ReadOnly);

            // jobPosition
            $this->jobPosition->setDbValueDef($rsnew, $this->jobPosition->CurrentValue, null, $this->jobPosition->ReadOnly);

            // submissionDate
            $this->submissionDate->setDbValueDef($rsnew, UnFormatDateTime($this->submissionDate->CurrentValue, $this->submissionDate->formatPattern()), null, $this->submissionDate->ReadOnly);

            // bankruptcy_tendency
            $this->bankruptcy_tendency->setDbValueDef($rsnew, $this->bankruptcy_tendency->CurrentValue, null, $this->bankruptcy_tendency->ReadOnly);

            // blacklist_tendency
            $this->blacklist_tendency->setDbValueDef($rsnew, $this->blacklist_tendency->CurrentValue, null, $this->blacklist_tendency->ReadOnly);

            // money_laundering_tendency
            $this->money_laundering_tendency->setDbValueDef($rsnew, $this->money_laundering_tendency->CurrentValue, null, $this->money_laundering_tendency->ReadOnly);

            // mobile_fraud_behavior
            $this->mobile_fraud_behavior->setDbValueDef($rsnew, $this->mobile_fraud_behavior->CurrentValue, null, $this->mobile_fraud_behavior->ReadOnly);

            // face_similarity_score
            $this->face_similarity_score->setDbValueDef($rsnew, $this->face_similarity_score->CurrentValue, null, $this->face_similarity_score->ReadOnly);

            // identification_verification_matched_flag
            $this->identification_verification_matched_flag->setDbValueDef($rsnew, $this->identification_verification_matched_flag->CurrentValue, null, $this->identification_verification_matched_flag->ReadOnly);

            // bankstatement_confident_score
            $this->bankstatement_confident_score->setDbValueDef($rsnew, $this->bankstatement_confident_score->CurrentValue, null, $this->bankstatement_confident_score->ReadOnly);

            // estimated_monthly_income
            $this->estimated_monthly_income->setDbValueDef($rsnew, $this->estimated_monthly_income->CurrentValue, null, $this->estimated_monthly_income->ReadOnly);

            // estimated_monthly_debt
            $this->estimated_monthly_debt->setDbValueDef($rsnew, $this->estimated_monthly_debt->CurrentValue, null, $this->estimated_monthly_debt->ReadOnly);

            // income_stability
            $this->income_stability->setDbValueDef($rsnew, $this->income_stability->CurrentValue, null, $this->income_stability->ReadOnly);

            // customer_grade
            $this->customer_grade->setDbValueDef($rsnew, $this->customer_grade->CurrentValue, null, $this->customer_grade->ReadOnly);

            // color_sign
            $this->color_sign->setDbValueDef($rsnew, $this->color_sign->CurrentValue, null, $this->color_sign->ReadOnly);

            // rental_period
            $this->rental_period->setDbValueDef($rsnew, $this->rental_period->CurrentValue, null, $this->rental_period->ReadOnly);
