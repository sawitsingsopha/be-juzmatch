 ...
 * Page class
 */
class AssetFacilitiesEdit extends AssetFacilities
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AssetFacilitiesEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("asset_facilities_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->asset_facilities_id->setQueryStringValue($keyValue);
                $this->asset_facilities_id->setOldValue($this->asset_facilities_id->QueryStringValue);
            } elseif (Post("asset_facilities_id") !== null) {
                $this->asset_facilities_id->setFormValue(Post("asset_facilities_id"));
                $this->asset_facilities_id->setOldValue($this->asset_facilities_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("asset_facilities_id") ?? Route("asset_facilities_id")) !== null) {
                    $this->asset_facilities_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("assetfacilitieslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "assetfacilitieslist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->asset_facilities_id->IsDetailKey) {
    ...
    public function renderRow()
    {
        ...
        // asset_facilities_id
        $this->asset_facilities_id->RowCssClass = "row";

        // asset_id
        $this->asset_id->RowCssClass = "row";

        // master_facilities_group_id
        $this->master_facilities_group_id->RowCssClass = "row";

        // master_facilities_id
        $this->master_facilities_id->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("assetfacilitieslist"), "", $this->TableVar, true);
        $pageId = "edit";
