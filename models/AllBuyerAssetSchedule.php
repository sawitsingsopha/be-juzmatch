...
/**
 * Table class for all_buyer_asset_schedule
 */
class AllBuyerAssetSchedule extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'all_buyer_asset_schedule';
        $this->TableName = 'all_buyer_asset_schedule';
        ...
        if(isset($_GET['installment'])) {
            ...
            $this->GridAddRowCount = $installment + 1;
        } else {
        }
        ...
        $this->expired_date = new DbField(
            ...
            'expired_date',
            '`expired_date`',
            CastDateFieldForLike("`expired_date`", 7, "juzmatch1"),
            135,
            19,
            7,
            false,
        ...
        $this->status_payment->Lookup = new Lookup('status_payment', 'all_buyer_asset_schedule', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "all_buyer_config_asset_schedule") {
            if ($this->buyer_config_asset_schedule_id->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`buyer_config_asset_schedule_id`", $this->buyer_config_asset_schedule_id->getSessionValue(), DATATYPE_NUMBER, "juzmatch1");
            } else {
                return "";
            }
        }
        if ($this->getCurrentMasterTable() == "number_deals_available") {
            if ($this->buyer_config_asset_schedule_id->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`buyer_config_asset_schedule_id`", $this->buyer_config_asset_schedule_id->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "all_buyer_config_asset_schedule") {
            if ($this->buyer_config_asset_schedule_id->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`buyer_config_asset_schedule_id`", $this->buyer_config_asset_schedule_id->getSessionValue(), DATATYPE_NUMBER, "juzmatch1");
            } else {
                return "";
            }
        }
        if ($this->getCurrentMasterTable() == "number_deals_available") {
            if ($this->buyer_config_asset_schedule_id->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`buyer_config_asset_schedule_id`", $this->buyer_config_asset_schedule_id->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "all_buyer_config_asset_schedule":
                $key = $keys["buyer_config_asset_schedule_id"] ?? "";
                if (EmptyValue($key)) {
                    if ($masterTable->buyer_config_asset_schedule_id->Required) { // Required field and empty value
                ...
                if ($validKeys) {
                    return "`buyer_config_asset_schedule_id`=" . QuotedValue($keys["buyer_config_asset_schedule_id"], $masterTable->buyer_config_asset_schedule_id->DataType, $masterTable->Dbid);
                }
                break;
            case "number_deals_available":
                $key = $keys["buyer_config_asset_schedule_id"] ?? "";
                if (EmptyValue($key)) {
                    if ($masterTable->buyer_config_asset_schedule_id->Required) { // Required field and empty value
                        return ""; // Return empty filter
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "all_buyer_config_asset_schedule":
                return "`buyer_config_asset_schedule_id`=" . QuotedValue($masterTable->buyer_config_asset_schedule_id->DbValue, $this->buyer_config_asset_schedule_id->DataType, $this->Dbid);
            case "number_deals_available":
                return "`buyer_config_asset_schedule_id`=" . QuotedValue($masterTable->buyer_config_asset_schedule_id->DbValue, $this->buyer_config_asset_schedule_id->DataType, $this->Dbid);
            case "number_of_accrued":
                return "`buyer_config_asset_schedule_id`=" . QuotedValue($masterTable->buyer_config_asset_schedule_id->DbValue, $this->buyer_config_asset_schedule_id->DataType, $this->Dbid);
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("allbuyerassetschedulelist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "allbuyerassetscheduleedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "allbuyerassetscheduleadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "AllBuyerAssetScheduleView";
            case Config("API_ADD_ACTION"):
                return "AllBuyerAssetScheduleAdd";
            case Config("API_EDIT_ACTION"):
                return "AllBuyerAssetScheduleEdit";
            case Config("API_DELETE_ACTION"):
                return "AllBuyerAssetScheduleDelete";
            case Config("API_LIST_ACTION"):
                return "AllBuyerAssetScheduleList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("allbuyerassetscheduleview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("allbuyerassetscheduleview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "allbuyerassetscheduleadd?" . $this->getUrlParm($parm);
        } else {
            $url = "allbuyerassetscheduleadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("allbuyerassetscheduleedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("allbuyerassetscheduleadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("allbuyerassetscheduledelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "all_buyer_config_asset_schedule" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->CurrentValue);
        }
        if ($this->getCurrentMasterTable() == "number_deals_available" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->CurrentValue);
        }
        if ($this->getCurrentMasterTable() == "number_of_accrued" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->CurrentValue);
        }
        if ($this->getCurrentMasterTable() == "number_of_unpaid_units" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->CurrentValue);
        }
        if ($this->getCurrentMasterTable() == "outstanding_amount" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->CurrentValue);
        }
        return $url;
    ...
    public function renderEditRow()
    {
        ...
        if ($this->buyer_config_asset_schedule_id->getSessionValue() != "") {
            $this->buyer_config_asset_schedule_id->CurrentValue = GetForeignKeyValue($this->buyer_config_asset_schedule_id->getSessionValue());
            $this->buyer_config_asset_schedule_id->ViewValue = $this->buyer_config_asset_schedule_id->CurrentValue;
            $this->buyer_config_asset_schedule_id->ViewValue = FormatNumber($this->buyer_config_asset_schedule_id->ViewValue, $this->buyer_config_asset_schedule_id->formatPattern());
            $this->buyer_config_asset_schedule_id->ViewCustomAttributes = "";
        } else {
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->principal);
                    $doc->exportCaption($this->remaining_principal);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->principal);
                        $doc->exportField($this->remaining_principal);
    ...
    {
        $table = 'all_buyer_asset_schedule';
        $usr = CurrentUserID();
