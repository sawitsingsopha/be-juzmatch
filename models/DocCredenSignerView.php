 ...
 * Page class
 */
class DocCredenSignerView extends DocCredenSigner
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocCredenSignerView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("doc_creden_signer_id") ?? Route("doc_creden_signer_id")) !== null) {
            $this->RecKey["doc_creden_signer_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("doc_creden_signer_id") ?? Route("doc_creden_signer_id")) !== null) {
                ...
                $this->RecKey["doc_creden_signer_id"] = $this->doc_creden_signer_id->QueryStringValue;
            } elseif (Post("doc_creden_signer_id") !== null) {
                $this->doc_creden_signer_id->setFormValue(Post("doc_creden_signer_id"));
                $this->RecKey["doc_creden_signer_id"] = $this->doc_creden_signer_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->doc_creden_signer_id->setQueryStringValue($keyValue);
                $this->RecKey["doc_creden_signer_id"] = $this->doc_creden_signer_id->QueryStringValue;
            } else {
                $returnUrl = "doccredensignerlist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "doccredensignerlist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "doccredensignerlist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
