...
/**
 * Table class for asset_pros
 */
class AssetPros extends DbTable
{
    protected $SqlFrom = "";
    ...
    public $udate;
    public $uip;
    public $master_asset_icon_pros_id;
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'asset_pros';
        $this->TableName = 'asset_pros';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`asset_pros`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // asset_pros_id
        $this->asset_pros_id = new DbField(
            'asset_pros',
            'asset_pros',
            'x_asset_pros_id',
            'asset_pros_id',
        ...
        $this->asset_pros_id->InputTextType = "text";
        $this->asset_pros_id->IsAutoIncrement = true; // Autoincrement field
        $this->asset_pros_id->IsPrimaryKey = true; // Primary key field
        $this->asset_pros_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['asset_pros_id'] = &$this->asset_pros_id;
        ...
        // master_asset_icon_pros_id
        $this->master_asset_icon_pros_id = new DbField(
            'asset_pros',
            'asset_pros',
            'x_master_asset_icon_pros_id',
            'master_asset_icon_pros_id',
            '`master_asset_icon_pros_id`',
            '`master_asset_icon_pros_id`',
            3,
        ...
        $this->master_asset_icon_pros_id->InputTextType = "text";
        $this->master_asset_icon_pros_id->Nullable = false; // NOT NULL field
        $this->master_asset_icon_pros_id->Required = true; // Required field
        $this->master_asset_icon_pros_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['master_asset_icon_pros_id'] = &$this->master_asset_icon_pros_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`asset_pros`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->asset_pros_id->setDbValue($conn->lastInsertId());
            $rs['asset_pros_id'] = $this->asset_pros_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('asset_pros_id', $rs)) {
                AddFilter($where, QuotedName('asset_pros_id', $this->Dbid) . '=' . QuotedValue($rs['asset_pros_id'], $this->asset_pros_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->asset_pros_id->DbValue = $row['asset_pros_id'];
        ...
        $this->master_asset_icon_pros_id->DbValue = $row['master_asset_icon_pros_id'];
    }
    ...
    {
        return "`asset_pros_id` = @asset_pros_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->asset_pros_id->CurrentValue : $this->asset_pros_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->asset_pros_id->CurrentValue = $keys[0];
            } else {
                $this->asset_pros_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('asset_pros_id', $row) ? $row['asset_pros_id'] : null;
        } else {
            $val = $this->asset_pros_id->OldValue !== null ? $this->asset_pros_id->OldValue : $this->asset_pros_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@asset_pros_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("assetproslist");
    }
    ...
    {
        global $Language;
        if ($pageName == "assetprosview") {
            return $Language->phrase("View");
        } elseif ($pageName == "assetprosedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "assetprosadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "AssetProsView";
            case Config("API_ADD_ACTION"):
                return "AssetProsAdd";
            case Config("API_EDIT_ACTION"):
                return "AssetProsEdit";
            case Config("API_DELETE_ACTION"):
                return "AssetProsDelete";
            case Config("API_LIST_ACTION"):
                return "AssetProsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("assetprosview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("assetprosview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "assetprosadd?" . $this->getUrlParm($parm);
        } else {
            $url = "assetprosadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("assetprosedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetprosadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("assetprosdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"asset_pros_id\":" . JsonEncode($this->asset_pros_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->asset_pros_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->asset_pros_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("asset_pros_id") ?? Route("asset_pros_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->asset_pros_id->CurrentValue = $key;
            } else {
                $this->asset_pros_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->master_asset_icon_pros_id->setDbValue($row['master_asset_icon_pros_id']);
    }
    ...
    public function renderListRow()
    {
        ...
        // asset_pros_id
        $this->asset_pros_id->ViewValue = $this->asset_pros_id->CurrentValue;
        $this->asset_pros_id->ViewCustomAttributes = "";
        ...
        $this->isactive->ViewValue = FormatNumber($this->isactive->ViewValue, $this->isactive->formatPattern());
        ...
        // master_asset_icon_pros_id
        $this->master_asset_icon_pros_id->ViewValue = $this->master_asset_icon_pros_id->CurrentValue;
        $this->master_asset_icon_pros_id->ViewValue = FormatNumber($this->master_asset_icon_pros_id->ViewValue, $this->master_asset_icon_pros_id->formatPattern());
        $this->master_asset_icon_pros_id->ViewCustomAttributes = "";

        // asset_pros_id
        $this->asset_pros_id->LinkCustomAttributes = "";
        $this->asset_pros_id->HrefValue = "";
        $this->asset_pros_id->TooltipValue = "";
        ...
        // master_asset_icon_pros_id
        $this->master_asset_icon_pros_id->LinkCustomAttributes = "";
        $this->master_asset_icon_pros_id->HrefValue = "";
        $this->master_asset_icon_pros_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // asset_pros_id
        $this->asset_pros_id->setupEditAttributes();
        $this->asset_pros_id->EditCustomAttributes = "";
        $this->asset_pros_id->EditValue = $this->asset_pros_id->CurrentValue;
        $this->asset_pros_id->ViewCustomAttributes = "";
        ...
        // master_asset_icon_pros_id
        $this->master_asset_icon_pros_id->setupEditAttributes();
        $this->master_asset_icon_pros_id->EditCustomAttributes = "";
        $this->master_asset_icon_pros_id->EditValue = $this->master_asset_icon_pros_id->CurrentValue;
        $this->master_asset_icon_pros_id->PlaceHolder = RemoveHtml($this->master_asset_icon_pros_id->caption());
        if (strval($this->master_asset_icon_pros_id->EditValue) != "" && is_numeric($this->master_asset_icon_pros_id->EditValue)) {
            $this->master_asset_icon_pros_id->EditValue = FormatNumber($this->master_asset_icon_pros_id->EditValue, null);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->asset_pros_id);
                    ...
                    $doc->exportCaption($this->master_asset_icon_pros_id);
                } else {
                    $doc->exportCaption($this->asset_pros_id);
                    $doc->exportCaption($this->asset_id);
                    $doc->exportCaption($this->title_en);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->asset_pros_id);
                        ...
                        $doc->exportField($this->master_asset_icon_pros_id);
                    } else {
                        $doc->exportField($this->asset_pros_id);
                        $doc->exportField($this->asset_id);
                        $doc->exportField($this->title_en);
