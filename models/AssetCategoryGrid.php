 ...
 * Page class
 */
class AssetCategoryGrid extends AssetCategory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AssetCategoryGrid";
    ...
    public $FormName = "fasset_categorygrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->asset_category_id->CurrentValue;
    ...
    {
        $this->asset_category_id->CurrentValue = null;
        $this->asset_category_id->OldValue = $this->asset_category_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'asset_category_id' first before field var 'x_asset_category_id'
        $val = $CurrentForm->hasValue("asset_category_id") ? $CurrentForm->getValue("asset_category_id") : $CurrentForm->getValue("x_asset_category_id");
        if (!$this->asset_category_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->asset_category_id->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->asset_category_id->CurrentValue = $this->asset_category_id->FormValue;
        }
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['asset_category_id'] = $this->asset_category_id->CurrentValue;
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->category_id->getSessionValue() != "") {
                ...
                if ($this->category_id->ViewValue !== null) { // Load from cache
                    ...
                    $lookupFilter = $lookupFilter->bindTo($this);
                    $sqlWrk = $this->category_id->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            $detailKeys['category_id'] = $keyValue;
            $masterTable = Container("category");
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'asset_category'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["category_id"] = $this->category_id->CurrentValue;
        $masterTable = Container("category");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
