...
class Province extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->province_id->IsAutoIncrement = true; // Autoincrement field
        $this->province_id->IsPrimaryKey = true; // Primary key field
        $this->province_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['province_id'] = &$this->province_id;

        // province_code
        $this->province_code = new DbField(
            'province',
            'province',
            'x_province_code',
            'province_code',
        ...
        $this->province_code->InputTextType = "text";
        ...
        // province_name
        $this->province_name = new DbField(
            'province',
            'province',
            'x_province_name',
            'province_name',
        ...
        $this->province_name->InputTextType = "text";
        $this->province_name->Nullable = false; // NOT NULL field
        $this->province_name->Required = true; // Required field
        $this->Fields['province_name'] = &$this->province_name;

        // province_name_en
        $this->province_name_en = new DbField(
            'province',
            'province',
            'x_province_name_en',
            'province_name_en',
        ...
        $this->province_name_en->InputTextType = "text";
        $this->province_name_en->Nullable = false; // NOT NULL field
        $this->province_name_en->Required = true; // Required field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`province`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->province_id->setDbValue($conn->lastInsertId());
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('province_id', $rs)) {
                AddFilter($where, QuotedName('province_id', $this->Dbid) . '=' . QuotedValue($rs['province_id'], $this->province_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->province_id->CurrentValue : $this->province_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->province_id->CurrentValue = $keys[0];
            } else {
                $this->province_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('province_id', $row) ? $row['province_id'] : null;
        } else {
            $val = $this->province_id->OldValue !== null ? $this->province_id->OldValue : $this->province_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@province_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    {
        $json = "";
        $json .= "\"province_id\":" . JsonEncode($this->province_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->province_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->province_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("province_id") ?? Route("province_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function renderEditRow()
    {
        ...
        // province_code
        $this->province_code->setupEditAttributes();
        ...
        // province_name
        $this->province_name->setupEditAttributes();
        $this->province_name->EditCustomAttributes = "";
        if (!$this->province_name->Raw) {
            $this->province_name->CurrentValue = HtmlDecode($this->province_name->CurrentValue);
        }
        $this->province_name->EditValue = $this->province_name->CurrentValue;
        $this->province_name->PlaceHolder = RemoveHtml($this->province_name->caption());

        // province_name_en
        $this->province_name_en->setupEditAttributes();
        $this->province_name_en->EditCustomAttributes = "";
        if (!$this->province_name_en->Raw) {
            $this->province_name_en->CurrentValue = HtmlDecode($this->province_name_en->CurrentValue);
        }
        $this->province_name_en->EditValue = $this->province_name_en->CurrentValue;
        $this->province_name_en->PlaceHolder = RemoveHtml($this->province_name_en->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->province_name);
                    $doc->exportCaption($this->province_name_en);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->province_name);
                        $doc->exportField($this->province_name_en);
