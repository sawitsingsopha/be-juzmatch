 ...
 * Page class
 */
class AssetCategoryEdit extends AssetCategory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AssetCategoryEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("asset_category_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->asset_category_id->setQueryStringValue($keyValue);
                $this->asset_category_id->setOldValue($this->asset_category_id->QueryStringValue);
            } elseif (Post("asset_category_id") !== null) {
                $this->asset_category_id->setFormValue(Post("asset_category_id"));
                $this->asset_category_id->setOldValue($this->asset_category_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("asset_category_id") ?? Route("asset_category_id")) !== null) {
                    $this->asset_category_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("assetcategorylist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "assetcategorylist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->asset_category_id->IsDetailKey) {
    ...
    public function renderRow()
    {
        ...
        // asset_category_id
        $this->asset_category_id->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("assetcategorylist"), "", $this->TableVar, true);
        $pageId = "edit";
