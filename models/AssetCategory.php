...
/**
 * Table class for asset_category
 */
class AssetCategory extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "`asset_category`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // asset_category_id
        $this->asset_category_id = new DbField(
            'asset_category',
            'asset_category',
            'x_asset_category_id',
            'asset_category_id',
        ...
        $this->asset_category_id->InputTextType = "text";
        $this->asset_category_id->IsAutoIncrement = true; // Autoincrement field
        $this->asset_category_id->IsPrimaryKey = true; // Primary key field
        $this->asset_category_id->Sortable = false; // Allow sort
        $this->asset_category_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['asset_category_id'] = &$this->asset_category_id;
        ...
        $this->category_id->Lookup = new Lookup('category_id', 'category', false, 'category_id', ["category_name","","",""], [], [], [], [], [], [], '`order_by` ASC', '', "`category_name`");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`asset_category`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->asset_category_id->setDbValue($conn->lastInsertId());
            $rs['asset_category_id'] = $this->asset_category_id->DbValue;
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('asset_category_id', $rs)) {
                AddFilter($where, QuotedName('asset_category_id', $this->Dbid) . '=' . QuotedValue($rs['asset_category_id'], $this->asset_category_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->asset_category_id->DbValue = $row['asset_category_id'];
    ...
    {
        return "`asset_category_id` = @asset_category_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->asset_category_id->CurrentValue : $this->asset_category_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->asset_category_id->CurrentValue = $keys[0];
            } else {
                $this->asset_category_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('asset_category_id', $row) ? $row['asset_category_id'] : null;
        } else {
            $val = $this->asset_category_id->OldValue !== null ? $this->asset_category_id->OldValue : $this->asset_category_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@asset_category_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("assetcategorylist");
    }
    ...
    {
        global $Language;
        if ($pageName == "assetcategoryview") {
            return $Language->phrase("View");
        } elseif ($pageName == "assetcategoryedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "assetcategoryadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "AssetCategoryAdd";
            case Config("API_EDIT_ACTION"):
                return "AssetCategoryEdit";
            case Config("API_DELETE_ACTION"):
                return "AssetCategoryDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("assetcategoryview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("assetcategoryview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "assetcategoryadd?" . $this->getUrlParm($parm);
        } else {
            $url = "assetcategoryadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetcategoryedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("assetcategoryadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("assetcategorydelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"asset_category_id\":" . JsonEncode($this->asset_category_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->asset_category_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->asset_category_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("asset_category_id") ?? Route("asset_category_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->asset_category_id->CurrentValue = $key;
            } else {
                $this->asset_category_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->asset_category_id->setDbValue($row['asset_category_id']);
    ...
    public function renderListRow()
    {
        ...
        // asset_category_id
        $this->asset_category_id->CellCssStyle = "white-space: nowrap;";
        ...
        // asset_category_id
        $this->asset_category_id->ViewValue = $this->asset_category_id->CurrentValue;
        $this->asset_category_id->ViewCustomAttributes = "";
        ...
        // asset_category_id
        $this->asset_category_id->LinkCustomAttributes = "";
        $this->asset_category_id->HrefValue = "";
        $this->asset_category_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // asset_category_id
        $this->asset_category_id->setupEditAttributes();
        $this->asset_category_id->EditCustomAttributes = "";
        $this->asset_category_id->EditValue = $this->asset_category_id->CurrentValue;
        $this->asset_category_id->ViewCustomAttributes = "";
