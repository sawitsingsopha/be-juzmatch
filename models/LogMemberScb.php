...
/**
 * Table class for log_member_scb
 */
class LogMemberScb extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'log_member_scb';
        $this->TableName = 'log_member_scb';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`log_member_scb`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // log_member_scb_id
        $this->log_member_scb_id = new DbField(
            'log_member_scb',
            'log_member_scb',
            'x_log_member_scb_id',
            'log_member_scb_id',
        ...
        $this->log_member_scb_id->InputTextType = "text";
        $this->log_member_scb_id->IsAutoIncrement = true; // Autoincrement field
        $this->log_member_scb_id->IsPrimaryKey = true; // Primary key field
        $this->log_member_scb_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['log_member_scb_id'] = &$this->log_member_scb_id;
        ...
        $this->due_date->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`log_member_scb`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->log_member_scb_id->setDbValue($conn->lastInsertId());
            $rs['log_member_scb_id'] = $this->log_member_scb_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('log_member_scb_id', $rs)) {
                AddFilter($where, QuotedName('log_member_scb_id', $this->Dbid) . '=' . QuotedValue($rs['log_member_scb_id'], $this->log_member_scb_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->log_member_scb_id->DbValue = $row['log_member_scb_id'];
    ...
    {
        return "`log_member_scb_id` = @log_member_scb_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->log_member_scb_id->CurrentValue : $this->log_member_scb_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->log_member_scb_id->CurrentValue = $keys[0];
            } else {
                $this->log_member_scb_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('log_member_scb_id', $row) ? $row['log_member_scb_id'] : null;
        } else {
            $val = $this->log_member_scb_id->OldValue !== null ? $this->log_member_scb_id->OldValue : $this->log_member_scb_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@log_member_scb_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("logmemberscblist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "logmemberscbedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "logmemberscbadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "LogMemberScbView";
            case Config("API_ADD_ACTION"):
                return "LogMemberScbAdd";
            case Config("API_EDIT_ACTION"):
                return "LogMemberScbEdit";
            case Config("API_DELETE_ACTION"):
                return "LogMemberScbDelete";
            case Config("API_LIST_ACTION"):
                return "LogMemberScbList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("logmemberscbview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("logmemberscbview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "logmemberscbadd?" . $this->getUrlParm($parm);
        } else {
            $url = "logmemberscbadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("logmemberscbedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logmemberscbadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("logmemberscbdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"log_member_scb_id\":" . JsonEncode($this->log_member_scb_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->log_member_scb_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->log_member_scb_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("log_member_scb_id") ?? Route("log_member_scb_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->log_member_scb_id->CurrentValue = $key;
            } else {
                $this->log_member_scb_id->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->log_member_scb_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->log_member_scb_id);
