 ...
 * Page class
 */
class PaymentInverterBookingGrid extends PaymentInverterBooking
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'payment_inverter_booking';

    // Page object name
    public $PageObjName = "PaymentInverterBookingGrid";
    ...
    public $FormName = "fpayment_inverter_bookinggrid";
    ...
    public function __construct()
    {
        ...
        // Table object (payment_inverter_booking)
        if (!isset($GLOBALS["payment_inverter_booking"]) || get_class($GLOBALS["payment_inverter_booking"]) == PROJECT_NAMESPACE . "payment_inverter_booking") {
            $GLOBALS["payment_inverter_booking"] = &$this;
        }
        $this->AddUrl = "paymentinverterbookingadd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'payment_inverter_booking');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("payment_inverter_booking");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->payment_number->setVisibility();
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "invertor_booking") {
            $masterTbl = Container("invertor_booking");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("invertorbookinglist"); // Return to master page
                return;
            } else {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->payment_inverter_booking_id->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_payment") && $CurrentForm->hasValue("o_payment") && $this->payment->CurrentValue != $this->payment->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_payment_number") && $CurrentForm->hasValue("o_payment_number") && $this->payment_number->CurrentValue != $this->payment_number->OldValue) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->payment->clearErrorMessage();
        $this->payment_number->clearErrorMessage();
    ...
    {
        $this->payment_inverter_booking_id->CurrentValue = null;
        $this->payment_inverter_booking_id->OldValue = $this->payment_inverter_booking_id->CurrentValue;
        ...
        $this->payment_number->CurrentValue = null;
        $this->payment_number->OldValue = $this->payment_number->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_payment")) {
            $this->payment->setOldValue($CurrentForm->getValue("o_payment"));
        }

        // Check field name 'payment_number' first before field var 'x_payment_number'
        $val = $CurrentForm->hasValue("payment_number") ? $CurrentForm->getValue("payment_number") : $CurrentForm->getValue("x_payment_number");
        if (!$this->payment_number->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->payment_number->Visible = false; // Disable update for API request
            } else {
                $this->payment_number->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_payment_number")) {
            $this->payment_number->setOldValue($CurrentForm->getValue("o_payment_number"));
        }
        ...
        // Check field name 'payment_inverter_booking_id' first before field var 'x_payment_inverter_booking_id'
        $val = $CurrentForm->hasValue("payment_inverter_booking_id") ? $CurrentForm->getValue("payment_inverter_booking_id") : $CurrentForm->getValue("x_payment_inverter_booking_id");
        if (!$this->payment_inverter_booking_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->payment_inverter_booking_id->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->payment_inverter_booking_id->CurrentValue = $this->payment_inverter_booking_id->FormValue;
        }
        $this->asset_id->CurrentValue = $this->asset_id->FormValue;
        $this->payment->CurrentValue = $this->payment->FormValue;
        $this->payment_number->CurrentValue = $this->payment_number->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['payment_inverter_booking_id'] = $this->payment_inverter_booking_id->CurrentValue;
        ...
        $row['payment_number'] = $this->payment_number->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->payment_number->EditValue = HtmlEncode($this->payment_number->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->payment_number->Required) {
            if (!$this->payment_number->IsDetailKey && EmptyValue($this->payment_number->FormValue)) {
                $this->payment_number->addErrorMessage(str_replace("%s", $this->payment_number->caption(), $this->payment_number->RequiredErrorMessage));
            }
        }
    ...
    protected function deleteRows()
    {
        ...
        foreach ($rsold as $row) {
            ...
            $thisKey .= $row['payment_inverter_booking_id'];
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            $masterTable = Container("invertor_booking");
            ...
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "invertor_booking", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'payment_inverter_booking'
        ...
        // payment
        $this->payment->setDbValueDef($rsnew, $this->payment->CurrentValue, 0, false);

        // payment_number
        $this->payment_number->setDbValueDef($rsnew, $this->payment_number->CurrentValue, "", false);
    ...
    protected function setupMasterParms()
    {
        ...
        if ($masterTblVar == "invertor_booking") {
            $masterTbl = Container("invertor_booking");
            $this->member_id->Visible = false;
