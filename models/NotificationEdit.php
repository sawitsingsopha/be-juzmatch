 ...
 * Page class
 */
class NotificationEdit extends Notification
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NotificationEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("notification_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->notification_id->setQueryStringValue($keyValue);
                $this->notification_id->setOldValue($this->notification_id->QueryStringValue);
            } elseif (Post("notification_id") !== null) {
                $this->notification_id->setFormValue(Post("notification_id"));
                $this->notification_id->setOldValue($this->notification_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("notification_id") ?? Route("notification_id")) !== null) {
                    $this->notification_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->notification_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("notificationlist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "notificationlist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->is_read->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->is_read->setFormValue($val, true, $validate);
            }
        }

        // Check field name 'notification_id' first before field var 'x_notification_id'
        $val = $CurrentForm->hasValue("notification_id") ? $CurrentForm->getValue("notification_id") : $CurrentForm->getValue("x_notification_id");
        if (!$this->notification_id->IsDetailKey) {
            $this->notification_id->setFormValue($val);
        }
    }
    ...
    {
        global $CurrentForm;
        $this->notification_id->CurrentValue = $this->notification_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // notification_id
        $this->notification_id->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->is_read->EditValue = HtmlEncode($this->is_read->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->is_read->Required) {
            if (!$this->is_read->IsDetailKey && EmptyValue($this->is_read->FormValue)) {
                $this->is_read->addErrorMessage(str_replace("%s", $this->is_read->caption(), $this->is_read->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->is_read->FormValue)) {
            $this->is_read->addErrorMessage($this->is_read->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // is_read
            $this->is_read->setDbValueDef($rsnew, $this->is_read->CurrentValue, 0, $this->is_read->ReadOnly);
