 ...
 * Page class
 */
class InvestorList extends Investor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InvestorList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "investoradd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "investordelete";
        $this->MultiUpdateUrl = "investorupdate";
    ...
    public function run()
    {
        ...
        $this->image_profile->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "finvestorsrch");
        }
        ...
        $filterList = Concat($filterList, $this->last_change_password->AdvancedSearch->toJson(), ","); // Field last_change_password
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "finvestorsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->last_change_password->AdvancedSearch->SearchValue = @$filter["x_last_change_password"];
        $this->last_change_password->AdvancedSearch->SearchOperator = @$filter["z_last_change_password"];
        $this->last_change_password->AdvancedSearch->SearchCondition = @$filter["v_last_change_password"];
        $this->last_change_password->AdvancedSearch->SearchValue2 = @$filter["y_last_change_password"];
        $this->last_change_password->AdvancedSearch->SearchOperator2 = @$filter["w_last_change_password"];
        $this->last_change_password->AdvancedSearch->save();
    }
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->last_change_password, $default, false); // last_change_password
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->last_change_password->AdvancedSearch->save(); // last_change_password
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->last_change_password->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->last_change_password->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->last_change_password->AdvancedSearch->load();
    }
    ...
    protected function setupListOptions()
    {
        ...
        $item = &$this->ListOptions->add("detail_investor_verify");
        $item->CssClass = "text-nowrap";
        ...
        $item = &$this->ListOptions->add("detail_invertor_booking");
        $item->CssClass = "text-nowrap";
        ...
        $item = &$this->ListOptions->add("detail_inverter_asset");
        $item->CssClass = "text-nowrap";
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("investor_verify");
        $pages->add("invertor_booking");
        $pages->add("inverter_asset");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finvestorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finvestorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_investor_verify"
        $opt = $this->ListOptions["detail_investor_verify"];
        if ($Security->allowList(CurrentProjectID() . 'investor_verify')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("investor_verify", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("investorverifylist?" . Config("TABLE_SHOW_MASTER") . "=investor&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("InvestorVerifyGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'investor')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=investor_verify");
        ...
        // "detail_invertor_booking"
        $opt = $this->ListOptions["detail_invertor_booking"];
        if ($Security->allowList(CurrentProjectID() . 'invertor_booking')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("invertor_booking", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("invertorbookinglist?" . Config("TABLE_SHOW_MASTER") . "=investor&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("InvertorBookingGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'investor')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=invertor_booking");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
        ...
        // "detail_inverter_asset"
        $opt = $this->ListOptions["detail_inverter_asset"];
        if ($Security->allowList(CurrentProjectID() . 'inverter_asset')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("inverter_asset", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("inverterassetlist?" . Config("TABLE_SHOW_MASTER") . "=investor&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("InverterAssetGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'investor')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=inverter_asset");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"finvestorsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"finvestorsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="finvestorlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->last_change_password->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->last_change_password->AdvancedSearch->SearchValue != "" || $this->last_change_password->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        // verify_key
        $this->verify_key->CellCssStyle = "white-space: nowrap;";

        // verify_status
        $this->verify_status->CellCssStyle = "white-space: nowrap;";

        // verify_date
        $this->verify_date->CellCssStyle = "white-space: nowrap;";

        // verify_ip
        $this->verify_ip->CellCssStyle = "white-space: nowrap;";

        // pipedrive_people_id
        $this->pipedrive_people_id->CellCssStyle = "white-space: nowrap;";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="finvestorlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"finvestorsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
