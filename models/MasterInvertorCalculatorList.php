 ...
 * Page class
 */
class MasterInvertorCalculatorList extends MasterInvertorCalculator
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterInvertorCalculatorList";
    ...
    public $FormName = "fmaster_invertor_calculatorlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "masterinvertorcalculatoradd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "masterinvertorcalculatordelete";
        $this->MultiUpdateUrl = "masterinvertorcalculatorupdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->investor_contract_period, $ctrl); // investor_contract_period
            $this->updateSort($this->investor_mortgage_without_house, $ctrl); // investor_mortgage_without_house
            $this->updateSort($this->invertor_mortgage_with_house, $ctrl); // invertor_mortgage_with_house
            $this->updateSort($this->invertor_mortgage_cash_without, $ctrl); // invertor_mortgage_cash_without
            $this->updateSort($this->invertor_mortgage_cash_with, $ctrl); // invertor_mortgage_cash_with
            $this->updateSort($this->invertor_dsr_ratio, $ctrl); // invertor_dsr_ratio
            $this->updateSort($this->invertor_monthly_payment, $ctrl); // invertor_monthly_payment
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->master_invertor_calculator_id->setSort("");
                $this->investor_contract_period->setSort("");
                $this->investor_mortgage_without_house->setSort("");
                $this->invertor_mortgage_with_house->setSort("");
                $this->invertor_mortgage_cash_without->setSort("");
                $this->invertor_mortgage_cash_with->setSort("");
                $this->invertor_dsr_ratio->setSort("");
                $this->invertor_monthly_payment->setSort("");
                $this->cip->setSort("");
                $this->cdate->setSort("");
                $this->cuser->setSort("");
                $this->uip->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmaster_invertor_calculatorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmaster_invertor_calculatorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->master_invertor_calculator_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("investor_contract_period", $this->createColumnOption("investor_contract_period"));
            $option->add("investor_mortgage_without_house", $this->createColumnOption("investor_mortgage_without_house"));
            $option->add("invertor_mortgage_with_house", $this->createColumnOption("invertor_mortgage_with_house"));
            $option->add("invertor_mortgage_cash_without", $this->createColumnOption("invertor_mortgage_cash_without"));
            $option->add("invertor_mortgage_cash_with", $this->createColumnOption("invertor_mortgage_cash_with"));
            $option->add("invertor_dsr_ratio", $this->createColumnOption("invertor_dsr_ratio"));
            $option->add("invertor_monthly_payment", $this->createColumnOption("invertor_monthly_payment"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmaster_invertor_calculatorsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmaster_invertor_calculatorsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmaster_invertor_calculatorlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
