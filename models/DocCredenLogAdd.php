 ...
 * Page class
 */
class DocCredenLogAdd extends DocCredenLog
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocCredenLogAdd";
    ...
    public function run()
    {
        ...
        $this->pdfbase64->setVisibility();
        $this->request_header->setVisibility();
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("doccredenloglist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "doccredenloglist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "doccredenlogview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->pdfbase64->CurrentValue = null;
        $this->pdfbase64->OldValue = $this->pdfbase64->CurrentValue;
        $this->request_header->CurrentValue = null;
        $this->request_header->OldValue = $this->request_header->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'pdfbase64' first before field var 'x_pdfbase64'
        $val = $CurrentForm->hasValue("pdfbase64") ? $CurrentForm->getValue("pdfbase64") : $CurrentForm->getValue("x_pdfbase64");
        if (!$this->pdfbase64->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->pdfbase64->Visible = false; // Disable update for API request
            } else {
                $this->pdfbase64->setFormValue($val);
            }
        }

        // Check field name 'request_header' first before field var 'x_request_header'
        $val = $CurrentForm->hasValue("request_header") ? $CurrentForm->getValue("request_header") : $CurrentForm->getValue("x_request_header");
        if (!$this->request_header->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->request_header->Visible = false; // Disable update for API request
            } else {
                $this->request_header->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->pdfbase64->CurrentValue = $this->pdfbase64->FormValue;
        $this->request_header->CurrentValue = $this->request_header->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['pdfbase64'] = $this->pdfbase64->CurrentValue;
        $row['request_header'] = $this->request_header->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        // pdfbase64
        $this->pdfbase64->RowCssClass = "row";

        // request_header
        $this->request_header->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // pdfbase64
            $this->pdfbase64->ViewValue = $this->pdfbase64->CurrentValue;
            $this->pdfbase64->ViewCustomAttributes = "";

            // request_header
            $this->request_header->ViewValue = $this->request_header->CurrentValue;
            $this->request_header->ViewCustomAttributes = "";
            ...
            // pdfbase64
            $this->pdfbase64->LinkCustomAttributes = "";
            $this->pdfbase64->HrefValue = "";

            // request_header
            $this->request_header->LinkCustomAttributes = "";
            $this->request_header->HrefValue = "";
            ...
            // pdfbase64
            $this->pdfbase64->setupEditAttributes();
            $this->pdfbase64->EditCustomAttributes = "";
            $this->pdfbase64->EditValue = HtmlEncode($this->pdfbase64->CurrentValue);
            $this->pdfbase64->PlaceHolder = RemoveHtml($this->pdfbase64->caption());

            // request_header
            $this->request_header->setupEditAttributes();
            $this->request_header->EditCustomAttributes = "";
            $this->request_header->EditValue = HtmlEncode($this->request_header->CurrentValue);
            $this->request_header->PlaceHolder = RemoveHtml($this->request_header->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->pdfbase64->Required) {
            if (!$this->pdfbase64->IsDetailKey && EmptyValue($this->pdfbase64->FormValue)) {
                $this->pdfbase64->addErrorMessage(str_replace("%s", $this->pdfbase64->caption(), $this->pdfbase64->RequiredErrorMessage));
            }
        }
        if ($this->request_header->Required) {
            if (!$this->request_header->IsDetailKey && EmptyValue($this->request_header->FormValue)) {
                $this->request_header->addErrorMessage(str_replace("%s", $this->request_header->caption(), $this->request_header->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // pdfbase64
        $this->pdfbase64->setDbValueDef($rsnew, $this->pdfbase64->CurrentValue, null, false);

        // request_header
        $this->request_header->setDbValueDef($rsnew, $this->request_header->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("doccredenloglist"), "", $this->TableVar, true);
