...
/**
 * Table class for investor
 */
class Investor extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->facebook_id->Sortable = false; // Allow sort
        ...
        $this->line_id->Sortable = false; // Allow sort
        ...
        $this->google_id->Sortable = false; // Allow sort
        ...
        $this->type->Lookup = new Lookup('type', 'investor', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->isactive->Lookup = new Lookup('isactive', 'investor', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->isbuyer->Lookup = new Lookup('isbuyer', 'investor', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->isinvertor->Lookup = new Lookup('isinvertor', 'investor', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->issale->Lookup = new Lookup('issale', 'investor', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->resetTimestamp->Sortable = false; // Allow sort
        ...
        $this->resetKeyTimestamp->Sortable = false; // Allow sort
        ...
        $this->reset_password_code->Sortable = false; // Allow sort
        ...
        $this->image_profile->Sortable = false; // Allow sort
        ...
        $this->verify_key->Sortable = false; // Allow sort
        ...
        $this->verify_status->Sortable = false; // Allow sort
        ...
        $this->verify_date->Sortable = false; // Allow sort
        ...
        $this->verify_ip->Sortable = false; // Allow sort
        ...
        $this->pipedrive_people_id->Sortable = false; // Allow sort
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "investor_verify") {
            $detailUrl = Container("investor_verify")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "invertor_booking") {
            $detailUrl = Container("invertor_booking")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "inverter_asset") {
            $detailUrl = Container("inverter_asset")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "investorlist";
        }
        return $detailUrl;
    }
    ...
    public function getSqlWhere() // Where
    {
        ...
        $this->DefaultFilter = "`isinvertor`=1";
    ...
    {
        // Cascade Update detail table 'investor_verify'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("investor_verify")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'juzmatch1'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("investor_verify")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("investor_verify")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("investor_verify")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'invertor_booking'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['member_id']) && $rsold['member_id'] != $rs['member_id'])) { // Update detail field 'member_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("invertor_booking")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'invertor_booking_id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("invertor_booking")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("invertor_booking")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("invertor_booking")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'inverter_asset'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['member_id']) && $rsold['member_id'] != $rs['member_id'])) { // Update detail field 'member_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("inverter_asset")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'juzmatch1'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                ...
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("inverter_asset")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("inverter_asset")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("inverter_asset")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'investor_verify'
        $dtlrows = Container("investor_verify")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "juzmatch1"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("investor_verify")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("investor_verify")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("investor_verify")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'invertor_booking'
        $dtlrows = Container("invertor_booking")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("invertor_booking")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("invertor_booking")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("invertor_booking")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'inverter_asset'
        $dtlrows = Container("inverter_asset")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "juzmatch1"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("inverter_asset")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("inverter_asset")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("inverter_asset")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("investorlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "investoredit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "investoradd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("investorview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("investorview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "investoradd?" . $this->getUrlParm($parm);
        } else {
            $url = "investoradd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("investoredit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("investoredit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("investoradd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("investoradd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("investordelete", $this->getUrlParm());
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->isinvertor);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->isinvertor);
