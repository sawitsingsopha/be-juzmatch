 ...
 * Page class
 */
class BuyerSaveBuyAssetAdd extends BuyerSaveBuyAsset
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'buyer_save_buy_asset';

    // Page object name
    public $PageObjName = "BuyerSaveBuyAssetAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (buyer_save_buy_asset)
        if (!isset($GLOBALS["buyer_save_buy_asset"]) || get_class($GLOBALS["buyer_save_buy_asset"]) == PROJECT_NAMESPACE . "buyer_save_buy_asset") {
            $GLOBALS["buyer_save_buy_asset"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'buyer_save_buy_asset');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("buyer_save_buy_asset");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['buyer_save_buy_asset_id'];
        }
        return $key;
    }
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->buyer_save_buy_asset_id->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->asser_id);
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("buyer_save_buy_asset_id") ?? Route("buyer_save_buy_asset_id")) !== null) {
                $this->buyer_save_buy_asset_id->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("buyersavebuyassetlist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "buyersavebuyassetlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "buyersavebuyassetview") {
    ...
    {
        $this->buyer_save_buy_asset_id->CurrentValue = null;
        $this->buyer_save_buy_asset_id->OldValue = $this->buyer_save_buy_asset_id->CurrentValue;
        $this->member_id->CurrentValue = null;
        $this->member_id->OldValue = $this->member_id->CurrentValue;
        $this->asser_id->CurrentValue = null;
        $this->asser_id->OldValue = $this->asser_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'asser_id' first before field var 'x_asser_id'
        $val = $CurrentForm->hasValue("asser_id") ? $CurrentForm->getValue("asser_id") : $CurrentForm->getValue("x_asser_id");
        if (!$this->asser_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->asser_id->Visible = false; // Disable update for API request
            } else {
                $this->asser_id->setFormValue($val);
            }
        }
        ...
        // Check field name 'buyer_save_buy_asset_id' first before field var 'x_buyer_save_buy_asset_id'
        $val = $CurrentForm->hasValue("buyer_save_buy_asset_id") ? $CurrentForm->getValue("buyer_save_buy_asset_id") : $CurrentForm->getValue("x_buyer_save_buy_asset_id");
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->asser_id->CurrentValue = $this->asser_id->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['buyer_save_buy_asset_id'] = $this->buyer_save_buy_asset_id->CurrentValue;
        $row['member_id'] = $this->member_id->CurrentValue;
        $row['asser_id'] = $this->asser_id->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        $this->buyer_save_buy_asset_id->RowCssClass = "row";

        // member_id
        $this->member_id->RowCssClass = "row";

        // asser_id
        $this->asser_id->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = trim(strval($this->asser_id->CurrentValue));
            if ($curVal != "") {
                $this->asser_id->ViewValue = $this->asser_id->lookupCacheOption($curVal);
            } else {
                $this->asser_id->ViewValue = $this->asser_id->Lookup !== null && is_array($this->asser_id->lookupOptions()) ? $curVal : null;
            }
            if ($this->asser_id->ViewValue !== null) { // Load from cache
                $this->asser_id->EditValue = array_values($this->asser_id->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`asset_id`" . SearchString("=", $this->asser_id->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->asser_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->asser_id->EditValue = $arwrk;
            }
            $this->asser_id->PlaceHolder = RemoveHtml($this->asser_id->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->asser_id->Required) {
            if (!$this->asser_id->IsDetailKey && EmptyValue($this->asser_id->FormValue)) {
                $this->asser_id->addErrorMessage(str_replace("%s", $this->asser_id->caption(), $this->asser_id->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // asser_id
        $this->asser_id->setDbValueDef($rsnew, $this->asser_id->CurrentValue, 0, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("buyersavebuyassetlist"), "", $this->TableVar, true);
