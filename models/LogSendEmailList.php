 ...
 * Page class
 */
class LogSendEmailList extends LogSendEmail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogSendEmailList";
    ...
    public $FormName = "flog_send_emaillist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "logsendemailadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "logsendemaildelete";
        $this->MultiUpdateUrl = "logsendemailupdate";
    ...
    public function run()
    {
        ...
        $this->log_email_id->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "flog_send_emailsrch");
        }
        $filterList = Concat($filterList, $this->log_email_id->AdvancedSearch->toJson(), ","); // Field log_email_id
        $filterList = Concat($filterList, $this->cc->AdvancedSearch->toJson(), ","); // Field cc
        ...
        $filterList = Concat($filterList, $this->email_from->AdvancedSearch->toJson(), ","); // Field email_from
        $filterList = Concat($filterList, $this->email_to->AdvancedSearch->toJson(), ","); // Field email_to
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "flog_send_emailsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field log_email_id
        $this->log_email_id->AdvancedSearch->SearchValue = @$filter["x_log_email_id"];
        $this->log_email_id->AdvancedSearch->SearchOperator = @$filter["z_log_email_id"];
        $this->log_email_id->AdvancedSearch->SearchCondition = @$filter["v_log_email_id"];
        $this->log_email_id->AdvancedSearch->SearchValue2 = @$filter["y_log_email_id"];
        $this->log_email_id->AdvancedSearch->SearchOperator2 = @$filter["w_log_email_id"];
        $this->log_email_id->AdvancedSearch->save();

        // Field cc
        $this->cc->AdvancedSearch->SearchValue = @$filter["x_cc"];
        $this->cc->AdvancedSearch->SearchOperator = @$filter["z_cc"];
        $this->cc->AdvancedSearch->SearchCondition = @$filter["v_cc"];
        $this->cc->AdvancedSearch->SearchValue2 = @$filter["y_cc"];
        $this->cc->AdvancedSearch->SearchOperator2 = @$filter["w_cc"];
        $this->cc->AdvancedSearch->save();
        ...
        // Field email_from
        $this->email_from->AdvancedSearch->SearchValue = @$filter["x_email_from"];
        $this->email_from->AdvancedSearch->SearchOperator = @$filter["z_email_from"];
        $this->email_from->AdvancedSearch->SearchCondition = @$filter["v_email_from"];
        $this->email_from->AdvancedSearch->SearchValue2 = @$filter["y_email_from"];
        $this->email_from->AdvancedSearch->SearchOperator2 = @$filter["w_email_from"];
        $this->email_from->AdvancedSearch->save();

        // Field email_to
        $this->email_to->AdvancedSearch->SearchValue = @$filter["x_email_to"];
        $this->email_to->AdvancedSearch->SearchOperator = @$filter["z_email_to"];
        $this->email_to->AdvancedSearch->SearchCondition = @$filter["v_email_to"];
        $this->email_to->AdvancedSearch->SearchValue2 = @$filter["y_email_to"];
        $this->email_to->AdvancedSearch->SearchOperator2 = @$filter["w_email_to"];
        $this->email_to->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->cip;
        $searchFlds[] = &$this->email_from;
        $searchFlds[] = &$this->email_to;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->log_email_id, $ctrl); // log_email_id
            $this->updateSort($this->cc, $ctrl); // cc
            ...
            $this->updateSort($this->email_from, $ctrl); // email_from
            $this->updateSort($this->email_to, $ctrl); // email_to
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->cuser->setSort("");
                $this->cip->setSort("");
                $this->email_from->setSort("");
                $this->email_to->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flog_send_emaillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flog_send_emaillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->log_email_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("log_email_id", $this->createColumnOption("log_email_id"));
            $option->add("cc", $this->createColumnOption("cc"));
            ...
            $option->add("email_from", $this->createColumnOption("email_from"));
            $option->add("email_to", $this->createColumnOption("email_to"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"flog_send_emailsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"flog_send_emailsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="flog_send_emaillist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // log_email_id
            $this->log_email_id->LinkCustomAttributes = "";
            $this->log_email_id->HrefValue = "";
            $this->log_email_id->TooltipValue = "";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="flog_send_emaillist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flog_send_emailsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
