 ...
 * Page class
 */
class LogMemberScbList extends LogMemberScb
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogMemberScbList";
    ...
    public $FormName = "flog_member_scblist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "logmemberscbadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "logmemberscbdelete";
        $this->MultiUpdateUrl = "logmemberscbupdate";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "flog_member_scbsrch");
        }
        $filterList = Concat($filterList, $this->log_member_scb_id->AdvancedSearch->toJson(), ","); // Field log_member_scb_id
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "flog_member_scbsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field log_member_scb_id
        $this->log_member_scb_id->AdvancedSearch->SearchValue = @$filter["x_log_member_scb_id"];
        $this->log_member_scb_id->AdvancedSearch->SearchOperator = @$filter["z_log_member_scb_id"];
        $this->log_member_scb_id->AdvancedSearch->SearchCondition = @$filter["v_log_member_scb_id"];
        $this->log_member_scb_id->AdvancedSearch->SearchValue2 = @$filter["y_log_member_scb_id"];
        $this->log_member_scb_id->AdvancedSearch->SearchOperator2 = @$filter["w_log_member_scb_id"];
        $this->log_member_scb_id->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->refreshtoken;
        $searchFlds[] = &$this->auth_code;
        $searchFlds[] = &$this->_token;
        $searchFlds[] = &$this->state;
        $searchFlds[] = &$this->cip;
        $searchFlds[] = &$this->status;
        $searchFlds[] = &$this->at_expire_in;
        $searchFlds[] = &$this->rt_expire_in;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->log_member_scb_id, $ctrl); // log_member_scb_id
            ...
            $this->updateSort($this->decision_timestamp, $ctrl); // decision_timestamp
            $this->updateSort($this->deposit_amount, $ctrl); // deposit_amount
            $this->updateSort($this->due_date, $ctrl); // due_date
            $this->updateSort($this->rental_fee, $ctrl); // rental_fee
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->log_member_scb_id->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flog_member_scblist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flog_member_scblist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->log_member_scb_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("log_member_scb_id", $this->createColumnOption("log_member_scb_id"));
            ...
            $option->add("decision_timestamp", $this->createColumnOption("decision_timestamp"));
            $option->add("deposit_amount", $this->createColumnOption("deposit_amount"));
            $option->add("due_date", $this->createColumnOption("due_date"));
            $option->add("rental_fee", $this->createColumnOption("rental_fee"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"flog_member_scbsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"flog_member_scbsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="flog_member_scblist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="flog_member_scblist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flog_member_scbsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
