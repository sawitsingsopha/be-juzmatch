 ...
 * Page class
 */
class SaleAssetGrid extends SaleAsset
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SaleAssetGrid";
    ...
    public $FormName = "fsale_assetgrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->sale_asset_id->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_status_live") && $CurrentForm->hasValue("o_status_live") && $this->status_live->CurrentValue != $this->status_live->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->status_live->clearErrorMessage();
    }
    ...
    {
        $this->sale_asset_id->CurrentValue = null;
        $this->sale_asset_id->OldValue = $this->sale_asset_id->CurrentValue;
        ...
        $this->status_live->CurrentValue = 0;
        $this->status_live->OldValue = $this->status_live->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'status_live' first before field var 'x_status_live'
        $val = $CurrentForm->hasValue("status_live") ? $CurrentForm->getValue("status_live") : $CurrentForm->getValue("x_status_live");
        if (!$this->status_live->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->status_live->Visible = false; // Disable update for API request
            } else {
                $this->status_live->setFormValue($val, true, $validate);
            }
        }
        if ($CurrentForm->hasValue("o_status_live")) {
            $this->status_live->setOldValue($CurrentForm->getValue("o_status_live"));
        }

        // Check field name 'sale_asset_id' first before field var 'x_sale_asset_id'
        $val = $CurrentForm->hasValue("sale_asset_id") ? $CurrentForm->getValue("sale_asset_id") : $CurrentForm->getValue("x_sale_asset_id");
        if (!$this->sale_asset_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->sale_asset_id->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->sale_asset_id->CurrentValue = $this->sale_asset_id->FormValue;
        }
        $this->asset_id->CurrentValue = $this->asset_id->FormValue;
        $this->status_live->CurrentValue = $this->status_live->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['sale_asset_id'] = $this->sale_asset_id->CurrentValue;
        $row['asset_id'] = $this->asset_id->CurrentValue;
        $row['member_id'] = $this->member_id->CurrentValue;
        $row['status_live'] = $this->status_live->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // status_live
            $this->status_live->setupEditAttributes();
            $this->status_live->EditCustomAttributes = "";
            $this->status_live->EditValue = HtmlEncode($this->status_live->CurrentValue);
            $this->status_live->PlaceHolder = RemoveHtml($this->status_live->caption());
            if (strval($this->status_live->EditValue) != "" && is_numeric($this->status_live->EditValue)) {
                $this->status_live->EditValue = FormatNumber($this->status_live->EditValue, null);
                $this->status_live->OldValue = $this->status_live->EditValue;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->status_live->Required) {
            if (!$this->status_live->IsDetailKey && EmptyValue($this->status_live->FormValue)) {
                $this->status_live->addErrorMessage(str_replace("%s", $this->status_live->caption(), $this->status_live->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->status_live->FormValue)) {
            $this->status_live->addErrorMessage($this->status_live->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // status_live
            $this->status_live->setDbValueDef($rsnew, $this->status_live->CurrentValue, 0, $this->status_live->ReadOnly);

            // Check referential integrity for master table 'seller'
            $detailKeys = [];
            $keyValue = $rsnew['member_id'] ?? $rsold['member_id'];
            $detailKeys['member_id'] = $keyValue;
            $masterTable = Container("seller");
            ...
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "seller", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "seller") {
            $this->member_id->CurrentValue = $this->member_id->getSessionValue();
        }

        // Check referential integrity for master table 'sale_asset'
        ...
        // status_live
        $this->status_live->setDbValueDef($rsnew, $this->status_live->CurrentValue, 0, strval($this->status_live->CurrentValue) == "");
