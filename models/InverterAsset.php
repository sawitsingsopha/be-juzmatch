...
/**
 * Table class for inverter_asset
 */
class InverterAsset extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'inverter_asset';
        $this->TableName = 'inverter_asset';
        ...
        $this->status->Lookup = new Lookup('status', 'inverter_asset', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->cdate = new DbField(
            ...
            'cdate',
            '`cdate`',
            CastDateFieldForLike("`cdate`", 0, "juzmatch1"),
            135,
            19,
            0,
            false,
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "asset_config_schedule") {
            $detailUrl = Container("asset_config_schedule")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
        ...
        if ($this->getCurrentDetailTable() == "asset_schedule") {
            $detailUrl = Container("asset_schedule")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "inverterassetlist";
        }
        return $detailUrl;
    }
    ...
    {
        // Cascade Update detail table 'asset_config_schedule'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("asset_config_schedule")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'asset_config_schedule_id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("asset_config_schedule")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("asset_config_schedule")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("asset_config_schedule")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'asset_schedule'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['member_id']) && $rsold['member_id'] != $rs['member_id'])) { // Update detail field 'member_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("asset_schedule")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'asset_schedule_id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("asset_schedule")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("asset_schedule")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("asset_schedule")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'asset_config_schedule'
        $dtlrows = Container("asset_config_schedule")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB") . " AND " . "`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("asset_config_schedule")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("asset_config_schedule")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("asset_config_schedule")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'asset_schedule'
        $dtlrows = Container("asset_schedule")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB") . " AND " . "`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("asset_schedule")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("asset_schedule")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("asset_schedule")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("inverterassetlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "inverterassetview") {
            return $Language->phrase("View");
        } elseif ($pageName == "inverterassetedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "inverterassetadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "InverterAssetView";
            case Config("API_ADD_ACTION"):
                return "InverterAssetAdd";
            case Config("API_EDIT_ACTION"):
                return "InverterAssetEdit";
            case Config("API_DELETE_ACTION"):
                return "InverterAssetDelete";
            case Config("API_LIST_ACTION"):
                return "InverterAssetList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("inverterassetview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("inverterassetview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "inverterassetadd?" . $this->getUrlParm($parm);
        } else {
            $url = "inverterassetadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("inverterassetedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("inverterassetedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("inverterassetadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("inverterassetadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("inverterassetdelete", $this->getUrlParm());
    }
