 ...
 * Page class
 */
class LogMemberScbView extends LogMemberScb
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogMemberScbView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("log_member_scb_id") ?? Route("log_member_scb_id")) !== null) {
            $this->RecKey["log_member_scb_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("log_member_scb_id") ?? Route("log_member_scb_id")) !== null) {
                ...
                $this->RecKey["log_member_scb_id"] = $this->log_member_scb_id->QueryStringValue;
            } elseif (Post("log_member_scb_id") !== null) {
                $this->log_member_scb_id->setFormValue(Post("log_member_scb_id"));
                $this->RecKey["log_member_scb_id"] = $this->log_member_scb_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->log_member_scb_id->setQueryStringValue($keyValue);
                $this->RecKey["log_member_scb_id"] = $this->log_member_scb_id->QueryStringValue;
            } else {
                $returnUrl = "logmemberscblist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "logmemberscblist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "logmemberscblist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
