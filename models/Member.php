...
class Member extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->type->Lookup = new Lookup('type', 'member', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->isactive->Lookup = new Lookup('isactive', 'member', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->isbuyer->Lookup = new Lookup('isbuyer', 'member', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        $this->isbuyer->OptionCount = 1;
        ...
        $this->isinvertor->Lookup = new Lookup('isinvertor', 'member', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        $this->isinvertor->OptionCount = 1;
        ...
        $this->issale->Lookup = new Lookup('issale', 'member', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        $this->issale->OptionCount = 1;
        ...
        $this->isnotification->Lookup = new Lookup('isnotification', 'member', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        $this->isnotification->OptionCount = 1;
        ...
        $this->verify_date = new DbField(
            ...
            'verify_date',
            '`verify_date`',
            CastDateFieldForLike("`verify_date`", 0, "DB"),
            135,
            19,
            0,
            false,
        ...
        $this->reset_password_date = new DbField(
            ...
            'reset_password_date',
            '`reset_password_date`',
            CastDateFieldForLike("`reset_password_date`", 0, "DB"),
            135,
            19,
            0,
            false,
        ...
        $this->reset_password_email_date = new DbField(
            ...
            'reset_password_email_date',
            '`reset_password_email_date`',
            CastDateFieldForLike("`reset_password_email_date`", 0, "DB"),
            135,
            19,
            0,
            false,
        ...
        $this->last_login = new DbField(
            ...
            'last_login',
            '`last_login`',
            CastDateFieldForLike("`last_login`", 111, "DB"),
            135,
            19,
            111,
            false,
        ...
        $this->last_login->DefaultErrorMessage = str_replace("%s", DateFormat(111), $Language->phrase("IncorrectDate"));
        ...
        $this->cdate = new DbField(
            ...
            'cdate',
            '`cdate`',
            CastDateFieldForLike("`cdate`", 117, "DB"),
            135,
            19,
            117,
            false,
        ...
        $this->verify_phone_date = new DbField(
            ...
            'verify_phone_date',
            '`verify_phone_date`',
            CastDateFieldForLike("`verify_phone_date`", 0, "DB"),
            135,
            19,
            0,
            false,
        ...
        $this->last_change_password = new DbField(
            ...
            'last_change_password',
            '`last_change_password`',
            CastDateFieldForLike("`last_change_password`", 0, "DB"),
            135,
            19,
            0,
            false,
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "asset_favorite") {
            $detailUrl = Container("asset_favorite")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("address")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
        ...
        // Cascade Update detail table 'asset_favorite'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['member_id']) && $rsold['member_id'] != $rs['member_id'])) { // Update detail field 'member_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("asset_favorite")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                ...
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("asset_favorite")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("asset_favorite")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("asset_favorite")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("address")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        ...
        // Cascade delete detail table 'asset_favorite'
        $dtlrows = Container("asset_favorite")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("asset_favorite")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("asset_favorite")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("asset_favorite")->rowDeleted($dtlrow);
            }
        }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("memberlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "memberedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "memberadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("memberview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("memberview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "memberadd?" . $this->getUrlParm($parm);
        } else {
            $url = "memberadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("memberedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("memberedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("memberadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("memberadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("memberdelete", $this->getUrlParm());
    }
    ...
    public function renderListRow()
    {
        ...
        if ($this->image_profile->UseColorbox) {
            ...
            $this->image_profile->LinkAttrs["data-rel"] = "member_x_image_profile";
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->isbuyer);
                    $doc->exportCaption($this->isinvertor);
                    $doc->exportCaption($this->issale);
                    $doc->exportCaption($this->isnotification);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->isbuyer);
                        $doc->exportField($this->isinvertor);
                        $doc->exportField($this->issale);
                        $doc->exportField($this->isnotification);
