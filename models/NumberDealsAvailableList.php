 ...
 * Page class
 */
class NumberDealsAvailableList extends NumberDealsAvailable
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'number_deals_available';

    // Page object name
    public $PageObjName = "NumberDealsAvailableList";
    ...
    public $FormName = "fnumber_deals_availablelist";
    ...
    public function __construct()
    {
        ...
        // Table object (number_deals_available)
        if (!isset($GLOBALS["number_deals_available"]) || get_class($GLOBALS["number_deals_available"]) == PROJECT_NAMESPACE . "number_deals_available") {
            $GLOBALS["number_deals_available"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "numberdealsavailableadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "numberdealsavailabledelete";
        $this->MultiUpdateUrl = "numberdealsavailableupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'number_deals_available');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("number_deals_available");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->price_paid->setVisibility();
        $this->remaining_price->setVisibility();
        $this->expiration_date->setVisibility();
        $this->accrued_period_diff->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fnumber_deals_availablesrch");
        }
        ...
        $filterList = Concat($filterList, $this->price_paid->AdvancedSearch->toJson(), ","); // Field price_paid
        $filterList = Concat($filterList, $this->remaining_price->AdvancedSearch->toJson(), ","); // Field remaining_price
        $filterList = Concat($filterList, $this->expiration_date->AdvancedSearch->toJson(), ","); // Field expiration_date
        $filterList = Concat($filterList, $this->accrued_period_diff->AdvancedSearch->toJson(), ","); // Field accrued_period_diff
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fnumber_deals_availablesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field price_paid
        $this->price_paid->AdvancedSearch->SearchValue = @$filter["x_price_paid"];
        $this->price_paid->AdvancedSearch->SearchOperator = @$filter["z_price_paid"];
        $this->price_paid->AdvancedSearch->SearchCondition = @$filter["v_price_paid"];
        $this->price_paid->AdvancedSearch->SearchValue2 = @$filter["y_price_paid"];
        $this->price_paid->AdvancedSearch->SearchOperator2 = @$filter["w_price_paid"];
        $this->price_paid->AdvancedSearch->save();

        // Field remaining_price
        $this->remaining_price->AdvancedSearch->SearchValue = @$filter["x_remaining_price"];
        $this->remaining_price->AdvancedSearch->SearchOperator = @$filter["z_remaining_price"];
        $this->remaining_price->AdvancedSearch->SearchCondition = @$filter["v_remaining_price"];
        $this->remaining_price->AdvancedSearch->SearchValue2 = @$filter["y_remaining_price"];
        $this->remaining_price->AdvancedSearch->SearchOperator2 = @$filter["w_remaining_price"];
        $this->remaining_price->AdvancedSearch->save();

        // Field expiration_date
        $this->expiration_date->AdvancedSearch->SearchValue = @$filter["x_expiration_date"];
        $this->expiration_date->AdvancedSearch->SearchOperator = @$filter["z_expiration_date"];
        $this->expiration_date->AdvancedSearch->SearchCondition = @$filter["v_expiration_date"];
        $this->expiration_date->AdvancedSearch->SearchValue2 = @$filter["y_expiration_date"];
        $this->expiration_date->AdvancedSearch->SearchOperator2 = @$filter["w_expiration_date"];
        $this->expiration_date->AdvancedSearch->save();

        // Field accrued_period_diff
        $this->accrued_period_diff->AdvancedSearch->SearchValue = @$filter["x_accrued_period_diff"];
        $this->accrued_period_diff->AdvancedSearch->SearchOperator = @$filter["z_accrued_period_diff"];
        $this->accrued_period_diff->AdvancedSearch->SearchCondition = @$filter["v_accrued_period_diff"];
        $this->accrued_period_diff->AdvancedSearch->SearchValue2 = @$filter["y_accrued_period_diff"];
        $this->accrued_period_diff->AdvancedSearch->SearchOperator2 = @$filter["w_accrued_period_diff"];
        $this->accrued_period_diff->AdvancedSearch->save();
    }
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->price_paid, $default, false); // price_paid
        $this->buildSearchSql($where, $this->remaining_price, $default, false); // remaining_price
        $this->buildSearchSql($where, $this->expiration_date, $default, false); // expiration_date
        $this->buildSearchSql($where, $this->accrued_period_diff, $default, false); // accrued_period_diff
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->price_paid->AdvancedSearch->save(); // price_paid
            $this->remaining_price->AdvancedSearch->save(); // remaining_price
            $this->expiration_date->AdvancedSearch->save(); // expiration_date
            $this->accrued_period_diff->AdvancedSearch->save(); // accrued_period_diff
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->price_paid->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->remaining_price->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->expiration_date->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->accrued_period_diff->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->price_paid->AdvancedSearch->unsetSession();
        $this->remaining_price->AdvancedSearch->unsetSession();
        $this->expiration_date->AdvancedSearch->unsetSession();
        $this->accrued_period_diff->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->price_paid->AdvancedSearch->load();
        $this->remaining_price->AdvancedSearch->load();
        $this->expiration_date->AdvancedSearch->load();
        $this->accrued_period_diff->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->price_paid, $ctrl); // price_paid
            $this->updateSort($this->remaining_price, $ctrl); // remaining_price
            $this->updateSort($this->expiration_date, $ctrl); // expiration_date
            $this->updateSort($this->accrued_period_diff, $ctrl); // accrued_period_diff
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->price_paid->setSort("");
                $this->remaining_price->setSort("");
                $this->expiration_date->setSort("");
                $this->accrued_period_diff->setSort("");
            }
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_all_buyer_asset_schedule"
        $item = &$this->ListOptions->add("detail_all_buyer_asset_schedule");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'all_buyer_asset_schedule');
        $item->OnLeft = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("all_buyer_asset_schedule");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fnumber_deals_availablelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fnumber_deals_availablelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_all_buyer_asset_schedule"
        $opt = $this->ListOptions["detail_all_buyer_asset_schedule"];
        if ($Security->allowList(CurrentProjectID() . 'all_buyer_asset_schedule')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("all_buyer_asset_schedule", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("allbuyerassetschedulelist?" . Config("TABLE_SHOW_MASTER") . "=number_deals_available&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("price_paid", $this->createColumnOption("price_paid"));
            $option->add("remaining_price", $this->createColumnOption("remaining_price"));
            $option->add("expiration_date", $this->createColumnOption("expiration_date"));
            $option->add("accrued_period_diff", $this->createColumnOption("accrued_period_diff"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fnumber_deals_availablesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fnumber_deals_availablesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fnumber_deals_availablelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // price_paid
        if ($this->price_paid->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->price_paid->AdvancedSearch->SearchValue != "" || $this->price_paid->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // remaining_price
        if ($this->remaining_price->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->remaining_price->AdvancedSearch->SearchValue != "" || $this->remaining_price->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // expiration_date
        if ($this->expiration_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->expiration_date->AdvancedSearch->SearchValue != "" || $this->expiration_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // accrued_period_diff
        if ($this->accrued_period_diff->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->accrued_period_diff->AdvancedSearch->SearchValue != "" || $this->accrued_period_diff->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    protected function newRow()
    {
        ...
        $row['price_paid'] = null;
        $row['remaining_price'] = null;
        $row['expiration_date'] = null;
        $row['accrued_period_diff'] = null;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->price_paid->EditValue = HtmlEncode($this->price_paid->AdvancedSearch->SearchValue);
            ...
            $this->remaining_price->EditValue = HtmlEncode($this->remaining_price->AdvancedSearch->SearchValue);
            ...
            $this->expiration_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->expiration_date->AdvancedSearch->SearchValue, $this->expiration_date->formatPattern()), $this->expiration_date->formatPattern()));
            $this->expiration_date->PlaceHolder = RemoveHtml($this->expiration_date->caption());
            $this->expiration_date->setupEditAttributes();
            $this->expiration_date->EditCustomAttributes = "";
            $this->expiration_date->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->expiration_date->AdvancedSearch->SearchValue2, $this->expiration_date->formatPattern()), $this->expiration_date->formatPattern()));
            $this->expiration_date->PlaceHolder = RemoveHtml($this->expiration_date->caption());

            // accrued_period_diff
            $this->accrued_period_diff->setupEditAttributes();
            $this->accrued_period_diff->EditCustomAttributes = "";
            $this->accrued_period_diff->EditValue = HtmlEncode($this->accrued_period_diff->AdvancedSearch->SearchValue);
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckNumber($this->price_paid->AdvancedSearch->SearchValue)) {
            $this->price_paid->addErrorMessage($this->price_paid->getErrorMessage(false));
        }
        if (!CheckNumber($this->remaining_price->AdvancedSearch->SearchValue)) {
            $this->remaining_price->addErrorMessage($this->remaining_price->getErrorMessage(false));
        }
        if (!CheckDate($this->expiration_date->AdvancedSearch->SearchValue, $this->expiration_date->formatPattern())) {
            $this->expiration_date->addErrorMessage($this->expiration_date->getErrorMessage(false));
        }
        if (!CheckDate($this->expiration_date->AdvancedSearch->SearchValue2, $this->expiration_date->formatPattern())) {
            $this->expiration_date->addErrorMessage($this->expiration_date->getErrorMessage(false));
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fnumber_deals_availablelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fnumber_deals_availablesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->asset_price->Visible || $this->price_paid->Visible || $this->remaining_price->Visible || $this->expiration_date->Visible;
    }
