 ...
 * Page class
 */
class DocTempList extends DocTemp
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocTempList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "doctempadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "doctempdelete";
        $this->MultiUpdateUrl = "doctempupdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->doc_temp_type, $ctrl); // doc_temp_type
            $this->updateSort($this->doc_temp_name, $ctrl); // doc_temp_name
            $this->updateSort($this->doc_temp_file, $ctrl); // doc_temp_file
            $this->updateSort($this->active_status, $ctrl); // active_status
            $this->updateSort($this->esign_page1, $ctrl); // esign_page1
            $this->updateSort($this->esign_page2, $ctrl); // esign_page2
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`doc_temp_id` DESC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->doc_temp_id->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->doc_temp_id->setSort("DESC");
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->doc_temp_type->setSort("");
                $this->doc_temp_name->setSort("");
                $this->doc_temp_file->setSort("");
                $this->active_status->setSort("");
                $this->esign_page1->setSort("");
                $this->esign_page2->setSort("");
                $this->cdate->setSort("");
                $this->cuser->setSort("");
                $this->cip->setSort("");
                $this->udate->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdoc_templist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdoc_templist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->doc_temp_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("doc_temp_type", $this->createColumnOption("doc_temp_type"));
            $option->add("doc_temp_name", $this->createColumnOption("doc_temp_name"));
            $option->add("doc_temp_file", $this->createColumnOption("doc_temp_file"));
            $option->add("active_status", $this->createColumnOption("active_status"));
            $option->add("esign_page1", $this->createColumnOption("esign_page1"));
            $option->add("esign_page2", $this->createColumnOption("esign_page2"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdoc_tempsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdoc_tempsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdoc_templist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        ...
        $row['doc_temp_type'] = null;
        $row['doc_temp_name'] = null;
        $row['doc_temp_file'] = null;
        $row['active_status'] = null;
        $row['esign_page1'] = null;
        $row['esign_page2'] = null;
        $row['cdate'] = null;
        $row['cuser'] = null;
        $row['cip'] = null;
        $row['udate'] = null;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fdoc_templist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
