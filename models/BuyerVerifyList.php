 ...
 * Page class
 */
class BuyerVerifyList extends BuyerVerify
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BuyerVerifyList";
    ...
    public $FormName = "fbuyer_verifylist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "buyerverifydelete";
        $this->MultiUpdateUrl = "buyerverifyupdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->installment_min, $ctrl); // installment_min
            $this->updateSort($this->installment_max, $ctrl); // installment_max
            $this->updateSort($this->num_bedroom, $ctrl); // num_bedroom
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->buyer_verify_id->setSort("");
                $this->category_id->setSort("");
                $this->member_id->setSort("");
                $this->installment_min->setSort("");
                $this->installment_max->setSort("");
                $this->num_bedroom->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuyer_verifylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbuyer_verifylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->buyer_verify_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("installment_min", $this->createColumnOption("installment_min"));
            $option->add("installment_max", $this->createColumnOption("installment_max"));
            $option->add("num_bedroom", $this->createColumnOption("num_bedroom"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbuyer_verifysrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbuyer_verifysrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbuyer_verifylist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    {
        $row = [];
        $row['buyer_verify_id'] = null;
        $row['category_id'] = null;
        $row['member_id'] = null;
        $row['installment_min'] = null;
        $row['installment_max'] = null;
        $row['num_bedroom'] = null;
        $row['latitude'] = null;
        $row['longitude'] = null;
        $row['cdate'] = null;
        $row['cuser'] = null;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fbuyer_verifylist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
