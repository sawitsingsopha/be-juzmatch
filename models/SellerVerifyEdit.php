 ...
 * Page class
 */
class SellerVerifyEdit extends SellerVerify
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SellerVerifyEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("seller_verify_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->seller_verify_id->setQueryStringValue($keyValue);
                $this->seller_verify_id->setOldValue($this->seller_verify_id->QueryStringValue);
            } elseif (Post("seller_verify_id") !== null) {
                $this->seller_verify_id->setFormValue(Post("seller_verify_id"));
                $this->seller_verify_id->setOldValue($this->seller_verify_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("seller_verify_id") ?? Route("seller_verify_id")) !== null) {
                    $this->seller_verify_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->seller_verify_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("sellerverifylist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "sellerverifylist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'seller_verify_id' first before field var 'x_seller_verify_id'
        $val = $CurrentForm->hasValue("seller_verify_id") ? $CurrentForm->getValue("seller_verify_id") : $CurrentForm->getValue("x_seller_verify_id");
        if (!$this->seller_verify_id->IsDetailKey) {
            $this->seller_verify_id->setFormValue($val);
        }
    }
    ...
    {
        global $CurrentForm;
        $this->seller_verify_id->CurrentValue = $this->seller_verify_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // seller_verify_id
        $this->seller_verify_id->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("sellerverifylist"), "", $this->TableVar, true);
        $pageId = "edit";
