 ...
 * Page class
 */
class DocTempEdit extends DocTemp
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocTempEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("doc_temp_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->doc_temp_id->setQueryStringValue($keyValue);
                $this->doc_temp_id->setOldValue($this->doc_temp_id->QueryStringValue);
            } elseif (Post("doc_temp_id") !== null) {
                $this->doc_temp_id->setFormValue(Post("doc_temp_id"));
                $this->doc_temp_id->setOldValue($this->doc_temp_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->doc_temp_id->IsDetailKey) {
            $this->doc_temp_id->setFormValue($val);
        }
		$this->doc_temp_file->OldUploadPath = "/upload/Doc_temp/";
		$this->doc_temp_file->UploadPath = $this->doc_temp_file->OldUploadPath;
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // doc_temp_type
            $this->doc_temp_type->setDbValueDef($rsnew, $this->doc_temp_type->CurrentValue, null, $this->doc_temp_type->ReadOnly);

            // doc_temp_name
            $this->doc_temp_name->setDbValueDef($rsnew, $this->doc_temp_name->CurrentValue, null, $this->doc_temp_name->ReadOnly);

            // doc_temp_file
            if ($this->doc_temp_file->Visible && !$this->doc_temp_file->ReadOnly && !$this->doc_temp_file->Upload->KeepFile) {
                $this->doc_temp_file->Upload->DbValue = $rsold['doc_temp_file']; // Get original value
            ...
            // esign_page1
            $this->esign_page1->setDbValueDef($rsnew, $this->esign_page1->CurrentValue, null, $this->esign_page1->ReadOnly);

            // esign_page2
            $this->esign_page2->setDbValueDef($rsnew, $this->esign_page2->CurrentValue, null, $this->esign_page2->ReadOnly);
            ...
            if ($this->doc_temp_file->Visible && !$this->doc_temp_file->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->doc_temp_file->Upload->FileName)) {
                    ...
                    $this->doc_temp_file->setDbValueDef($rsnew, $this->doc_temp_file->Upload->FileName, null, $this->doc_temp_file->ReadOnly);
                }
            }
