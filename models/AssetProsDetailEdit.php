 ...
 * Page class
 */
class AssetProsDetailEdit extends AssetProsDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AssetProsDetailEdit";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "assetprosdetailview") {
                        $row["view"] = "1";
                    }
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("asset_pros_detail_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->asset_pros_detail_id->setQueryStringValue($keyValue);
                $this->asset_pros_detail_id->setOldValue($this->asset_pros_detail_id->QueryStringValue);
            } elseif (Post("asset_pros_detail_id") !== null) {
                $this->asset_pros_detail_id->setFormValue(Post("asset_pros_detail_id"));
                $this->asset_pros_detail_id->setOldValue($this->asset_pros_detail_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("asset_pros_detail_id") ?? Route("asset_pros_detail_id")) !== null) {
                    $this->asset_pros_detail_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("assetprosdetaillist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "assetprosdetaillist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->asset_pros_detail_id->IsDetailKey) {
    ...
    public function renderRow()
    {
        ...
        $this->asset_pros_detail_id->RowCssClass = "row";

        // asset_pros_id
        $this->asset_pros_id->RowCssClass = "row";
        ...
        // group_type
        $this->group_type->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->asset_id->getSessionValue() != "") {
                ...
                if ($this->asset_id->ViewValue !== null) { // Load from cache
                    ...
                    $lookupFilter = $lookupFilter->bindTo($this);
                    $sqlWrk = $this->asset_id->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("assetprosdetaillist"), "", $this->TableVar, true);
        $pageId = "edit";
