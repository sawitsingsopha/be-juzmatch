 ...
 * Page class
 */
class MemberScbDetailLogEdit extends MemberScbDetailLog
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MemberScbDetailLogEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("member_scb_detail_log_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->member_scb_detail_log_id->setQueryStringValue($keyValue);
                $this->member_scb_detail_log_id->setOldValue($this->member_scb_detail_log_id->QueryStringValue);
            } elseif (Post("member_scb_detail_log_id") !== null) {
                $this->member_scb_detail_log_id->setFormValue(Post("member_scb_detail_log_id"));
                $this->member_scb_detail_log_id->setOldValue($this->member_scb_detail_log_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->member_scb_detail_log_id->IsDetailKey) {
            $this->member_scb_detail_log_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->member_scb_detail_log_id->CurrentValue = $this->member_scb_detail_log_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->member_scb_detail_log_id->Required) {
            if (!$this->member_scb_detail_log_id->IsDetailKey && EmptyValue($this->member_scb_detail_log_id->FormValue)) {
                $this->member_scb_detail_log_id->addErrorMessage(str_replace("%s", $this->member_scb_detail_log_id->caption(), $this->member_scb_detail_log_id->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // member_scb_id
            $this->member_scb_id->setDbValueDef($rsnew, $this->member_scb_id->CurrentValue, null, $this->member_scb_id->ReadOnly);

            // paid_amt
            $this->paid_amt->setDbValueDef($rsnew, $this->paid_amt->CurrentValue, null, $this->paid_amt->ReadOnly);

            // pay_date
            $this->pay_date->setDbValueDef($rsnew, UnFormatDateTime($this->pay_date->CurrentValue, $this->pay_date->formatPattern()), null, $this->pay_date->ReadOnly);
