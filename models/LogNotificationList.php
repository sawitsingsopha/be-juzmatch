 ...
 * Page class
 */
class LogNotificationList extends LogNotification
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'log_notification';

    // Page object name
    public $PageObjName = "LogNotificationList";
    ...
    public $FormName = "flog_notificationlist";
    ...
    public function __construct()
    {
        ...
        // Table object (log_notification)
        if (!isset($GLOBALS["log_notification"]) || get_class($GLOBALS["log_notification"]) == PROJECT_NAMESPACE . "log_notification") {
            $GLOBALS["log_notification"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "lognotificationadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "lognotificationdelete";
        $this->MultiUpdateUrl = "lognotificationupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'log_notification');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("log_notification");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->log_notification_id->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->log_notification_id->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "flog_notificationsrch");
        }
        $filterList = Concat($filterList, $this->log_notification_id->AdvancedSearch->toJson(), ","); // Field log_notification_id
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "flog_notificationsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field log_notification_id
        $this->log_notification_id->AdvancedSearch->SearchValue = @$filter["x_log_notification_id"];
        $this->log_notification_id->AdvancedSearch->SearchOperator = @$filter["z_log_notification_id"];
        $this->log_notification_id->AdvancedSearch->SearchCondition = @$filter["v_log_notification_id"];
        $this->log_notification_id->AdvancedSearch->SearchValue2 = @$filter["y_log_notification_id"];
        $this->log_notification_id->AdvancedSearch->SearchOperator2 = @$filter["w_log_notification_id"];
        $this->log_notification_id->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->log_notification_id, $ctrl); // log_notification_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->log_notification_id->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flog_notificationlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flog_notificationlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->log_notification_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("log_notification_id", $this->createColumnOption("log_notification_id"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"flog_notificationsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"flog_notificationsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="flog_notificationlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->log_notification_id->setDbValue($row['log_notification_id']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            // log_notification_id
            $this->log_notification_id->ViewValue = $this->log_notification_id->CurrentValue;
            $this->log_notification_id->ViewCustomAttributes = "";
            ...
            // log_notification_id
            $this->log_notification_id->LinkCustomAttributes = "";
            $this->log_notification_id->HrefValue = "";
            $this->log_notification_id->TooltipValue = "";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="flog_notificationlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flog_notificationsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
