 ...
 * Page class
 */
class SaleAssetList extends SaleAsset
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'sale_asset';

    // Page object name
    public $PageObjName = "SaleAssetList";
    ...
    public $FormName = "fsale_assetlist";
    ...
    public function __construct()
    {
        ...
        // Table object (sale_asset)
        if (!isset($GLOBALS["sale_asset"]) || get_class($GLOBALS["sale_asset"]) == PROJECT_NAMESPACE . "sale_asset") {
            $GLOBALS["sale_asset"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "saleassetadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "saleassetdelete";
        $this->MultiUpdateUrl = "saleassetupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sale_asset');
        }
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->sale_asset_id->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->status_live->setVisibility();
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "seller") {
            $masterTbl = Container("seller");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("sellerlist"); // Return to master page
                return;
            } else {
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->status_live, $ctrl); // status_live
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->sale_asset_id->setSort("");
                $this->asset_id->setSort("");
                $this->member_id->setSort("");
                $this->status_live->setSort("");
            }
    ...
    protected function setupListOptions()
    {
        ...
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'asset');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fsale_assetlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fsale_assetlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'appointment')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("appointmentlist?" . Config("TABLE_SHOW_MASTER") . "=sale_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("AppointmentGrid");
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sale_asset')) {
        ...
        if ($Security->allowList(CurrentProjectID() . 'asset_warning')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("assetwarninglist?" . Config("TABLE_SHOW_MASTER") . "=sale_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        if ($Security->allowList(CurrentProjectID() . 'asset')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("assetlist?" . Config("TABLE_SHOW_MASTER") . "=sale_asset&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("AssetGrid");
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sale_asset')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->sale_asset_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("status_live", $this->createColumnOption("status_live"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsale_assetsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsale_assetsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsale_assetlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->sale_asset_id->setDbValue($row['sale_asset_id']);
        $this->asset_id->setDbValue($row['asset_id']);
        $this->member_id->setDbValue($row['member_id']);
        $this->status_live->setDbValue($row['status_live']);
    }
    ...
    {
        $row = [];
        $row['sale_asset_id'] = null;
        $row['asset_id'] = null;
        $row['member_id'] = null;
        $row['status_live'] = null;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        // sale_asset_id
        $this->sale_asset_id->CellCssStyle = "white-space: nowrap;";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // status_live
            $this->status_live->ViewValue = $this->status_live->CurrentValue;
            $this->status_live->ViewValue = FormatNumber($this->status_live->ViewValue, $this->status_live->formatPattern());
            $this->status_live->ViewCustomAttributes = "";
            ...
            // status_live
            $this->status_live->LinkCustomAttributes = "";
            $this->status_live->HrefValue = "";
            $this->status_live->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fsale_assetlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "seller") {
            $seller = Container("seller");
            $rsmaster = $seller->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $seller;
                    $seller->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
