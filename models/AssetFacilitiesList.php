 ...
 * Page class
 */
class AssetFacilitiesList extends AssetFacilities
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'asset_facilities';

    // Page object name
    public $PageObjName = "AssetFacilitiesList";
    ...
    public $FormName = "fasset_facilitieslist";
    ...
    public function __construct()
    {
        ...
        // Table object (asset_facilities)
        if (!isset($GLOBALS["asset_facilities"]) || get_class($GLOBALS["asset_facilities"]) == PROJECT_NAMESPACE . "asset_facilities") {
            $GLOBALS["asset_facilities"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "assetfacilitiesadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "assetfacilitiesdelete";
        $this->MultiUpdateUrl = "assetfacilitiesupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'asset_facilities');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("asset_facilities");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->asset_facilities_id->Visible = false;
        }
        if ($this->isAddOrEdit()) {
    ...
    public function run()
    {
        ...
        $this->master_facilities_group_id->setVisibility();
        $this->master_facilities_id->setVisibility();
        ...
        $this->setupLookupOptions($this->master_facilities_group_id);
        $this->setupLookupOptions($this->master_facilities_id);
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fasset_facilitiessrch");
        }
        $filterList = Concat($filterList, $this->master_facilities_group_id->AdvancedSearch->toJson(), ","); // Field master_facilities_group_id
        $filterList = Concat($filterList, $this->master_facilities_id->AdvancedSearch->toJson(), ","); // Field master_facilities_id
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fasset_facilitiessrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field master_facilities_group_id
        $this->master_facilities_group_id->AdvancedSearch->SearchValue = @$filter["x_master_facilities_group_id"];
        $this->master_facilities_group_id->AdvancedSearch->SearchOperator = @$filter["z_master_facilities_group_id"];
        $this->master_facilities_group_id->AdvancedSearch->SearchCondition = @$filter["v_master_facilities_group_id"];
        $this->master_facilities_group_id->AdvancedSearch->SearchValue2 = @$filter["y_master_facilities_group_id"];
        $this->master_facilities_group_id->AdvancedSearch->SearchOperator2 = @$filter["w_master_facilities_group_id"];
        $this->master_facilities_group_id->AdvancedSearch->save();

        // Field master_facilities_id
        $this->master_facilities_id->AdvancedSearch->SearchValue = @$filter["x_master_facilities_id"];
        $this->master_facilities_id->AdvancedSearch->SearchOperator = @$filter["z_master_facilities_id"];
        $this->master_facilities_id->AdvancedSearch->SearchCondition = @$filter["v_master_facilities_id"];
        $this->master_facilities_id->AdvancedSearch->SearchValue2 = @$filter["y_master_facilities_id"];
        $this->master_facilities_id->AdvancedSearch->SearchOperator2 = @$filter["w_master_facilities_id"];
        $this->master_facilities_id->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->master_facilities_group_id, $default, false); // master_facilities_group_id
        $this->buildSearchSql($where, $this->master_facilities_id, $default, false); // master_facilities_id
        ...
        if (!$default && $this->Command == "search") {
            $this->master_facilities_group_id->AdvancedSearch->save(); // master_facilities_group_id
            $this->master_facilities_id->AdvancedSearch->save(); // master_facilities_id
    ...
    {
        if ($this->master_facilities_group_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->master_facilities_id->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->master_facilities_group_id->AdvancedSearch->unsetSession();
        $this->master_facilities_id->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->master_facilities_group_id->AdvancedSearch->load();
        $this->master_facilities_id->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->master_facilities_group_id, $ctrl); // master_facilities_group_id
            $this->updateSort($this->master_facilities_id, $ctrl); // master_facilities_id
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`asset_facilities_id` ASC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->asset_facilities_id->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->asset_facilities_id->setSort("ASC");
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->asset_facilities_id->setSort("");
                $this->asset_id->setSort("");
                $this->master_facilities_group_id->setSort("");
                $this->master_facilities_id->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_facilitieslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasset_facilitieslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->asset_facilities_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("master_facilities_group_id", $this->createColumnOption("master_facilities_group_id"));
            $option->add("master_facilities_id", $this->createColumnOption("master_facilities_id"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fasset_facilitiessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fasset_facilitiessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fasset_facilitieslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->master_facilities_group_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->master_facilities_group_id->AdvancedSearch->SearchValue != "" || $this->master_facilities_group_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // master_facilities_id
        if ($this->master_facilities_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->master_facilities_id->AdvancedSearch->SearchValue != "" || $this->master_facilities_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    {
        $row = [];
        $row['asset_facilities_id'] = null;
        $row['asset_id'] = null;
        $row['master_facilities_group_id'] = null;
        $row['master_facilities_id'] = null;
        $row['isactive'] = null;
        $row['cdate'] = null;
        $row['cip'] = null;
        $row['cuser'] = null;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fasset_facilitieslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fasset_facilitiessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-ew-action=\"highlight\" data-form=\"fasset_facilitiessrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";
        ...
        if ($fld->Lookup !== null && $fld->Lookup->Options === null) {
            ...
            switch ($fld->FieldVar) {
                case "x_asset_id":
                    ...
                    break;
                case "x_master_facilities_group_id":
                    break;
                case "x_master_facilities_id":
                    break;
                case "x_isactive":
                    break;
                default:
