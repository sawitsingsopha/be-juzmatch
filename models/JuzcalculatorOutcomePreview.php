 ...
 * Page class
 */
class JuzcalculatorOutcomePreview extends JuzcalculatorOutcome
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JuzcalculatorOutcomePreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "juzcalculator") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=juzcalculator&" . GetForeignKeyUrl("fk_juzcalculator_id", $this->juzcalculator_id->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeysFromFilter($f)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "juzcalculator") {
            $find = "`juzcalculator_id`=";
            $x = strpos($f, $find);
            if ($x !== false) {
                $x += strlen($find);
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                $this->juzcalculator_id->setQueryStringValue($val);
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("juzcalculatoroutcomelist"), "", $this->TableVar, true);
        $pageId = "preview";
