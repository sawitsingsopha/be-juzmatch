 ...
 * Page class
 */
class DocCredenRunningList extends DocCredenRunning
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'doc_creden_running';

    // Page object name
    public $PageObjName = "DocCredenRunningList";
    ...
    public $FormName = "fdoc_creden_runninglist";
    ...
    public function __construct()
    {
        ...
        // Table object (doc_creden_running)
        if (!isset($GLOBALS["doc_creden_running"]) || get_class($GLOBALS["doc_creden_running"]) == PROJECT_NAMESPACE . "doc_creden_running") {
            $GLOBALS["doc_creden_running"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "doccredenrunningadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "doccredenrunningdelete";
        $this->MultiUpdateUrl = "doccredenrunningupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'doc_creden_running');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("doc_creden_running");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->year, $ctrl); // year
            $this->updateSort($this->month, $ctrl); // month
            $this->updateSort($this->running, $ctrl); // running
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdoc_creden_runninglist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdoc_creden_runninglist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->year->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->month->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("year", $this->createColumnOption("year"));
            $option->add("month", $this->createColumnOption("month"));
            $option->add("running", $this->createColumnOption("running"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdoc_creden_runningsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdoc_creden_runningsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdoc_creden_runninglist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->running->setDbValue($row['running']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // running
            $this->running->ViewValue = $this->running->CurrentValue;
            $this->running->ViewValue = FormatNumber($this->running->ViewValue, $this->running->formatPattern());
            $this->running->ViewCustomAttributes = "";
            ...
            // running
            $this->running->LinkCustomAttributes = "";
            $this->running->HrefValue = "";
            $this->running->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fdoc_creden_runninglist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
