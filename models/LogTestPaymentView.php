 ...
 * Page class
 */
class LogTestPaymentView extends LogTestPayment
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogTestPaymentView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("log_test_payment_id") ?? Route("log_test_payment_id")) !== null) {
            $this->RecKey["log_test_payment_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("log_test_payment_id") ?? Route("log_test_payment_id")) !== null) {
                ...
                $this->RecKey["log_test_payment_id"] = $this->log_test_payment_id->QueryStringValue;
            } elseif (Post("log_test_payment_id") !== null) {
                $this->log_test_payment_id->setFormValue(Post("log_test_payment_id"));
                $this->RecKey["log_test_payment_id"] = $this->log_test_payment_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->log_test_payment_id->setQueryStringValue($keyValue);
                $this->RecKey["log_test_payment_id"] = $this->log_test_payment_id->QueryStringValue;
            } else {
                $returnUrl = "logtestpaymentlist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "logtestpaymentlist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "logtestpaymentlist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
