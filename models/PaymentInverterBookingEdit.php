 ...
 * Page class
 */
class PaymentInverterBookingEdit extends PaymentInverterBooking
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PaymentInverterBookingEdit";
    ...
    public function run()
    {
        ...
        $this->payment_number->Required = false;
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("payment_inverter_booking_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->payment_inverter_booking_id->setQueryStringValue($keyValue);
                $this->payment_inverter_booking_id->setOldValue($this->payment_inverter_booking_id->QueryStringValue);
            } elseif (Post("payment_inverter_booking_id") !== null) {
                $this->payment_inverter_booking_id->setFormValue(Post("payment_inverter_booking_id"));
                $this->payment_inverter_booking_id->setOldValue($this->payment_inverter_booking_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("payment_inverter_booking_id") ?? Route("payment_inverter_booking_id")) !== null) {
                    $this->payment_inverter_booking_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("paymentinverterbookinglist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "paymentinverterbookinglist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->payment_inverter_booking_id->IsDetailKey) {
    ...
    public function renderRow()
    {
        ...
        $this->payment_inverter_booking_id->RowCssClass = "row";
        ...
        // payment_number
        $this->payment_number->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("paymentinverterbookinglist"), "", $this->TableVar, true);
        $pageId = "edit";
