 ...
 * Page class
 */
class AllBuyerConfigAssetScheduleList extends AllBuyerConfigAssetSchedule
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AllBuyerConfigAssetScheduleList";
    ...
    public $FormName = "fall_buyer_config_asset_schedulelist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "allbuyerconfigassetscheduleadd?" . Config("TABLE_SHOW_DETAIL") . "=all_buyer_asset_schedule";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "allbuyerconfigassetscheduledelete";
        $this->MultiUpdateUrl = "allbuyerconfigassetscheduleupdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->down_price, $ctrl); // down_price
            $this->updateSort($this->installment_price_per, $ctrl); // installment_price_per
            $this->updateSort($this->annual_interest, $ctrl); // annual_interest
            $this->updateSort($this->number_days_pay_first_month, $ctrl); // number_days_pay_first_month
            $this->updateSort($this->number_days_in_first_month, $ctrl); // number_days_in_first_month
            $this->updateSort($this->move_in_on_20th, $ctrl); // move_in_on_20th
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->annual_interest->setSort("");
                $this->number_days_pay_first_month->setSort("");
                $this->number_days_in_first_month->setSort("");
                $this->move_in_on_20th->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fall_buyer_config_asset_schedulelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fall_buyer_config_asset_schedulelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'all_buyer_asset_schedule')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("allbuyerassetschedulelist?" . Config("TABLE_SHOW_MASTER") . "=all_buyer_config_asset_schedule&" . GetForeignKeyUrl("fk_buyer_config_asset_schedule_id", $this->buyer_config_asset_schedule_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("AllBuyerAssetScheduleGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'all_buyer_config_asset_schedule')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=all_buyer_asset_schedule");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "all_buyer_asset_schedule";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'all_buyer_config_asset_schedule')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=all_buyer_asset_schedule");
                ...
                $detailEditTblVar .= "all_buyer_asset_schedule";
            }
            if ($links != "") {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->buyer_config_asset_schedule_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_all_buyer_asset_schedule");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=all_buyer_asset_schedule");
                $detailPage = Container("AllBuyerAssetScheduleGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'all_buyer_config_asset_schedule') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "all_buyer_asset_schedule";
                }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("installment_all", $this->createColumnOption("installment_all"));
            $option->add("asset_price", $this->createColumnOption("asset_price"));
            $option->add("booking_price", $this->createColumnOption("booking_price"));
            $option->add("down_price", $this->createColumnOption("down_price"));
            $option->add("installment_price_per", $this->createColumnOption("installment_price_per"));
            $option->add("annual_interest", $this->createColumnOption("annual_interest"));
            $option->add("number_days_pay_first_month", $this->createColumnOption("number_days_pay_first_month"));
            $option->add("number_days_in_first_month", $this->createColumnOption("number_days_in_first_month"));
            $option->add("move_in_on_20th", $this->createColumnOption("move_in_on_20th"));
            $option->add("date_start_installment", $this->createColumnOption("date_start_installment"));
            $option->add("status_approve", $this->createColumnOption("status_approve"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fall_buyer_config_asset_schedulesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fall_buyer_config_asset_schedulesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fall_buyer_config_asset_schedulelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        // Initialize URLs
        $this->ViewUrl = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=all_buyer_asset_schedule");
        $this->EditUrl = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=all_buyer_asset_schedule");
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fall_buyer_config_asset_schedulelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
