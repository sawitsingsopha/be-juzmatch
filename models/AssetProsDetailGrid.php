 ...
 * Page class
 */
class AssetProsDetailGrid extends AssetProsDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AssetProsDetailGrid";
    ...
    public $FormName = "fasset_pros_detailgrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->asset_pros_detail_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_detail") && $CurrentForm->hasValue("o_detail") && $this->detail->CurrentValue != $this->detail->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_group_type") && $CurrentForm->hasValue("o_group_type") && $this->group_type->CurrentValue != $this->group_type->OldValue) {
            return false;
        }
    ...
    {
        $this->detail->clearErrorMessage();
        $this->group_type->clearErrorMessage();
    ...
    {
        $this->asset_pros_detail_id->CurrentValue = null;
        $this->asset_pros_detail_id->OldValue = $this->asset_pros_detail_id->CurrentValue;
        $this->asset_pros_id->CurrentValue = null;
        $this->asset_pros_id->OldValue = $this->asset_pros_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_detail")) {
            $this->detail->setOldValue($CurrentForm->getValue("o_detail"));
        }

        // Check field name 'group_type' first before field var 'x_group_type'
        $val = $CurrentForm->hasValue("group_type") ? $CurrentForm->getValue("group_type") : $CurrentForm->getValue("x_group_type");
        if (!$this->group_type->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->group_type->Visible = false; // Disable update for API request
            } else {
                $this->group_type->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_group_type")) {
            $this->group_type->setOldValue($CurrentForm->getValue("o_group_type"));
        }
        ...
        // Check field name 'asset_pros_detail_id' first before field var 'x_asset_pros_detail_id'
        $val = $CurrentForm->hasValue("asset_pros_detail_id") ? $CurrentForm->getValue("asset_pros_detail_id") : $CurrentForm->getValue("x_asset_pros_detail_id");
        if (!$this->asset_pros_detail_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->asset_pros_detail_id->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->asset_pros_detail_id->CurrentValue = $this->asset_pros_detail_id->FormValue;
        }
        $this->detail->CurrentValue = $this->detail->FormValue;
        $this->group_type->CurrentValue = $this->group_type->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['asset_pros_detail_id'] = $this->asset_pros_detail_id->CurrentValue;
        $row['asset_pros_id'] = $this->asset_pros_id->CurrentValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->group_type->Required) {
            if (!$this->group_type->IsDetailKey && EmptyValue($this->group_type->FormValue)) {
                $this->group_type->addErrorMessage(str_replace("%s", $this->group_type->caption(), $this->group_type->RequiredErrorMessage));
            }
        }
    ...
    protected function deleteRows()
    {
        ...
        foreach ($rsold as $row) {
            ...
            $thisKey .= $row['asset_pros_detail_id'];
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // group_type
            $this->group_type->setDbValueDef($rsnew, $this->group_type->CurrentValue, null, $this->group_type->ReadOnly);
            ...
            // isactive
            $this->isactive->setDbValueDef($rsnew, $this->isactive->CurrentValue, 0, $this->isactive->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'asset_pros_detail'
        ...
        // group_type
        $this->group_type->setDbValueDef($rsnew, $this->group_type->CurrentValue, null, false);
        ...
        // isactive
        $this->isactive->setDbValueDef($rsnew, $this->isactive->CurrentValue, 0, strval($this->isactive->CurrentValue) == "");
