...
/**
 * Table class for buyer_asset
 */
class BuyerAsset extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'buyer_asset';
        $this->TableName = 'buyer_asset';
        ...
        $this->status_payment->Lookup = new Lookup('status_payment', 'buyer_asset', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->status_expire->Lookup = new Lookup('status_expire', 'buyer_asset', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "buyer_asset_rent") {
            $detailUrl = Container("buyer_asset_rent")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
        ...
        if ($this->getCurrentDetailTable() == "todo_list") {
            $detailUrl = Container("todo_list")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "plan_loan") {
            $detailUrl = Container("plan_loan")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "buyer_asset_ready_buy") {
            $detailUrl = Container("buyer_asset_ready_buy")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        ...
        if ($this->getCurrentDetailTable() == "buyer_config_asset_schedule") {
            $detailUrl = Container("buyer_config_asset_schedule")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "buyerassetlist";
        }
        return $detailUrl;
    }
    ...
    {
        // Cascade Update detail table 'buyer_asset_rent'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("buyer_asset_rent")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("buyer_asset_rent")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("buyer_asset_rent")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("buyer_asset_rent")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'todo_list'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['asset_id']) && $rsold['asset_id'] != $rs['asset_id'])) { // Update detail field 'asset_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("todo_list")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'todo_list_id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("todo_list")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("todo_list")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("todo_list")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'plan_loan'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['asset_id']) && $rsold['asset_id'] != $rs['asset_id'])) { // Update detail field 'asset_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("plan_loan")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                ...
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("plan_loan")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("plan_loan")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("plan_loan")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'buyer_asset_ready_buy'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['asset_id']) && $rsold['asset_id'] != $rs['asset_id'])) { // Update detail field 'asset_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("buyer_asset_ready_buy")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'buyer_asset_ready_buy_id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("buyer_asset_ready_buy")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("buyer_asset_ready_buy")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("buyer_asset_ready_buy")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("buyer_asset_schedule")->loadRs("`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'buyer_asset_rent'
        $dtlrows = Container("buyer_asset_rent")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB") . " AND " . "`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("buyer_asset_rent")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("buyer_asset_rent")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("buyer_asset_rent")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'todo_list'
        $dtlrows = Container("todo_list")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB") . " AND " . "`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("todo_list")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("todo_list")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("todo_list")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'plan_loan'
        $dtlrows = Container("plan_loan")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB") . " AND " . "`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("plan_loan")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("plan_loan")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("plan_loan")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'buyer_asset_ready_buy'
        $dtlrows = Container("buyer_asset_ready_buy")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB") . " AND " . "`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("buyer_asset_ready_buy")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("buyer_asset_ready_buy")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("buyer_asset_ready_buy")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'buyer_asset_schedule'
        $dtlrows = Container("buyer_asset_schedule")->loadRs("`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB") . " AND " . "`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("buyerassetlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "buyerassetview") {
            return $Language->phrase("View");
        } elseif ($pageName == "buyerassetedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "buyerassetadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "BuyerAssetView";
            case Config("API_ADD_ACTION"):
                return "BuyerAssetAdd";
            case Config("API_EDIT_ACTION"):
                return "BuyerAssetEdit";
            case Config("API_DELETE_ACTION"):
                return "BuyerAssetDelete";
            case Config("API_LIST_ACTION"):
                return "BuyerAssetList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("buyerassetview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("buyerassetview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "buyerassetadd?" . $this->getUrlParm($parm);
        } else {
            $url = "buyerassetadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("buyerassetedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("buyerassetedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("buyerassetadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("buyerassetadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("buyerassetdelete", $this->getUrlParm());
    }
