 ...
 * Page class
 */
class AssetWarningEdit extends AssetWarning
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AssetWarningEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("asset_warning_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->asset_warning_id->setQueryStringValue($keyValue);
                $this->asset_warning_id->setOldValue($this->asset_warning_id->QueryStringValue);
            } elseif (Post("asset_warning_id") !== null) {
                $this->asset_warning_id->setFormValue(Post("asset_warning_id"));
                $this->asset_warning_id->setOldValue($this->asset_warning_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->asset_warning_id->IsDetailKey) {
