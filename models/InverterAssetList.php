 ...
 * Page class
 */
class InverterAssetList extends InverterAsset
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'inverter_asset';

    // Page object name
    public $PageObjName = "InverterAssetList";
    ...
    public $FormName = "finverter_assetlist";
    ...
    public function __construct()
    {
        ...
        // Table object (inverter_asset)
        if (!isset($GLOBALS["inverter_asset"]) || get_class($GLOBALS["inverter_asset"]) == PROJECT_NAMESPACE . "inverter_asset") {
            $GLOBALS["inverter_asset"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "inverterassetadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "inverterassetdelete";
        $this->MultiUpdateUrl = "inverterassetupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'inverter_asset');
        }
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['payment_inverter_booking_id'];
        }
        return $key;
    }
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->payment_inverter_booking_id->Visible = false;
        }
        if ($this->isAddOrEdit()) {
    ...
    public function run()
    {
        ...
        $this->payment_number->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "finverter_assetsrch");
        }
        ...
        $filterList = Concat($filterList, $this->payment_number->AdvancedSearch->toJson(), ","); // Field payment_number
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "finverter_assetsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->payment_number->AdvancedSearch->SearchValue = @$filter["x_payment_number"];
        $this->payment_number->AdvancedSearch->SearchOperator = @$filter["z_payment_number"];
        $this->payment_number->AdvancedSearch->SearchCondition = @$filter["v_payment_number"];
        $this->payment_number->AdvancedSearch->SearchValue2 = @$filter["y_payment_number"];
        $this->payment_number->AdvancedSearch->SearchOperator2 = @$filter["w_payment_number"];
        $this->payment_number->AdvancedSearch->save();
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->payment_inverter_booking_id->setSort("");
                $this->asset_id->setSort("");
                $this->member_id->setSort("");
                $this->payment->setSort("");
                $this->type->setSort("");
                $this->payment_number->setSort("");
                $this->status->setSort("");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_asset_config_schedule"
        $item = &$this->ListOptions->add("detail_asset_config_schedule");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'asset_config_schedule');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_asset_schedule"
        $item = &$this->ListOptions->add("detail_asset_schedule");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'asset_schedule');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("asset_config_schedule");
        $pages->add("asset_schedule");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finverter_assetlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finverter_assetlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_asset_config_schedule"
        $opt = $this->ListOptions["detail_asset_config_schedule"];
        if ($Security->allowList(CurrentProjectID() . 'asset_config_schedule')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("asset_config_schedule", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("assetconfigschedulelist?" . Config("TABLE_SHOW_MASTER") . "=inverter_asset&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("AssetConfigScheduleGrid");
            if ($links != "") {
        ...
        // "detail_asset_schedule"
        $opt = $this->ListOptions["detail_asset_schedule"];
        if ($Security->allowList(CurrentProjectID() . 'asset_schedule')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("asset_schedule", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("assetschedulelist?" . Config("TABLE_SHOW_MASTER") . "=inverter_asset&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "&" . GetForeignKeyUrl("fk_asset_id", $this->asset_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("AssetScheduleGrid");
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->payment_inverter_booking_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"finverter_assetsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"finverter_assetsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="finverter_assetlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    {
        $row = [];
        $row['payment_inverter_booking_id'] = null;
        $row['asset_id'] = null;
        $row['member_id'] = null;
        $row['payment'] = null;
        $row['type'] = null;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="finverter_assetlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"finverter_assetsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
