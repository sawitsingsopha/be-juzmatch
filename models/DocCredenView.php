 ...
 * Page class
 */
class DocCredenView extends DocCreden
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocCredenView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("doc_creden_id") ?? Route("doc_creden_id")) !== null) {
            $this->RecKey["doc_creden_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("doc_creden_id") ?? Route("doc_creden_id")) !== null) {
                ...
                $this->RecKey["doc_creden_id"] = $this->doc_creden_id->QueryStringValue;
            } elseif (Post("doc_creden_id") !== null) {
                $this->doc_creden_id->setFormValue(Post("doc_creden_id"));
                $this->RecKey["doc_creden_id"] = $this->doc_creden_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->doc_creden_id->setQueryStringValue($keyValue);
                $this->RecKey["doc_creden_id"] = $this->doc_creden_id->QueryStringValue;
            } else {
                $returnUrl = "doccredenlist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "doccredenlist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "doccredenlist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
