 ...
 * Page class
 */
class DocTempView extends DocTemp
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocTempView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("doc_temp_id") ?? Route("doc_temp_id")) !== null) {
            $this->RecKey["doc_temp_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("doc_temp_id") ?? Route("doc_temp_id")) !== null) {
                ...
                $this->RecKey["doc_temp_id"] = $this->doc_temp_id->QueryStringValue;
            } elseif (Post("doc_temp_id") !== null) {
                $this->doc_temp_id->setFormValue(Post("doc_temp_id"));
                $this->RecKey["doc_temp_id"] = $this->doc_temp_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->doc_temp_id->setQueryStringValue($keyValue);
                $this->RecKey["doc_temp_id"] = $this->doc_temp_id->QueryStringValue;
            } else {
                $returnUrl = "doctemplist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "doctemplist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "doctemplist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
