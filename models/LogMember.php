...
/**
 * Table class for log_member
 */
class LogMember extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'log_member';
        $this->TableName = 'log_member';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`log_member`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // log_member_id
        $this->log_member_id = new DbField(
            'log_member',
            'log_member',
            'x_log_member_id',
            'log_member_id',
        ...
        $this->log_member_id->InputTextType = "text";
        $this->log_member_id->IsAutoIncrement = true; // Autoincrement field
        $this->log_member_id->IsPrimaryKey = true; // Primary key field
        $this->log_member_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['log_member_id'] = &$this->log_member_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`log_member`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->log_member_id->setDbValue($conn->lastInsertId());
            $rs['log_member_id'] = $this->log_member_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('log_member_id', $rs)) {
                AddFilter($where, QuotedName('log_member_id', $this->Dbid) . '=' . QuotedValue($rs['log_member_id'], $this->log_member_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->log_member_id->DbValue = $row['log_member_id'];
    ...
    {
        return "`log_member_id` = @log_member_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->log_member_id->CurrentValue : $this->log_member_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->log_member_id->CurrentValue = $keys[0];
            } else {
                $this->log_member_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('log_member_id', $row) ? $row['log_member_id'] : null;
        } else {
            $val = $this->log_member_id->OldValue !== null ? $this->log_member_id->OldValue : $this->log_member_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@log_member_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("logmemberlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "logmemberview") {
            return $Language->phrase("View");
        } elseif ($pageName == "logmemberedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "logmemberadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "LogMemberView";
            case Config("API_ADD_ACTION"):
                return "LogMemberAdd";
            case Config("API_EDIT_ACTION"):
                return "LogMemberEdit";
            case Config("API_DELETE_ACTION"):
                return "LogMemberDelete";
            case Config("API_LIST_ACTION"):
                return "LogMemberList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("logmemberview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("logmemberview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "logmemberadd?" . $this->getUrlParm($parm);
        } else {
            $url = "logmemberadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("logmemberedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logmemberadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("logmemberdelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"log_member_id\":" . JsonEncode($this->log_member_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->log_member_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->log_member_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("log_member_id") ?? Route("log_member_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->log_member_id->CurrentValue = $key;
            } else {
                $this->log_member_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->log_member_id->setDbValue($row['log_member_id']);
    ...
    public function renderListRow()
    {
        ...
        // log_member_id
        $this->log_member_id->ViewValue = $this->log_member_id->CurrentValue;
        $this->log_member_id->ViewCustomAttributes = "";
        ...
        // log_member_id
        $this->log_member_id->LinkCustomAttributes = "";
        $this->log_member_id->HrefValue = "";
        $this->log_member_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // log_member_id
        $this->log_member_id->setupEditAttributes();
        $this->log_member_id->EditCustomAttributes = "";
        $this->log_member_id->EditValue = $this->log_member_id->CurrentValue;
        $this->log_member_id->ViewCustomAttributes = "";
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->log_member_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->log_member_id);
