...
/**
 * Table class for log_buyer_asset_schedule
 */
class LogBuyerAssetSchedule extends DbTable
{
    protected $SqlFrom = "";
    ...
    // Fields
    public $log_buyer_asset_schedule_id;
    public $asset_id;
    public $member_id;
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'log_buyer_asset_schedule';
        $this->TableName = 'log_buyer_asset_schedule';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`log_buyer_asset_schedule`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // log_buyer_asset_schedule_id
        $this->log_buyer_asset_schedule_id = new DbField(
            'log_buyer_asset_schedule',
            'log_buyer_asset_schedule',
            'x_log_buyer_asset_schedule_id',
            'log_buyer_asset_schedule_id',
            '`log_buyer_asset_schedule_id`',
            '`log_buyer_asset_schedule_id`',
            3,
        ...
        $this->log_buyer_asset_schedule_id->InputTextType = "text";
        $this->log_buyer_asset_schedule_id->IsAutoIncrement = true; // Autoincrement field
        $this->log_buyer_asset_schedule_id->IsPrimaryKey = true; // Primary key field
        $this->log_buyer_asset_schedule_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['log_buyer_asset_schedule_id'] = &$this->log_buyer_asset_schedule_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`log_buyer_asset_schedule`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->log_buyer_asset_schedule_id->setDbValue($conn->lastInsertId());
            $rs['log_buyer_asset_schedule_id'] = $this->log_buyer_asset_schedule_id->DbValue;
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('log_buyer_asset_schedule_id', $rs)) {
                AddFilter($where, QuotedName('log_buyer_asset_schedule_id', $this->Dbid) . '=' . QuotedValue($rs['log_buyer_asset_schedule_id'], $this->log_buyer_asset_schedule_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->log_buyer_asset_schedule_id->DbValue = $row['log_buyer_asset_schedule_id'];
    ...
    {
        return "`log_buyer_asset_schedule_id` = @log_buyer_asset_schedule_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->log_buyer_asset_schedule_id->CurrentValue : $this->log_buyer_asset_schedule_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->log_buyer_asset_schedule_id->CurrentValue = $keys[0];
            } else {
                $this->log_buyer_asset_schedule_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('log_buyer_asset_schedule_id', $row) ? $row['log_buyer_asset_schedule_id'] : null;
        } else {
            $val = $this->log_buyer_asset_schedule_id->OldValue !== null ? $this->log_buyer_asset_schedule_id->OldValue : $this->log_buyer_asset_schedule_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@log_buyer_asset_schedule_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("logbuyerassetschedulelist");
    }
    ...
    {
        global $Language;
        if ($pageName == "logbuyerassetscheduleview") {
            return $Language->phrase("View");
        } elseif ($pageName == "logbuyerassetscheduleedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "logbuyerassetscheduleadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "LogBuyerAssetScheduleView";
            case Config("API_ADD_ACTION"):
                return "LogBuyerAssetScheduleAdd";
            case Config("API_EDIT_ACTION"):
                return "LogBuyerAssetScheduleEdit";
            case Config("API_DELETE_ACTION"):
                return "LogBuyerAssetScheduleDelete";
            case Config("API_LIST_ACTION"):
                return "LogBuyerAssetScheduleList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("logbuyerassetscheduleview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("logbuyerassetscheduleview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "logbuyerassetscheduleadd?" . $this->getUrlParm($parm);
        } else {
            $url = "logbuyerassetscheduleadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logbuyerassetscheduleedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logbuyerassetscheduleadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("logbuyerassetscheduledelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"log_buyer_asset_schedule_id\":" . JsonEncode($this->log_buyer_asset_schedule_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->log_buyer_asset_schedule_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->log_buyer_asset_schedule_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("log_buyer_asset_schedule_id") ?? Route("log_buyer_asset_schedule_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->log_buyer_asset_schedule_id->CurrentValue = $key;
            } else {
                $this->log_buyer_asset_schedule_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->log_buyer_asset_schedule_id->setDbValue($row['log_buyer_asset_schedule_id']);
    ...
    public function renderListRow()
    {
        ...
        // log_buyer_asset_schedule_id
        $this->log_buyer_asset_schedule_id->ViewValue = $this->log_buyer_asset_schedule_id->CurrentValue;
        $this->log_buyer_asset_schedule_id->ViewCustomAttributes = "";
        ...
        // log_buyer_asset_schedule_id
        $this->log_buyer_asset_schedule_id->LinkCustomAttributes = "";
        $this->log_buyer_asset_schedule_id->HrefValue = "";
        $this->log_buyer_asset_schedule_id->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // log_buyer_asset_schedule_id
        $this->log_buyer_asset_schedule_id->setupEditAttributes();
        $this->log_buyer_asset_schedule_id->EditCustomAttributes = "";
        $this->log_buyer_asset_schedule_id->EditValue = $this->log_buyer_asset_schedule_id->CurrentValue;
        $this->log_buyer_asset_schedule_id->ViewCustomAttributes = "";
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->log_buyer_asset_schedule_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->log_buyer_asset_schedule_id);
