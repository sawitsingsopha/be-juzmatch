 ...
 * Page class
 */
class SellerView extends Seller
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("member_id") ?? Route("member_id")) !== null) {
                ...
                $returnUrl = "sellerlist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "sellerlist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "sellerlist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item = &$option->add("detail_seller_verify");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("seller_verify", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("sellerverifylist?" . Config("TABLE_SHOW_MASTER") . "=seller&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("SellerVerifyGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'seller')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=seller_verify"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        // "detail_sale_asset"
        $item = &$option->add("detail_sale_asset");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("sale_asset", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("saleassetlist?" . Config("TABLE_SHOW_MASTER") . "=seller&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("SaleAssetGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'seller')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sale_asset"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("seller_verify", $detailTblVar)) {
                $detailPageObj = Container("SellerVerifyGrid");
            ...
            if (in_array("sale_asset", $detailTblVar)) {
                $detailPageObj = Container("SaleAssetGrid");
                if ($detailPageObj->DetailView) {
                    $detailPageObj->CurrentMode = "view";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("sellerlist"), "", $this->TableVar, true);
        $pageId = "view";
