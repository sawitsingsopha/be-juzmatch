 ...
 * Page class
 */
class AssetFavoriteGrid extends AssetFavorite
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AssetFavoriteGrid";
    ...
    public $FormName = "fasset_favoritegrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->asset_favorite_id->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_isfavorite") && $CurrentForm->hasValue("o_isfavorite") && $this->isfavorite->CurrentValue != $this->isfavorite->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->isfavorite->clearErrorMessage();
    ...
    {
        $this->asset_favorite_id->CurrentValue = null;
        $this->asset_favorite_id->OldValue = $this->asset_favorite_id->CurrentValue;
        ...
        $this->isfavorite->CurrentValue = null;
        $this->isfavorite->OldValue = $this->isfavorite->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'isfavorite' first before field var 'x_isfavorite'
        $val = $CurrentForm->hasValue("isfavorite") ? $CurrentForm->getValue("isfavorite") : $CurrentForm->getValue("x_isfavorite");
        if (!$this->isfavorite->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->isfavorite->Visible = false; // Disable update for API request
            } else {
                $this->isfavorite->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_isfavorite")) {
            $this->isfavorite->setOldValue($CurrentForm->getValue("o_isfavorite"));
        }
        ...
        // Check field name 'asset_favorite_id' first before field var 'x_asset_favorite_id'
        $val = $CurrentForm->hasValue("asset_favorite_id") ? $CurrentForm->getValue("asset_favorite_id") : $CurrentForm->getValue("x_asset_favorite_id");
        if (!$this->asset_favorite_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->asset_favorite_id->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->asset_favorite_id->CurrentValue = $this->asset_favorite_id->FormValue;
        }
        $this->asset_id->CurrentValue = $this->asset_id->FormValue;
        $this->isfavorite->CurrentValue = $this->isfavorite->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['asset_favorite_id'] = $this->asset_favorite_id->CurrentValue;
        $row['asset_id'] = $this->asset_id->CurrentValue;
        $row['member_id'] = $this->member_id->CurrentValue;
        $row['isfavorite'] = $this->isfavorite->CurrentValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->isfavorite->Required) {
            if (!$this->isfavorite->IsDetailKey && EmptyValue($this->isfavorite->FormValue)) {
                $this->isfavorite->addErrorMessage(str_replace("%s", $this->isfavorite->caption(), $this->isfavorite->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // isfavorite
            $this->isfavorite->setDbValueDef($rsnew, $this->isfavorite->CurrentValue, null, $this->isfavorite->ReadOnly);

            // cdate
            $this->cdate->CurrentValue = CurrentDateTime();
            $this->cdate->setDbValueDef($rsnew, $this->cdate->CurrentValue, null);

            // Check referential integrity for master table 'member'
            $detailKeys = [];
            $keyValue = $rsnew['member_id'] ?? $rsold['member_id'];
            $detailKeys['member_id'] = $keyValue;
            $masterTable = Container("member");
            ...
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "member", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'asset_favorite'
        ...
        // isfavorite
        $this->isfavorite->setDbValueDef($rsnew, $this->isfavorite->CurrentValue, null, false);
