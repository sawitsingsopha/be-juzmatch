...
/**
 * Table class for invertor_booking
 */
class InvertorBooking extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'invertor_booking';
        $this->TableName = 'invertor_booking';
        ...
        $this->date_booking = new DbField(
            ...
            'date_booking',
            '`date_booking`',
            CastDateFieldForLike("`date_booking`", 111, "DB"),
            135,
            19,
            111,
            false,
        ...
        $this->date_booking->DefaultErrorMessage = str_replace("%s", DateFormat(111), $Language->phrase("IncorrectDate"));
        ...
        $this->status_expire->Lookup = new Lookup('status_expire', 'invertor_booking', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->payment_status->Lookup = new Lookup('payment_status', 'invertor_booking', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    {
        // Cascade Update detail table 'payment_inverter_booking'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("payment_inverter_booking")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB') . " AND " . "`asset_id` = " . QuotedValue($rsold['asset_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("payment_inverter_booking")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("payment_inverter_booking")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("payment_inverter_booking")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'payment_inverter_booking'
        $dtlrows = Container("payment_inverter_booking")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB") . " AND " . "`asset_id` = " . QuotedValue($rs['asset_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("payment_inverter_booking")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("payment_inverter_booking")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("payment_inverter_booking")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("invertorbookinglist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "invertorbookingedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "invertorbookingadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "InvertorBookingView";
            case Config("API_ADD_ACTION"):
                return "InvertorBookingAdd";
            case Config("API_EDIT_ACTION"):
                return "InvertorBookingEdit";
            case Config("API_DELETE_ACTION"):
                return "InvertorBookingDelete";
            case Config("API_LIST_ACTION"):
                return "InvertorBookingList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("invertorbookingview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("invertorbookingview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "invertorbookingadd?" . $this->getUrlParm($parm);
        } else {
            $url = "invertorbookingadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("invertorbookingedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("invertorbookingedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("invertorbookingadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("invertorbookingadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("invertorbookingdelete", $this->getUrlParm());
    }
