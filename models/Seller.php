...
/**
 * Table class for seller
 */
class Seller extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->last_login = new DbField(
            ...
            'last_login',
            '`last_login`',
            CastDateFieldForLike("`last_login`", 0, "juzmatch3"),
            135,
            19,
            0,
            false,
        ...
        $this->type->Lookup = new Lookup('type', 'seller', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->isactive->Lookup = new Lookup('isactive', 'seller', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->isbuyer->Lookup = new Lookup('isbuyer', 'seller', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->isinvertor->Lookup = new Lookup('isinvertor', 'seller', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->issale->Lookup = new Lookup('issale', 'seller', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->reset_password_date = new DbField(
            ...
            'reset_password_date',
            '`reset_password_date`',
            CastDateFieldForLike("`reset_password_date`", 0, "juzmatch3"),
            135,
            19,
            0,
            false,
        ...
        $this->reset_password_email_date = new DbField(
            ...
            'reset_password_email_date',
            '`reset_password_email_date`',
            CastDateFieldForLike("`reset_password_email_date`", 0, "juzmatch3"),
            135,
            19,
            0,
            false,
        ...
        $this->verify_date = new DbField(
            ...
            'verify_date',
            '`verify_date`',
            CastDateFieldForLike("`verify_date`", 0, "juzmatch3"),
            135,
            19,
            0,
            false,
        ...
        $this->cdate = new DbField(
            ...
            'cdate',
            '`cdate`',
            CastDateFieldForLike("`cdate`", 117, "juzmatch3"),
            135,
            19,
            117,
            false,
        ...
        $this->udate = new DbField(
            ...
            'udate',
            '`udate`',
            CastDateFieldForLike("`udate`", 0, "juzmatch3"),
            135,
            19,
            0,
            false,
        ...
        $this->verify_phone_date = new DbField(
            ...
            'verify_phone_date',
            '`verify_phone_date`',
            CastDateFieldForLike("`verify_phone_date`", 0, "juzmatch3"),
            135,
            19,
            0,
            false,
        ...
        $this->last_change_password = new DbField(
            ...
            'last_change_password',
            '`last_change_password`',
            CastDateFieldForLike("`last_change_password`", 0, "juzmatch3"),
            135,
            19,
            0,
            false,
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "seller_verify") {
            $detailUrl = Container("seller_verify")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "sale_asset") {
            $detailUrl = Container("sale_asset")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_member_id", $this->member_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "sellerlist";
        }
        return $detailUrl;
    }
    ...
    {
        // Cascade Update detail table 'seller_verify'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("seller_verify")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'seller_verify_id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("seller_verify")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("seller_verify")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("seller_verify")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'sale_asset'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['member_id']) && $rsold['member_id'] != $rs['member_id'])) { // Update detail field 'member_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("sale_asset")->loadRs("`member_id` = " . QuotedValue($rsold['member_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'sale_asset_id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("sale_asset")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("sale_asset")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("sale_asset")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'seller_verify'
        $dtlrows = Container("seller_verify")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("seller_verify")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("seller_verify")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("seller_verify")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'sale_asset'
        $dtlrows = Container("sale_asset")->loadRs("`member_id` = " . QuotedValue($rs['member_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("sale_asset")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("sale_asset")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("sale_asset")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("sellerlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "sellerview") {
            return $Language->phrase("View");
        } elseif ($pageName == "selleredit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "selleradd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("sellerview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("sellerview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "selleradd?" . $this->getUrlParm($parm);
        } else {
            $url = "selleradd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("selleredit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("selleredit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("selleradd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("selleradd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("sellerdelete", $this->getUrlParm());
    }
    ...
    public function renderListRow()
    {
        ...
        if ($this->image_profile->UseColorbox) {
            ...
            $this->image_profile->LinkAttrs["data-rel"] = "seller_x_image_profile";
