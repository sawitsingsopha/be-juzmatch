...
var fasset_banneradd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fasset_banneradd = new ew.Form("fasset_banneradd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fasset_banneradd;

    // Add fields
    var fields = currentTable.fields;
    fasset_banneradd.addFields([
    ...
    // Form_CustomValidate
    fasset_banneradd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fasset_banneradd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fasset_banneradd.lists.asset_id = <?= $Page->asset_id->toClientList($Page) ?>;
    fasset_banneradd.lists.isactive = <?= $Page->isactive->toClientList($Page) ?>;
    loadjs.done("fasset_banneradd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fasset_banneradd" id="fasset_banneradd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_asset_banner_asset_id">
    <select
        ...
        data-select2-id="fasset_banneradd_x_asset_id"
...
<script>
loadjs.ready("fasset_banneradd", function() {
    var options = { name: "x_asset_id", selectId: "fasset_banneradd_x_asset_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasset_banneradd.lists.asset_id.lookupOptions.length) {
        options.data = { id: "x_asset_id", form: "fasset_banneradd" };
    } else {
        options.ajax = { id: "x_asset_id", form: "fasset_banneradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
