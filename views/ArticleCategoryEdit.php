...
// Page object
$ArticleCategoryEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { article_category: currentTable } });
var currentForm, currentPageID;
var farticle_categoryedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    farticle_categoryedit = new ew.Form("farticle_categoryedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = farticle_categoryedit;

    // Add fields
    var fields = currentTable.fields;
    farticle_categoryedit.addFields([
    ...
    // Form_CustomValidate
    farticle_categoryedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farticle_categoryedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    farticle_categoryedit.lists.active_status = <?= $Page->active_status->toClientList($Page) ?>;
    loadjs.done("farticle_categoryedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="farticle_categoryedit" id="farticle_categoryedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } ?>
    ...
    <input type="hidden" data-table="article_category" data-field="x_article_category_id" data-hidden="1" name="x_article_category_id" id="x_article_category_id" value="<?= HtmlEncode($Page->article_category_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
