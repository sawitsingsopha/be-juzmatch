...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fdoc_tempadd = new ew.Form("fdoc_tempadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fdoc_tempadd;

    // Add fields
    var fields = currentTable.fields;
    fdoc_tempadd.addFields([
    ...
    // Form_CustomValidate
    fdoc_tempadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fdoc_tempadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fdoc_tempadd.lists.doc_temp_type = <?= $Page->doc_temp_type->toClientList($Page) ?>;
    fdoc_tempadd.lists.active_status = <?= $Page->active_status->toClientList($Page) ?>;
    loadjs.done("fdoc_tempadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fdoc_tempadd" id="fdoc_tempadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_doc_temp_doc_temp_type">
    <select
        ...
        data-select2-id="fdoc_tempadd_x_doc_temp_type"
        data-table="doc_temp"
...
<script>
loadjs.ready("fdoc_tempadd", function() {
    var options = { name: "x_doc_temp_type", selectId: "fdoc_tempadd_x_doc_temp_type" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdoc_tempadd.lists.doc_temp_type.lookupOptions.length) {
        options.data = { id: "x_doc_temp_type", form: "fdoc_tempadd" };
    } else {
        options.ajax = { id: "x_doc_temp_type", form: "fdoc_tempadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="hidden" name="fa_x_doc_temp_file" id= "fa_x_doc_temp_file" value="0">
