...
// Page object
$AssetProsDetailEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { asset_pros_detail: currentTable } });
var currentForm, currentPageID;
var fasset_pros_detailedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fasset_pros_detailedit = new ew.Form("fasset_pros_detailedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fasset_pros_detailedit;

    // Add fields
    var fields = currentTable.fields;
    fasset_pros_detailedit.addFields([
    ...
    // Form_CustomValidate
    fasset_pros_detailedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fasset_pros_detailedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fasset_pros_detailedit.lists.asset_id = <?= $Page->asset_id->toClientList($Page) ?>;
    fasset_pros_detailedit.lists.group_type = <?= $Page->group_type->toClientList($Page) ?>;
    fasset_pros_detailedit.lists.isactive = <?= $Page->isactive->toClientList($Page) ?>;
    loadjs.done("fasset_pros_detailedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fasset_pros_detailedit" id="fasset_pros_detailedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_asset_pros_detail_asset_id">
    <select
        ...
        data-select2-id="fasset_pros_detailedit_x_asset_id"
...
<script>
loadjs.ready("fasset_pros_detailedit", function() {
    var options = { name: "x_asset_id", selectId: "fasset_pros_detailedit_x_asset_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasset_pros_detailedit.lists.asset_id.lookupOptions.length) {
        options.data = { id: "x_asset_id", form: "fasset_pros_detailedit" };
    } else {
        options.ajax = { id: "x_asset_id", form: "fasset_pros_detailedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_asset_pros_detail_group_type">
    <select
        ...
        data-select2-id="fasset_pros_detailedit_x_group_type"
        data-table="asset_pros_detail"
...
<script>
loadjs.ready("fasset_pros_detailedit", function() {
    var options = { name: "x_group_type", selectId: "fasset_pros_detailedit_x_group_type" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasset_pros_detailedit.lists.group_type.lookupOptions.length) {
        options.data = { id: "x_group_type", form: "fasset_pros_detailedit" };
    } else {
        options.ajax = { id: "x_group_type", form: "fasset_pros_detailedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="asset_pros_detail" data-field="x_asset_pros_detail_id" data-hidden="1" name="x_asset_pros_detail_id" id="x_asset_pros_detail_id" value="<?= HtmlEncode($Page->asset_pros_detail_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
