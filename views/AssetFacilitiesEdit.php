...
// Page object
$AssetFacilitiesEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { asset_facilities: currentTable } });
var currentForm, currentPageID;
var fasset_facilitiesedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fasset_facilitiesedit = new ew.Form("fasset_facilitiesedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fasset_facilitiesedit;

    // Add fields
    var fields = currentTable.fields;
    fasset_facilitiesedit.addFields([
    ...
    // Form_CustomValidate
    fasset_facilitiesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fasset_facilitiesedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fasset_facilitiesedit.lists.master_facilities_group_id = <?= $Page->master_facilities_group_id->toClientList($Page) ?>;
    fasset_facilitiesedit.lists.master_facilities_id = <?= $Page->master_facilities_id->toClientList($Page) ?>;
    fasset_facilitiesedit.lists.isactive = <?= $Page->isactive->toClientList($Page) ?>;
    loadjs.done("fasset_facilitiesedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fasset_facilitiesedit" id="fasset_facilitiesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="asset_facilities" data-field="x_asset_id" data-hidden="1" name="x_asset_id" id="x_asset_id" value="<?= HtmlEncode($Page->asset_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<?php $Page->master_facilities_group_id->EditAttrs->prepend("onchange", "ew.updateOptions.call(this);"); ?>
    <select
        ...
        data-select2-id="fasset_facilitiesedit_x_master_facilities_group_id"
...
<script>
loadjs.ready("fasset_facilitiesedit", function() {
    var options = { name: "x_master_facilities_group_id", selectId: "fasset_facilitiesedit_x_master_facilities_group_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasset_facilitiesedit.lists.master_facilities_group_id.lookupOptions.length) {
        options.data = { id: "x_master_facilities_group_id", form: "fasset_facilitiesedit" };
    } else {
        options.ajax = { id: "x_master_facilities_group_id", form: "fasset_facilitiesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_asset_facilities_master_facilities_id">
    <select
        ...
        data-select2-id="fasset_facilitiesedit_x_master_facilities_id"
        data-table="asset_facilities"
...
<script>
loadjs.ready("fasset_facilitiesedit", function() {
    var options = { name: "x_master_facilities_id", selectId: "fasset_facilitiesedit_x_master_facilities_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasset_facilitiesedit.lists.master_facilities_id.lookupOptions.length) {
        options.data = { id: "x_master_facilities_id", form: "fasset_facilitiesedit" };
    } else {
        options.ajax = { id: "x_master_facilities_id", form: "fasset_facilitiesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="asset_facilities" data-field="x_asset_facilities_id" data-hidden="1" name="x_asset_facilities_id" id="x_asset_facilities_id" value="<?= HtmlEncode($Page->asset_facilities_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
