...
// Page object
$MasterFacilitiesEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { master_facilities: currentTable } });
var currentForm, currentPageID;
var fmaster_facilitiesedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmaster_facilitiesedit = new ew.Form("fmaster_facilitiesedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmaster_facilitiesedit;

    // Add fields
    var fields = currentTable.fields;
    fmaster_facilitiesedit.addFields([
    ...
    // Form_CustomValidate
    fmaster_facilitiesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmaster_facilitiesedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmaster_facilitiesedit.lists.master_facilities_group_id = <?= $Page->master_facilities_group_id->toClientList($Page) ?>;
    fmaster_facilitiesedit.lists.isactive = <?= $Page->isactive->toClientList($Page) ?>;
    loadjs.done("fmaster_facilitiesedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmaster_facilitiesedit" id="fmaster_facilitiesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_master_facilities_master_facilities_group_id">
    <select
        ...
        data-select2-id="fmaster_facilitiesedit_x_master_facilities_group_id"
...
<script>
loadjs.ready("fmaster_facilitiesedit", function() {
    var options = { name: "x_master_facilities_group_id", selectId: "fmaster_facilitiesedit_x_master_facilities_group_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmaster_facilitiesedit.lists.master_facilities_group_id.lookupOptions.length) {
        options.data = { id: "x_master_facilities_group_id", form: "fmaster_facilitiesedit" };
    } else {
        options.ajax = { id: "x_master_facilities_group_id", form: "fmaster_facilitiesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="master_facilities" data-field="x_master_facilities_id" data-hidden="1" name="x_master_facilities_id" id="x_master_facilities_id" value="<?= HtmlEncode($Page->master_facilities_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
