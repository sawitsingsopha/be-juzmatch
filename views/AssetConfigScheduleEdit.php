...
// Page object
$AssetConfigScheduleEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { asset_config_schedule: currentTable } });
var currentForm, currentPageID;
var fasset_config_scheduleedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fasset_config_scheduleedit = new ew.Form("fasset_config_scheduleedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fasset_config_scheduleedit;

    // Add fields
    var fields = currentTable.fields;
    fasset_config_scheduleedit.addFields([
    ...
    // Form_CustomValidate
    fasset_config_scheduleedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fasset_config_scheduleedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fasset_config_scheduleedit.lists.status_approve = <?= $Page->status_approve->toClientList($Page) ?>;
    loadjs.done("fasset_config_scheduleedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fasset_config_scheduleedit" id="fasset_config_scheduleedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="asset_config_schedule" data-field="x_member_id" data-hidden="1" name="x_member_id" id="x_member_id" value="<?= HtmlEncode($Page->member_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="asset_config_schedule" data-field="x_asset_id" data-hidden="1" name="x_asset_id" id="x_asset_id" value="<?= HtmlEncode($Page->asset_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fasset_config_scheduleedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fasset_config_scheduleedit", "x_date_start_installment", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="asset_config_schedule" data-field="x_asset_config_schedule_id" data-hidden="1" name="x_asset_config_schedule_id" id="x_asset_config_schedule_id" value="<?= HtmlEncode($Page->asset_config_schedule_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
