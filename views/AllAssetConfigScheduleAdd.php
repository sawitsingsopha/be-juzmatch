...
// Page object
$AllAssetConfigScheduleAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { all_asset_config_schedule: currentTable } });
var currentForm, currentPageID;
var fall_asset_config_scheduleadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fall_asset_config_scheduleadd = new ew.Form("fall_asset_config_scheduleadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fall_asset_config_scheduleadd;

    // Add fields
    var fields = currentTable.fields;
    fall_asset_config_scheduleadd.addFields([
    ...
    // Form_CustomValidate
    fall_asset_config_scheduleadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fall_asset_config_scheduleadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fall_asset_config_scheduleadd.lists.member_id = <?= $Page->member_id->toClientList($Page) ?>;
    fall_asset_config_scheduleadd.lists.asset_id = <?= $Page->asset_id->toClientList($Page) ?>;
    fall_asset_config_scheduleadd.lists.status_approve = <?= $Page->status_approve->toClientList($Page) ?>;
    loadjs.done("fall_asset_config_scheduleadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fall_asset_config_scheduleadd" id="fall_asset_config_scheduleadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_all_asset_config_schedule_member_id">
    <select
        ...
        data-select2-id="fall_asset_config_scheduleadd_x_member_id"
...
<script>
loadjs.ready("fall_asset_config_scheduleadd", function() {
    var options = { name: "x_member_id", selectId: "fall_asset_config_scheduleadd_x_member_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fall_asset_config_scheduleadd.lists.member_id.lookupOptions.length) {
        options.data = { id: "x_member_id", form: "fall_asset_config_scheduleadd" };
    } else {
        options.ajax = { id: "x_member_id", form: "fall_asset_config_scheduleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.all_asset_config_schedule.fields.member_id.selectOptions);
...
<span id="el_all_asset_config_schedule_asset_id">
    <select
        ...
        data-select2-id="fall_asset_config_scheduleadd_x_asset_id"
        data-table="all_asset_config_schedule"
...
<script>
loadjs.ready("fall_asset_config_scheduleadd", function() {
    var options = { name: "x_asset_id", selectId: "fall_asset_config_scheduleadd_x_asset_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fall_asset_config_scheduleadd.lists.asset_id.lookupOptions.length) {
        options.data = { id: "x_asset_id", form: "fall_asset_config_scheduleadd" };
    } else {
        options.ajax = { id: "x_asset_id", form: "fall_asset_config_scheduleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.all_asset_config_schedule.fields.asset_id.selectOptions);
    ew.createSelect(options);
});
</script>
</span>
...
<script>
loadjs.ready(["fall_asset_config_scheduleadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fall_asset_config_scheduleadd", "x_date_start_installment", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
