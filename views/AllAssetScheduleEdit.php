...
// Page object
$AllAssetScheduleEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { all_asset_schedule: currentTable } });
var currentForm, currentPageID;
var fall_asset_scheduleedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fall_asset_scheduleedit = new ew.Form("fall_asset_scheduleedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fall_asset_scheduleedit;

    // Add fields
    var fields = currentTable.fields;
    fall_asset_scheduleedit.addFields([
    ...
    // Form_CustomValidate
    fall_asset_scheduleedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fall_asset_scheduleedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fall_asset_scheduleedit.lists.status_payment = <?= $Page->status_payment->toClientList($Page) ?>;
    loadjs.done("fall_asset_scheduleedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fall_asset_scheduleedit" id="fall_asset_scheduleedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="all_asset_schedule" data-field="x_asset_id" data-hidden="1" name="x_asset_id" id="x_asset_id" value="<?= HtmlEncode($Page->asset_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="all_asset_schedule" data-field="x_member_id" data-hidden="1" name="x_member_id" id="x_member_id" value="<?= HtmlEncode($Page->member_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fall_asset_scheduleedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fall_asset_scheduleedit", "x_expired_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fall_asset_scheduleedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fall_asset_scheduleedit", "x_date_payment", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_all_asset_schedule_status_payment">
    <select
        ...
        data-select2-id="fall_asset_scheduleedit_x_status_payment"
...
<script>
loadjs.ready("fall_asset_scheduleedit", function() {
    var options = { name: "x_status_payment", selectId: "fall_asset_scheduleedit_x_status_payment" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fall_asset_scheduleedit.lists.status_payment.lookupOptions.length) {
        options.data = { id: "x_status_payment", form: "fall_asset_scheduleedit" };
    } else {
        options.ajax = { id: "x_status_payment", form: "fall_asset_scheduleedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="all_asset_schedule" data-field="x_asset_schedule_id" data-hidden="1" name="x_asset_schedule_id" id="x_asset_schedule_id" value="<?= HtmlEncode($Page->asset_schedule_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
