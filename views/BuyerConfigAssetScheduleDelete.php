...
// Page object
$BuyerConfigAssetScheduleDelete = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { buyer_config_asset_schedule: currentTable } });
var currentForm, currentPageID;
var fbuyer_config_asset_scheduledelete;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fbuyer_config_asset_scheduledelete = new ew.Form("fbuyer_config_asset_scheduledelete", "delete");
    currentPageID = ew.PAGE_ID = "delete";
    currentForm = fbuyer_config_asset_scheduledelete;
    loadjs.done("fbuyer_config_asset_scheduledelete");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fbuyer_config_asset_scheduledelete" id="fbuyer_config_asset_scheduledelete" class="ew-form ew-delete-form" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->installment_all->Visible) { // installment_all ?>
        <th class="<?= $Page->installment_all->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_installment_all" class="buyer_config_asset_schedule_installment_all"><?= $Page->installment_all->caption() ?></span></th>
<?php } ?>
<?php if ($Page->installment_price_per->Visible) { // installment_price_per ?>
        <th class="<?= $Page->installment_price_per->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_installment_price_per" class="buyer_config_asset_schedule_installment_price_per"><?= $Page->installment_price_per->caption() ?></span></th>
<?php } ?>
<?php if ($Page->date_start_installment->Visible) { // date_start_installment ?>
        <th class="<?= $Page->date_start_installment->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_date_start_installment" class="buyer_config_asset_schedule_date_start_installment"><?= $Page->date_start_installment->caption() ?></span></th>
<?php } ?>
<?php if ($Page->status_approve->Visible) { // status_approve ?>
        <th class="<?= $Page->status_approve->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_status_approve" class="buyer_config_asset_schedule_status_approve"><?= $Page->status_approve->caption() ?></span></th>
<?php } ?>
<?php if ($Page->asset_price->Visible) { // asset_price ?>
        <th class="<?= $Page->asset_price->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_asset_price" class="buyer_config_asset_schedule_asset_price"><?= $Page->asset_price->caption() ?></span></th>
<?php } ?>
<?php if ($Page->booking_price->Visible) { // booking_price ?>
        <th class="<?= $Page->booking_price->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_booking_price" class="buyer_config_asset_schedule_booking_price"><?= $Page->booking_price->caption() ?></span></th>
<?php } ?>
<?php if ($Page->down_price->Visible) { // down_price ?>
        <th class="<?= $Page->down_price->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_down_price" class="buyer_config_asset_schedule_down_price"><?= $Page->down_price->caption() ?></span></th>
<?php } ?>
<?php if ($Page->move_in_on_20th->Visible) { // move_in_on_20th ?>
        <th class="<?= $Page->move_in_on_20th->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_move_in_on_20th" class="buyer_config_asset_schedule_move_in_on_20th"><?= $Page->move_in_on_20th->caption() ?></span></th>
<?php } ?>
<?php if ($Page->number_days_pay_first_month->Visible) { // number_days_pay_first_month ?>
        <th class="<?= $Page->number_days_pay_first_month->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_number_days_pay_first_month" class="buyer_config_asset_schedule_number_days_pay_first_month"><?= $Page->number_days_pay_first_month->caption() ?></span></th>
<?php } ?>
<?php if ($Page->number_days_in_first_month->Visible) { // number_days_in_first_month ?>
        <th class="<?= $Page->number_days_in_first_month->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_number_days_in_first_month" class="buyer_config_asset_schedule_number_days_in_first_month"><?= $Page->number_days_in_first_month->caption() ?></span></th>
<?php } ?>
<?php if ($Page->cdate->Visible) { // cdate ?>
        <th class="<?= $Page->cdate->headerCellClass() ?>"><span id="elh_buyer_config_asset_schedule_cdate" class="buyer_config_asset_schedule_cdate"><?= $Page->cdate->caption() ?></span></th>
<?php } ?>
    </tr>
    </thead>
    <tbody>
