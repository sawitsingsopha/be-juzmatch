...
// Page object
$BuyerAssetScheduleAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { buyer_asset_schedule: currentTable } });
var currentForm, currentPageID;
var fbuyer_asset_scheduleadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fbuyer_asset_scheduleadd = new ew.Form("fbuyer_asset_scheduleadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fbuyer_asset_scheduleadd;

    // Add fields
    var fields = currentTable.fields;
    fbuyer_asset_scheduleadd.addFields([
    ...
    // Form_CustomValidate
    fbuyer_asset_scheduleadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbuyer_asset_scheduleadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fbuyer_asset_scheduleadd.lists.asset_id = <?= $Page->asset_id->toClientList($Page) ?>;
    fbuyer_asset_scheduleadd.lists.member_id = <?= $Page->member_id->toClientList($Page) ?>;
    fbuyer_asset_scheduleadd.lists.status_payment = <?= $Page->status_payment->toClientList($Page) ?>;
    loadjs.done("fbuyer_asset_scheduleadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fbuyer_asset_scheduleadd" id="fbuyer_asset_scheduleadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_buyer_asset_schedule_asset_id">
    <select
        ...
        data-select2-id="fbuyer_asset_scheduleadd_x_asset_id"
...
<script>
loadjs.ready("fbuyer_asset_scheduleadd", function() {
    var options = { name: "x_asset_id", selectId: "fbuyer_asset_scheduleadd_x_asset_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbuyer_asset_scheduleadd.lists.asset_id.lookupOptions.length) {
        options.data = { id: "x_asset_id", form: "fbuyer_asset_scheduleadd" };
    } else {
        options.ajax = { id: "x_asset_id", form: "fbuyer_asset_scheduleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.buyer_asset_schedule.fields.asset_id.selectOptions);
...
<span id="el_buyer_asset_schedule_member_id">
    <select
        ...
        data-select2-id="fbuyer_asset_scheduleadd_x_member_id"
        data-table="buyer_asset_schedule"
...
<script>
loadjs.ready("fbuyer_asset_scheduleadd", function() {
    var options = { name: "x_member_id", selectId: "fbuyer_asset_scheduleadd_x_member_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbuyer_asset_scheduleadd.lists.member_id.lookupOptions.length) {
        options.data = { id: "x_member_id", form: "fbuyer_asset_scheduleadd" };
    } else {
        options.ajax = { id: "x_member_id", form: "fbuyer_asset_scheduleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.buyer_asset_schedule.fields.member_id.selectOptions);
    ew.createSelect(options);
});
</script>
</span>
...
<script>
loadjs.ready(["fbuyer_asset_scheduleadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fbuyer_asset_scheduleadd", "x_expired_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fbuyer_asset_scheduleadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fbuyer_asset_scheduleadd", "x_date_payment", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_buyer_asset_schedule_status_payment">
    <select
        ...
        data-select2-id="fbuyer_asset_scheduleadd_x_status_payment"
        data-table="buyer_asset_schedule"
...
<script>
loadjs.ready("fbuyer_asset_scheduleadd", function() {
    var options = { name: "x_status_payment", selectId: "fbuyer_asset_scheduleadd_x_status_payment" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbuyer_asset_scheduleadd.lists.status_payment.lookupOptions.length) {
        options.data = { id: "x_status_payment", form: "fbuyer_asset_scheduleadd" };
    } else {
        options.ajax = { id: "x_status_payment", form: "fbuyer_asset_scheduleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
