...
var fmpay_req_logedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmpay_req_logedit = new ew.Form("fmpay_req_logedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmpay_req_logedit;

    // Add fields
    var fields = currentTable.fields;
    fmpay_req_logedit.addFields([
    ...
    // Form_CustomValidate
    fmpay_req_logedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmpay_req_logedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fmpay_req_logedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmpay_req_logedit" id="fmpay_req_logedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_mpay_req_log_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_mpay_req_log_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="mpay_req_log" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fmpay_req_logedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fmpay_req_logedit", "x_req_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
