...
// Page object
$AllAssetScheduleAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { all_asset_schedule: currentTable } });
var currentForm, currentPageID;
var fall_asset_scheduleadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fall_asset_scheduleadd = new ew.Form("fall_asset_scheduleadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fall_asset_scheduleadd;

    // Add fields
    var fields = currentTable.fields;
    fall_asset_scheduleadd.addFields([
    ...
    // Form_CustomValidate
    fall_asset_scheduleadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fall_asset_scheduleadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fall_asset_scheduleadd.lists.asset_id = <?= $Page->asset_id->toClientList($Page) ?>;
    fall_asset_scheduleadd.lists.member_id = <?= $Page->member_id->toClientList($Page) ?>;
    fall_asset_scheduleadd.lists.status_payment = <?= $Page->status_payment->toClientList($Page) ?>;
    loadjs.done("fall_asset_scheduleadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fall_asset_scheduleadd" id="fall_asset_scheduleadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="t" value="all_asset_schedule">
...
<?php if ($Page->asset_id->Visible) { // asset_id ?>
    <div id="r_asset_id"<?= $Page->asset_id->rowAttributes() ?>>
        <label id="elh_all_asset_schedule_asset_id" for="x_asset_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->asset_id->caption() ?><?= $Page->asset_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->asset_id->cellAttributes() ?>>
<?php if ($Page->asset_id->getSessionValue() != "") { ?>
<span id="el_all_asset_schedule_asset_id">
...
<span id="el_all_asset_schedule_asset_id">
    <select
        ...
        data-select2-id="fall_asset_scheduleadd_x_asset_id"
...
<script>
loadjs.ready("fall_asset_scheduleadd", function() {
    var options = { name: "x_asset_id", selectId: "fall_asset_scheduleadd_x_asset_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fall_asset_scheduleadd.lists.asset_id.lookupOptions.length) {
        options.data = { id: "x_asset_id", form: "fall_asset_scheduleadd" };
    } else {
        options.ajax = { id: "x_asset_id", form: "fall_asset_scheduleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php if ($Page->member_id->Visible) { // member_id ?>
    <div id="r_member_id"<?= $Page->member_id->rowAttributes() ?>>
        <label id="elh_all_asset_schedule_member_id" for="x_member_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->member_id->caption() ?><?= $Page->member_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->member_id->cellAttributes() ?>>
<?php if ($Page->member_id->getSessionValue() != "") { ?>
<span id="el_all_asset_schedule_member_id">
...
<span id="el_all_asset_schedule_member_id">
    <select
        ...
        data-select2-id="fall_asset_scheduleadd_x_member_id"
        data-table="all_asset_schedule"
...
<script>
loadjs.ready("fall_asset_scheduleadd", function() {
    var options = { name: "x_member_id", selectId: "fall_asset_scheduleadd_x_member_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fall_asset_scheduleadd.lists.member_id.lookupOptions.length) {
        options.data = { id: "x_member_id", form: "fall_asset_scheduleadd" };
    } else {
        options.ajax = { id: "x_member_id", form: "fall_asset_scheduleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.all_asset_schedule.fields.member_id.selectOptions);
    ew.createSelect(options);
});
</script>
</span>
...
<?php if ($Page->installment_all->Visible) { // installment_all ?>
    <div id="r_installment_all"<?= $Page->installment_all->rowAttributes() ?>>
        <label id="elh_all_asset_schedule_installment_all" for="x_installment_all" class="<?= $Page->LeftColumnClass ?>"><?= $Page->installment_all->caption() ?><?= $Page->installment_all->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->installment_all->cellAttributes() ?>>
<span id="el_all_asset_schedule_installment_all">
<input type="<?= $Page->installment_all->getInputTextType() ?>" name="x_installment_all" id="x_installment_all" data-table="all_asset_schedule" data-field="x_installment_all" value="<?= $Page->installment_all->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Page->installment_all->getPlaceHolder()) ?>"<?= $Page->installment_all->editAttributes() ?> aria-describedby="x_installment_all_help">
...
<?php if ($Page->num_installment->Visible) { // num_installment ?>
    <div id="r_num_installment"<?= $Page->num_installment->rowAttributes() ?>>
        <label id="elh_all_asset_schedule_num_installment" for="x_num_installment" class="<?= $Page->LeftColumnClass ?>"><?= $Page->num_installment->caption() ?><?= $Page->num_installment->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->num_installment->cellAttributes() ?>>
<span id="el_all_asset_schedule_num_installment">
...
<?php if ($Page->receive_per_installment_invertor->Visible) { // receive_per_installment_invertor ?>
    <div id="r_receive_per_installment_invertor"<?= $Page->receive_per_installment_invertor->rowAttributes() ?>>
        <label id="elh_all_asset_schedule_receive_per_installment_invertor" for="x_receive_per_installment_invertor" class="<?= $Page->LeftColumnClass ?>"><?= $Page->receive_per_installment_invertor->caption() ?><?= $Page->receive_per_installment_invertor->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->receive_per_installment_invertor->cellAttributes() ?>>
<span id="el_all_asset_schedule_receive_per_installment_invertor">
<input type="<?= $Page->receive_per_installment_invertor->getInputTextType() ?>" name="x_receive_per_installment_invertor" id="x_receive_per_installment_invertor" data-table="all_asset_schedule" data-type="currency" data-field="x_receive_per_installment_invertor"  value="<?= $Page->receive_per_installment_invertor->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Page->receive_per_installment_invertor->getPlaceHolder()) ?>"<?= $Page->receive_per_installment_invertor->editAttributes() ?> aria-describedby="x_receive_per_installment_invertor_help">
...
<?php if ($Page->expired_date->Visible) { // expired_date ?>
    <div id="r_expired_date"<?= $Page->expired_date->rowAttributes() ?>>
        <label id="elh_all_asset_schedule_expired_date" for="x_expired_date" class="<?= $Page->LeftColumnClass ?>"><?= $Page->expired_date->caption() ?><?= $Page->expired_date->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->expired_date->cellAttributes() ?>>
<span id="el_all_asset_schedule_expired_date">
<input type="<?= $Page->expired_date->getInputTextType() ?>" name="x_expired_date" id="x_expired_date" data-table="all_asset_schedule" data-field="x_expired_date" value="<?= $Page->expired_date->EditValue ?>" placeholder="<?= HtmlEncode($Page->expired_date->getPlaceHolder()) ?>"<?= $Page->expired_date->editAttributes() ?> aria-describedby="x_expired_date_help">
...
<script>
loadjs.ready(["fall_asset_scheduleadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fall_asset_scheduleadd", "x_expired_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->date_payment->Visible) { // date_payment ?>
    <div id="r_date_payment"<?= $Page->date_payment->rowAttributes() ?>>
        <label id="elh_all_asset_schedule_date_payment" for="x_date_payment" class="<?= $Page->LeftColumnClass ?>"><?= $Page->date_payment->caption() ?><?= $Page->date_payment->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->date_payment->cellAttributes() ?>>
<span id="el_all_asset_schedule_date_payment">
<input type="<?= $Page->date_payment->getInputTextType() ?>" name="x_date_payment" id="x_date_payment" data-table="all_asset_schedule" data-field="x_date_payment" value="<?= $Page->date_payment->EditValue ?>" placeholder="<?= HtmlEncode($Page->date_payment->getPlaceHolder()) ?>"<?= $Page->date_payment->editAttributes() ?> aria-describedby="x_date_payment_help">
...
loadjs.ready(["fall_asset_scheduleadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fall_asset_scheduleadd", "x_date_payment", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->status_payment->Visible) { // status_payment ?>
    <div id="r_status_payment"<?= $Page->status_payment->rowAttributes() ?>>
        <label id="elh_all_asset_schedule_status_payment" for="x_status_payment" class="<?= $Page->LeftColumnClass ?>"><?= $Page->status_payment->caption() ?><?= $Page->status_payment->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->status_payment->cellAttributes() ?>>
<span id="el_all_asset_schedule_status_payment">
    <select
        id="x_status_payment"
        name="x_status_payment"
        class="form-select ew-select<?= $Page->status_payment->isInvalidClass() ?>"
        data-select2-id="fall_asset_scheduleadd_x_status_payment"
        data-table="all_asset_schedule"
...
<script>
loadjs.ready("fall_asset_scheduleadd", function() {
    var options = { name: "x_status_payment", selectId: "fall_asset_scheduleadd_x_status_payment" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fall_asset_scheduleadd.lists.status_payment.lookupOptions.length) {
        options.data = { id: "x_status_payment", form: "fall_asset_scheduleadd" };
    } else {
        options.ajax = { id: "x_status_payment", form: "fall_asset_scheduleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
