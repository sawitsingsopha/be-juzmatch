...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fdoc_credenedit = new ew.Form("fdoc_credenedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fdoc_credenedit;

    // Add fields
    var fields = currentTable.fields;
    fdoc_credenedit.addFields([
        ...
        ["doc_creden_id", [fields.doc_creden_id.visible && fields.doc_creden_id.required ? ew.Validators.required(fields.doc_creden_id.caption) : null], fields.doc_creden_id.isInvalid],
    ...
    // Form_CustomValidate
    fdoc_credenedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fdoc_credenedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fdoc_credenedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fdoc_credenedit" id="fdoc_credenedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->doc_creden_id->Visible) { // doc_creden_id ?>
    <div id="r_doc_creden_id"<?= $Page->doc_creden_id->rowAttributes() ?>>
        <label id="elh_doc_creden_doc_creden_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->doc_creden_id->caption() ?><?= $Page->doc_creden_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->doc_creden_id->getDisplayValue($Page->doc_creden_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="doc_creden" data-field="x_doc_creden_id" data-hidden="1" name="x_doc_creden_id" id="x_doc_creden_id" value="<?= HtmlEncode($Page->doc_creden_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fdoc_credenedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdoc_credenedit", "x_cdate", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fdoc_credenedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdoc_credenedit", "x_udate", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
