...
ew.deepAssign(ew.vars, { tables: { number_deals_available: currentTable } });
var currentForm, currentPageID;
var fnumber_deals_availablelist;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fnumber_deals_availablelist = new ew.Form("fnumber_deals_availablelist", "list");
    currentPageID = ew.PAGE_ID = "list";
    currentForm = fnumber_deals_availablelist;
    fnumber_deals_availablelist.formKeyCountName = "<?= $Page->FormKeyCountName ?>";
    loadjs.done("fnumber_deals_availablelist");
});
var fnumber_deals_availablesrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object for search
    fnumber_deals_availablesrch = new ew.Form("fnumber_deals_availablesrch", "list");
    currentSearchForm = fnumber_deals_availablesrch;

    // Add fields
    var fields = currentTable.fields;
    fnumber_deals_availablesrch.addFields([
    ...
    // Validate form
    fnumber_deals_availablesrch.validate = function () {
    ...
    // Form_CustomValidate
    fnumber_deals_availablesrch.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fnumber_deals_availablesrch.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists

    // Filters
    fnumber_deals_availablesrch.filterList = <?= $Page->getFilterList() ?>;
    loadjs.done("fnumber_deals_availablesrch");
});
</script>
<script>
...
<form name="fnumber_deals_availablesrch" id="fnumber_deals_availablesrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>">
<div id="fnumber_deals_availablesrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="number_deals_available">
...
<input type="hidden" name="z_asset_price" id="z_asset_price" value="=">
        ...
        <div id="el_number_deals_available_asset_price" class="ew-search-field">
<input type="<?= $Page->asset_price->getInputTextType() ?>" name="x_asset_price" id="x_asset_price" data-table="number_deals_available" data-field="x_asset_price" value="<?= $Page->asset_price->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Page->asset_price->getPlaceHolder()) ?>"<?= $Page->asset_price->editAttributes() ?>>
...
<input type="hidden" name="z_price_paid" id="z_price_paid" value="=">
        ...
        <div id="el_number_deals_available_price_paid" class="ew-search-field">
<input type="<?= $Page->price_paid->getInputTextType() ?>" name="x_price_paid" id="x_price_paid" data-table="number_deals_available" data-field="x_price_paid" value="<?= $Page->price_paid->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Page->price_paid->getPlaceHolder()) ?>"<?= $Page->price_paid->editAttributes() ?>>
...
<input type="hidden" name="z_remaining_price" id="z_remaining_price" value="=">
        ...
        <div id="el_number_deals_available_remaining_price" class="ew-search-field">
<input type="<?= $Page->remaining_price->getInputTextType() ?>" name="x_remaining_price" id="x_remaining_price" data-table="number_deals_available" data-field="x_remaining_price" value="<?= $Page->remaining_price->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Page->remaining_price->getPlaceHolder()) ?>"<?= $Page->remaining_price->editAttributes() ?>>
...
<input type="hidden" name="z_expiration_date" id="z_expiration_date" value="BETWEEN">
        ...
        <div id="el_number_deals_available_expiration_date" class="ew-search-field">
<input type="<?= $Page->expiration_date->getInputTextType() ?>" name="x_expiration_date" id="x_expiration_date" data-table="number_deals_available" data-field="x_expiration_date" value="<?= $Page->expiration_date->EditValue ?>" placeholder="<?= HtmlEncode($Page->expiration_date->getPlaceHolder()) ?>"<?= $Page->expiration_date->editAttributes() ?>>
...
<script>
loadjs.ready(["fnumber_deals_availablesrch", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fnumber_deals_availablesrch", "x_expiration_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</div>
        ...
        <div id="el2_number_deals_available_expiration_date" class="ew-search-field2">
<input type="<?= $Page->expiration_date->getInputTextType() ?>" name="y_expiration_date" id="y_expiration_date" data-table="number_deals_available" data-field="x_expiration_date" value="<?= $Page->expiration_date->EditValue2 ?>" placeholder="<?= HtmlEncode($Page->expiration_date->getPlaceHolder()) ?>"<?= $Page->expiration_date->editAttributes() ?>>
<div class="invalid-feedback"><?= $Page->expiration_date->getErrorMessage(false) ?></div>
<?php if (!$Page->expiration_date->ReadOnly && !$Page->expiration_date->Disabled && !isset($Page->expiration_date->EditAttrs["readonly"]) && !isset($Page->expiration_date->EditAttrs["disabled"])) { ?>
<script>
loadjs.ready(["fnumber_deals_availablesrch", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fnumber_deals_availablesrch", "y_expiration_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</div>
...
<div class="card ew-card ew-grid<?php if ($Page->isAddOrEdit()) { ?> ew-grid-add-edit<?php } ?> number_deals_available">
<form name="fnumber_deals_availablelist" id="fnumber_deals_availablelist" class="ew-form ew-list-form" action="<?= CurrentPageUrl(false) ?>" method="post">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="t" value="number_deals_available">
<div id="gmp_number_deals_available" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit()) { ?>
<table id="tbl_number_deals_availablelist" class="table table-bordered table-hover table-sm ew-table"><!-- .ew-table -->
<thead>
...
<?php if ($Page->asset_code->Visible) { // asset_code ?>
        <th data-name="asset_code" class="<?= $Page->asset_code->headerCellClass() ?>"><div id="elh_number_deals_available_asset_code" class="number_deals_available_asset_code"><?= $Page->renderFieldHeader($Page->asset_code) ?></div></th>
<?php } ?>
<?php if ($Page->_title->Visible) { // title ?>
        <th data-name="_title" class="<?= $Page->_title->headerCellClass() ?>"><div id="elh_number_deals_available__title" class="number_deals_available__title"><?= $Page->renderFieldHeader($Page->_title) ?></div></th>
<?php } ?>
<?php if ($Page->asset_price->Visible) { // asset_price ?>
        <th data-name="asset_price" class="<?= $Page->asset_price->headerCellClass() ?>"><div id="elh_number_deals_available_asset_price" class="number_deals_available_asset_price"><?= $Page->renderFieldHeader($Page->asset_price) ?></div></th>
<?php } ?>
<?php if ($Page->price_paid->Visible) { // price_paid ?>
        <th data-name="price_paid" class="<?= $Page->price_paid->headerCellClass() ?>"><div id="elh_number_deals_available_price_paid" class="number_deals_available_price_paid"><?= $Page->renderFieldHeader($Page->price_paid) ?></div></th>
<?php } ?>
<?php if ($Page->remaining_price->Visible) { // remaining_price ?>
        <th data-name="remaining_price" class="<?= $Page->remaining_price->headerCellClass() ?>"><div id="elh_number_deals_available_remaining_price" class="number_deals_available_remaining_price"><?= $Page->renderFieldHeader($Page->remaining_price) ?></div></th>
<?php } ?>
<?php if ($Page->expiration_date->Visible) { // expiration_date ?>
        <th data-name="expiration_date" class="<?= $Page->expiration_date->headerCellClass() ?>"><div id="elh_number_deals_available_expiration_date" class="number_deals_available_expiration_date"><?= $Page->renderFieldHeader($Page->expiration_date) ?></div></th>
<?php } ?>
<?php if ($Page->accrued_period_diff->Visible) { // accrued_period_diff ?>
        <th data-name="accrued_period_diff" class="<?= $Page->accrued_period_diff->headerCellClass() ?>"><div id="elh_number_deals_available_accrued_period_diff" class="number_deals_available_accrued_period_diff"><?= $Page->renderFieldHeader($Page->accrued_period_diff) ?></div></th>
<?php } ?>
<?php
...
while ($Page->RecordCount < $Page->StopRecord) {
    ...
    if ($Page->RecordCount >= $Page->StartRecord) {
        ...
        $Page->RowAttrs->merge([
            ...
            "id" => "r" . $Page->RowCount . "_number_deals_available",
...
<span id="el<?= $Page->RowCount ?>_number_deals_available_asset_code" class="el_number_deals_available_asset_code">
...
<span id="el<?= $Page->RowCount ?>_number_deals_available__title" class="el_number_deals_available__title">
...
<span id="el<?= $Page->RowCount ?>_number_deals_available_asset_price" class="el_number_deals_available_asset_price">
...
<span id="el<?= $Page->RowCount ?>_number_deals_available_price_paid" class="el_number_deals_available_price_paid">
...
<span id="el<?= $Page->RowCount ?>_number_deals_available_remaining_price" class="el_number_deals_available_remaining_price">
...
<span id="el<?= $Page->RowCount ?>_number_deals_available_expiration_date" class="el_number_deals_available_expiration_date">
...
<span id="el<?= $Page->RowCount ?>_number_deals_available_accrued_period_diff" class="el_number_deals_available_accrued_period_diff">
...
loadjs.ready("fixedheadertable", function () {
    ew.fixedHeaderTable({
        delay: 0,
        container: "gmp_number_deals_available",
        width: "100%",
        height: "500px"
    });
});
...
loadjs.ready("head", function() {
    ew.addEventHandlers("number_deals_available");
});
</script>
<script>
