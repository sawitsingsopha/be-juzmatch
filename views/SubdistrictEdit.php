...
// Page object
$SubdistrictEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { subdistrict: currentTable } });
var currentForm, currentPageID;
var fsubdistrictedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fsubdistrictedit = new ew.Form("fsubdistrictedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fsubdistrictedit;

    // Add fields
    var fields = currentTable.fields;
    fsubdistrictedit.addFields([
        ["subdistrict_id", [fields.subdistrict_id.visible && fields.subdistrict_id.required ? ew.Validators.required(fields.subdistrict_id.caption) : null], fields.subdistrict_id.isInvalid],
    ...
    // Form_CustomValidate
    fsubdistrictedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsubdistrictedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fsubdistrictedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fsubdistrictedit" id="fsubdistrictedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->subdistrict_id->Visible) { // subdistrict_id ?>
    <div id="r_subdistrict_id"<?= $Page->subdistrict_id->rowAttributes() ?>>
        <label id="elh_subdistrict_subdistrict_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->subdistrict_id->caption() ?><?= $Page->subdistrict_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->subdistrict_id->cellAttributes() ?>>
<span id="el_subdistrict_subdistrict_id">
<span<?= $Page->subdistrict_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->subdistrict_id->getDisplayValue($Page->subdistrict_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="subdistrict" data-field="x_subdistrict_id" data-hidden="1" name="x_subdistrict_id" id="x_subdistrict_id" value="<?= HtmlEncode($Page->subdistrict_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
