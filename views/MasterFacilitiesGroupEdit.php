...
// Page object
$MasterFacilitiesGroupEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { master_facilities_group: currentTable } });
var currentForm, currentPageID;
var fmaster_facilities_groupedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmaster_facilities_groupedit = new ew.Form("fmaster_facilities_groupedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmaster_facilities_groupedit;

    // Add fields
    var fields = currentTable.fields;
    fmaster_facilities_groupedit.addFields([
    ...
    // Form_CustomValidate
    fmaster_facilities_groupedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmaster_facilities_groupedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmaster_facilities_groupedit.lists.isactive = <?= $Page->isactive->toClientList($Page) ?>;
    loadjs.done("fmaster_facilities_groupedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmaster_facilities_groupedit" id="fmaster_facilities_groupedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } ?>
    ...
    <input type="hidden" data-table="master_facilities_group" data-field="x_master_facilities_group_id" data-hidden="1" name="x_master_facilities_group_id" id="x_master_facilities_group_id" value="<?= HtmlEncode($Page->master_facilities_group_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
