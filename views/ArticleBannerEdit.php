...
// Page object
$ArticleBannerEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { article_banner: currentTable } });
var currentForm, currentPageID;
var farticle_banneredit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    farticle_banneredit = new ew.Form("farticle_banneredit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = farticle_banneredit;

    // Add fields
    var fields = currentTable.fields;
    farticle_banneredit.addFields([
    ...
    // Form_CustomValidate
    farticle_banneredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farticle_banneredit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    farticle_banneredit.lists.active_status = <?= $Page->active_status->toClientList($Page) ?>;
    loadjs.done("farticle_banneredit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="farticle_banneredit" id="farticle_banneredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } ?>
    ...
    <input type="hidden" data-table="article_banner" data-field="x_article_banner_id" data-hidden="1" name="x_article_banner_id" id="x_article_banner_id" value="<?= HtmlEncode($Page->article_banner_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
