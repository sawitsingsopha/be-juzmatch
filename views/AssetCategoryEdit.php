...
// Page object
$AssetCategoryEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { asset_category: currentTable } });
var currentForm, currentPageID;
var fasset_categoryedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fasset_categoryedit = new ew.Form("fasset_categoryedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fasset_categoryedit;

    // Add fields
    var fields = currentTable.fields;
    fasset_categoryedit.addFields([
    ...
    // Form_CustomValidate
    fasset_categoryedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fasset_categoryedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fasset_categoryedit.lists.category_id = <?= $Page->category_id->toClientList($Page) ?>;
    fasset_categoryedit.lists.asset_id = <?= $Page->asset_id->toClientList($Page) ?>;
    loadjs.done("fasset_categoryedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fasset_categoryedit" id="fasset_categoryedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_asset_category_category_id">
    <select
        ...
        data-select2-id="fasset_categoryedit_x_category_id"
...
<script>
loadjs.ready("fasset_categoryedit", function() {
    var options = { name: "x_category_id", selectId: "fasset_categoryedit_x_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasset_categoryedit.lists.category_id.lookupOptions.length) {
        options.data = { id: "x_category_id", form: "fasset_categoryedit" };
    } else {
        options.ajax = { id: "x_category_id", form: "fasset_categoryedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_asset_category_asset_id">
    <select
        ...
        data-select2-id="fasset_categoryedit_x_asset_id"
        data-table="asset_category"
...
<script>
loadjs.ready("fasset_categoryedit", function() {
    var options = { name: "x_asset_id", selectId: "fasset_categoryedit_x_asset_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasset_categoryedit.lists.asset_id.lookupOptions.length) {
        options.data = { id: "x_asset_id", form: "fasset_categoryedit" };
    } else {
        options.ajax = { id: "x_asset_id", form: "fasset_categoryedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="asset_category" data-field="x_asset_category_id" data-hidden="1" name="x_asset_category_id" id="x_asset_category_id" value="<?= HtmlEncode($Page->asset_category_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
