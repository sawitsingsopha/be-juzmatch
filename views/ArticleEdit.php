...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    farticleedit = new ew.Form("farticleedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = farticleedit;

    // Add fields
    var fields = currentTable.fields;
    farticleedit.addFields([
    ...
    // Form_CustomValidate
    farticleedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farticleedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    farticleedit.lists.article_category_id = <?= $Page->article_category_id->toClientList($Page) ?>;
    farticleedit.lists.highlight = <?= $Page->highlight->toClientList($Page) ?>;
    farticleedit.lists.active_status = <?= $Page->active_status->toClientList($Page) ?>;
    loadjs.done("farticleedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="farticleedit" id="farticleedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_article_article_category_id">
    <select
        ...
        data-select2-id="farticleedit_x_article_category_id"
        data-table="article"
...
<script>
loadjs.ready("farticleedit", function() {
    var options = { name: "x_article_category_id", selectId: "farticleedit_x_article_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (farticleedit.lists.article_category_id.lookupOptions.length) {
        options.data = { id: "x_article_category_id", form: "farticleedit" };
    } else {
        options.ajax = { id: "x_article_category_id", form: "farticleedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="article" data-field="x_article_id" data-hidden="1" name="x_article_id" id="x_article_id" value="<?= HtmlEncode($Page->article_id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
