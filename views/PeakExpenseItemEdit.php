...
// Page object
$PeakExpenseItemEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { peak_expense_item: currentTable } });
var currentForm, currentPageID;
var fpeak_expense_itemedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpeak_expense_itemedit = new ew.Form("fpeak_expense_itemedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpeak_expense_itemedit;

    // Add fields
    var fields = currentTable.fields;
    fpeak_expense_itemedit.addFields([
        ["peak_expense_item_id", [fields.peak_expense_item_id.visible && fields.peak_expense_item_id.required ? ew.Validators.required(fields.peak_expense_item_id.caption) : null], fields.peak_expense_item_id.isInvalid],
    ...
    // Form_CustomValidate
    fpeak_expense_itemedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpeak_expense_itemedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpeak_expense_itemedit.lists.isdelete = <?= $Page->isdelete->toClientList($Page) ?>;
    loadjs.done("fpeak_expense_itemedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fpeak_expense_itemedit" id="fpeak_expense_itemedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->peak_expense_item_id->Visible) { // peak_expense_item_id ?>
    <div id="r_peak_expense_item_id"<?= $Page->peak_expense_item_id->rowAttributes() ?>>
        <label id="elh_peak_expense_item_peak_expense_item_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->peak_expense_item_id->caption() ?><?= $Page->peak_expense_item_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->peak_expense_item_id->cellAttributes() ?>>
<span id="el_peak_expense_item_peak_expense_item_id">
<span<?= $Page->peak_expense_item_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->peak_expense_item_id->getDisplayValue($Page->peak_expense_item_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="peak_expense_item" data-field="x_peak_expense_item_id" data-hidden="1" name="x_peak_expense_item_id" id="x_peak_expense_item_id" value="<?= HtmlEncode($Page->peak_expense_item_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="form-check form-switch d-inline-block">
    <input type="checkbox" class="form-check-input<?= $Page->isdelete->isInvalidClass() ?>" data-table="peak_expense_item" data-field="x_isdelete" name="x_isdelete[]" id="x_isdelete_474742" value="1"<?= ConvertToBool($Page->isdelete->CurrentValue) ? " checked" : "" ?><?= $Page->isdelete->editAttributes() ?> aria-describedby="x_isdelete_help">
...
<script>
loadjs.ready(["fpeak_expense_itemedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpeak_expense_itemedit", "x_cdate", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fpeak_expense_itemedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpeak_expense_itemedit", "x_udate", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
