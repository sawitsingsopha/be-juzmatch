...
// Page object
$SettingLangEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { setting_lang: currentTable } });
var currentForm, currentPageID;
var fsetting_langedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fsetting_langedit = new ew.Form("fsetting_langedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fsetting_langedit;

    // Add fields
    var fields = currentTable.fields;
    fsetting_langedit.addFields([
        ["setting_lang_id", [fields.setting_lang_id.visible && fields.setting_lang_id.required ? ew.Validators.required(fields.setting_lang_id.caption) : null], fields.setting_lang_id.isInvalid],
    ...
    // Form_CustomValidate
    fsetting_langedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsetting_langedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fsetting_langedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fsetting_langedit" id="fsetting_langedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->setting_lang_id->Visible) { // setting_lang_id ?>
    <div id="r_setting_lang_id"<?= $Page->setting_lang_id->rowAttributes() ?>>
        <label id="elh_setting_lang_setting_lang_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->setting_lang_id->caption() ?><?= $Page->setting_lang_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->setting_lang_id->cellAttributes() ?>>
<span id="el_setting_lang_setting_lang_id">
<span<?= $Page->setting_lang_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->setting_lang_id->getDisplayValue($Page->setting_lang_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="setting_lang" data-field="x_setting_lang_id" data-hidden="1" name="x_setting_lang_id" id="x_setting_lang_id" value="<?= HtmlEncode($Page->setting_lang_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fsetting_langedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsetting_langedit", "x_udate", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
