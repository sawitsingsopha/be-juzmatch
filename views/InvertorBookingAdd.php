...
// Page object
$InvertorBookingAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { invertor_booking: currentTable } });
var currentForm, currentPageID;
var finvertor_bookingadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    finvertor_bookingadd = new ew.Form("finvertor_bookingadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = finvertor_bookingadd;

    // Add fields
    var fields = currentTable.fields;
    finvertor_bookingadd.addFields([
    ...
    // Form_CustomValidate
    finvertor_bookingadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    finvertor_bookingadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    finvertor_bookingadd.lists.member_id = <?= $Page->member_id->toClientList($Page) ?>;
    finvertor_bookingadd.lists.asset_id = <?= $Page->asset_id->toClientList($Page) ?>;
    finvertor_bookingadd.lists.status_expire = <?= $Page->status_expire->toClientList($Page) ?>;
    finvertor_bookingadd.lists.payment_status = <?= $Page->payment_status->toClientList($Page) ?>;
    loadjs.done("finvertor_bookingadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="finvertor_bookingadd" id="finvertor_bookingadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_invertor_booking_member_id">
    <select
        ...
        data-select2-id="finvertor_bookingadd_x_member_id"
...
<script>
loadjs.ready("finvertor_bookingadd", function() {
    var options = { name: "x_member_id", selectId: "finvertor_bookingadd_x_member_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (finvertor_bookingadd.lists.member_id.lookupOptions.length) {
        options.data = { id: "x_member_id", form: "finvertor_bookingadd" };
    } else {
        options.ajax = { id: "x_member_id", form: "finvertor_bookingadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.invertor_booking.fields.member_id.selectOptions);
...
<span id="el_invertor_booking_asset_id">
    <select
        ...
        data-select2-id="finvertor_bookingadd_x_asset_id"
        data-table="invertor_booking"
...
<script>
loadjs.ready("finvertor_bookingadd", function() {
    var options = { name: "x_asset_id", selectId: "finvertor_bookingadd_x_asset_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (finvertor_bookingadd.lists.asset_id.lookupOptions.length) {
        options.data = { id: "x_asset_id", form: "finvertor_bookingadd" };
    } else {
        options.ajax = { id: "x_asset_id", form: "finvertor_bookingadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["finvertor_bookingadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("finvertor_bookingadd", "x_date_booking", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("payment_inverter_booking", explode(",", $Page->getCurrentDetailTable())) && $payment_inverter_booking->DetailAdd) {
?>
