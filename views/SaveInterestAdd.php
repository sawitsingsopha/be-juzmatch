...
// Page object
$SaveInterestAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { save_interest: currentTable } });
var currentForm, currentPageID;
var fsave_interestadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fsave_interestadd = new ew.Form("fsave_interestadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fsave_interestadd;

    // Add fields
    var fields = currentTable.fields;
    fsave_interestadd.addFields([
    ...
    // Form_CustomValidate
    fsave_interestadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsave_interestadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fsave_interestadd.lists.member_id = <?= $Page->member_id->toClientList($Page) ?>;
    loadjs.done("fsave_interestadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fsave_interestadd" id="fsave_interestadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_save_interest_member_id">
    <select
        ...
        data-select2-id="fsave_interestadd_x_member_id"
        data-table="save_interest"
...
<script>
loadjs.ready("fsave_interestadd", function() {
    var options = { name: "x_member_id", selectId: "fsave_interestadd_x_member_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsave_interestadd.lists.member_id.lookupOptions.length) {
        options.data = { id: "x_member_id", form: "fsave_interestadd" };
    } else {
        options.ajax = { id: "x_member_id", form: "fsave_interestadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.save_interest.fields.member_id.selectOptions);
